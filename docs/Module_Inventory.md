...
Routes stay `/donations/*`. Operations: `/donations/payments/transactions`, `/recurring`, `/import-match`, `/receipts`. Analytics: `/donations/reports` (Giving Summary, Donor Giving, Campaign Performance, Pledge Performance, Recurring Giving). Campaign Performance **Campaign Groups** and Campaign → Groups both **Export CSV**. Donor Giving (Individual / Household / Group) lists gifts only — pledge status and outstanding balance stay on **Pledges**.
...
Campaign workspace tabs: Overview, **Event**, **Fundraising Plan**, Pledges, Donations, **Sponsorship**, Groups, **Wishlist**. Workspace tabs stay pinned while page content scrolls. On Overview, Campaign Goal KPI cards stay pinned with the tabs and wrap instead of scrolling sideways. **Event** (`?tab=events`) lists the campaign’s `internal_events` row linked via `ticketing_config.linkedCampaignId` (attach existing or **Create event** in place when none is linked; click the row to open the event workspace). Ticketed events pin KPI cards for sold / remaining / checked in (each with a ticket-type breakdown), revenue, and ticket donations. Orgs without Event Management see a subscribe message instead of the event list. Fundraising Plan has internal views **Strategy | Prospects** (`?tab=plan`, `?tab=plan&section=prospects`). KPI cards stay pinned above those views with the workspace tabs. Legacy `?tab=strategy` and `?tab=prospects` still open those views. Each campaign has one goal (`campaigns.goal_amount`); Goal Breakdown phases are retired (`scripts/270_disable_campaign_goal_phases.sql`). Prospects is a shared donation/sponsorship outreach pipeline (`ask_type` on `campaign_prospects`; people and Directory organizations can both be prospects; activity log; convert donations to pledges and sponsorships to `campaign_sponsorships`). **Sponsorship** has internal views **Sponsors | Packages**: committed sponsorships plus campaign-owned sponsorship packages and benefit fulfillment. Wishlist items are campaign priorities (`campaign_wishlist_items`); pledged/collected come from `pledges`/`payments.wishlist_item_id`. Public donate: `/donate/w/{token}`. SQL: `scripts/267_campaign_wishlist.sql`, `scripts/284_campaign_sponsorship_prospects.sql`, `scripts/285_campaign_sponsorship_packages.sql`, `scripts/292_campaign_same_organization_fks.sql`. Staff add/edit/collect pledges through one **Pledge Details** window (`components/donations/pledge-details-dialog.tsx`) on the global Pledges page, campaign workspace, prospects, and contact Financial pledges. Contact search can **Create** a missing person or organization without leaving the window.
...
* Publish: bazaar create/edit; Event workspace Settings → General → Community Calendar card
...
* Staff flyout: **Overview** (`/event-management`) · **Events** (`/event-management/events`) · Master Calendar · **Check-in** (`/event-management/check-in`) · Reports · Settings (**General** = optional on-site **Approval required**, default off; **Categories** for ticketed event groups; Notifications)
...
* Ticketing is folded into Event Management (no Ticketing sidebar). Ticket sales KPIs live on Overview; ticketed rows on Events show Issued / Remaining / Revenue / Category. Org-wide **Check-in** uses `TicketCheckInScanner`: phone camera QR (`TicketQrCamera`), computer type-a-code, plus active ticketed events. Org-wide lookup: `checkInOrgTicketByCode`.
* Overview ticket sales cards (active ticketed, issued, revenue) include inactive historical ticket types and page all completed orders (not the first 1,000 only)
* Reports **Orders** (`/event-management/reports/orders`) defaults the Events filter to **Active events** (All / Past / a specific event remain available)
* Events is one view/manage list (search, department, Active/Draft/Past, Ticketed, Category). Sorted newest date first. Status is **Draft**, **Published**, or **Completed** (date has passed). Default column order: Event, Department, Date, Time, Location, Space, Status, Actions. Ticketed rows can also show Category (`ticketing_event_categories`, SQL `287`), Issued, Remaining, and Revenue after Status. Event names are blue links. **Columns** chooses which table columns to show. Manage categories from Settings → Categories. Actions is **Copy** only; edit and delete stay on the event workspace. Opening a row goes to `/event-management/[id]`.
* Reports tabs: **Orders** · **Tickets** (day/event/customer analytics) · **Childcare**. Legacy `/event-management/ticketing/*` redirects.
...
* UI: overview dashboard (pinned colorful KPI row, Finance with revenue/expenses/net, Recent orders at bottom), **Settings → Tickets** (Paid/Free + Starts/Ends + ticket types), Orders tab (pinned KPIs + check-in; attendee table scrolls), finance, reports (attendee CSV), feature switches
...
* **Fundraising Dinner Sep 12 2026 Square donations:** 95 gifts ($32,191) on campaign **Annual Fundraiser - September 2026** with group attribution (`FUNDRAISER_DINNER_DONATIONS_SEP2026_V1`). Script: `node scripts/import-fundraiser-dinner-donations.mjs` / `--execute`. Export CSV from Campaign → Groups and Reports → Campaign Performance → Campaign Groups.
