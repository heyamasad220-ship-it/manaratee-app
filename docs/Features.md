...
**Program Registrations vs Reports (August 2026):** Program Workspace **Registrations** is the single operational list (who is registered). Clickable Enrolled / Waitlisted / Cancelled cards filter the table; **Reports** no longer duplicates that roster. Movable rows show the offering name plus a **Change** tag — the drop-down appears only when changing. Workspace Reports is **Overview | Trends | Attendance**: enrollment summary, enrollment by offering (capacity/available only when limited), monthly registration counts, and existing attendance. Offering enrolled/waitlisted/cancelled numbers open Registrations with those filters. Org-wide `/programs/reports/enrollments` and `/programs/reports?tab=waitlist` are unchanged. Key files: `department-students-panel.tsx`, `department-participants-panel.tsx`, `program-workspace-reports-panel.tsx`, `program-workspace-enrollment-reports.tsx`, `lib/programs/registration-table-columns.ts`, `lib/programs/program-enrollment-summary.ts`, `lib/programs/program-enrollment-summary-queries.ts`, `lib/programs/program-workspace-path.ts`.
...
**Programs → Reports tabs (July 2026 / August 2026):** **Reports** is a Programs Home destination (not a sidebar flyout). Opens `/programs/reports/enrollments`. Shared report tab bar (`ProgramsReportsNav`): **Enrollments** | **Add-ons** | **Payment Summary** | **Waitlist** | **Attendance**. On a **program workspace**, Add-ons lives under **Finance** (not Reports). **Childcare** moved to **Event Management → Reports** (`/event-management/reports/childcare`). Registration, Transactions, and Payroll moved to Programs Home cards (**Registrations**, **Finance**). Bare `/programs/reports` redirects to Enrollments. Legacy `?tab=enrollment` redirects to Registrations; `?tab=transactions` → Transactions; `?tab=childcare` → Event Management Childcare; `?tab=enrollments` → Enrollments; `?tab=tuition-plans` / `?tab=payment-summary` → Payment Summary; `?tab=addons` / `?tab=add-ons` → Add-ons. Leftover `/programs/reports/childcare` redirects to Event Management Reports. **`/programs`** is Programs Home (Overview), not a catalog redirect.
