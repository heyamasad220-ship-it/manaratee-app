...
**Contacts Groups record type (June 2026 / July 2026):** CRM `contact_type = group` remains for giving attribution but **is not a contact profile**. Detail UI is **`/donations/groups/[id]`** for Group Donation / Membership Group collectives. When the collective is a **Department** (linked via `linked_department_id`, or unique same-name match — auto-linked on open), Group Giving redirects to the shared **Department workspace** at **`/workforce/departments/[id]`**. Department tabs: **Overview** (years/seasons + flyer; Super Admin archive; legacy `?tab=offerings` → Overview), **Programs** (`?tab=programs` — year/season offerings list with Add Program; `?year=` prefill; legacy `/programs/[id]?tab=offerings` redirects here when the year has a department), **Enrollments** (UI label; URL `?tab=rosters` or alias `?tab=enrollments` — year/season + program filters, include cancelled/withdrawn, parent/guardian, Export CSV, registration links — merged from year/season Programs → Reports; legacy `/programs/[id]?tab=reports` redirects here), **Applications**, **Schedule**, **Financial** (sub-tabs: Payroll, Expenses, Financial Summary; legacy `?tab=payroll|expenses|budget` map here; **Employees** live under Financial → Payroll — merged columns, no email/phone; legacy `?tab=employees` → Payroll), **Group giving** when linked, **Activity** (events only), and **Archive** (archived years). Layout: tabs sit under the department name; each tab shows its own summary KPI cards (not a global strip). Financial assistance stays at org/committee level (not under departments). Legacy `?tab=payments` / `participants` → Enrollments; `babysitting` → Payroll. Apply **`scripts/167_giving_group_category.sql`**. Opening `/contacts/[id]` for a group redirects appropriately. Key files: `components/departments/department-group-workspace-client.tsx`, `components/departments/department-programs-panel.tsx`, `components/departments/department-student-payments-panel.tsx`, `components/departments/department-group-giving-panel.tsx`, `lib/departments/department-giving-link.ts`, `lib/departments/department-programs.ts`, `lib/donations/donation-group-path.ts`.

**Department Years/Seasons = Programs Catalog (July 2026):** Year/season setup for a department lives on **Overview** (list, add year, flyer, archive). The old standalone **Years/Seasons** tab (`?tab=offerings` / `department-offerings-panel.tsx` catalog UI) is removed from the workspace nav; legacy URLs open Overview. Sellable **Programs** (offerings) for open years live on the department **Programs** tab (`?tab=programs`). Programs **Catalog** (`/programs/catalog`) remains the org-wide year/season catalog. Staff UI: Year/Season = `programs` row; Program = `program_offerings` row (`lib/programs/program-display-labels.ts`).
...
**Department year programs / Overview + Archive (July 2026):** Department workspace default tab is **Overview** (year setup for that department): list open year programs, add next year (optional copy of courses + teachers, empty roster), upload/change year flyer; **Super Admin** can **Archive** (confirm name) → program `archived`, offerings closed, read-only. Operating tabs (**Programs**, **Enrollments**, **Applications**, **Schedule**, **Financial** including Payroll / Expenses / Financial Summary) and Overview KPIs include **open** years/seasons only (`draft`/`active`/`paused`) — after archive, those tabs reset to remaining open years (enrollments/applications by open `program_id`; payroll/expenses/budget periods by open year dates). **Employees** (under Financial → Payroll), Group giving, and Activity stay department-wide. **Archive** tab (`?tab=reports`) lists archived years only → closeout report (financial summary totals, roster-style students, teachers = all course instructors **plus** staff with approved/paid payroll overlapping year dates — not only one instructor per course; **Export CSV**). Key files: `department-overview-panel.tsx`, `department-programs-panel.tsx`, `department-student-payments.ts`, `department-participants.ts`, `department-active-programs.ts`, `department-payroll.ts`, `department-budget.ts`, `program-application-actions.ts`, `department-reports-panel.tsx`, `department-year-actions.ts`.

**QIL year import (July 2026):** Historical QIL 2025–26 roster + Stripe payment CSV → department / **one year program** `Quran Institute for Ladies 2025-2026` / **course offerings** / enrollments / `program_charges`. Requires **`scripts/174_enrollment_unique_per_offering.sql`** (unique active enrollment is per offering so students can take multiple courses). If courses were wrongly created as `QIL — {course}` programs, consolidate with `node scripts/migrate-qil-courses-to-offerings.mjs --execute` after running 174. Import: `node scripts/import-qil-year.mjs` / `--execute`. Display fix for imported rows: `node scripts/fix-qil-enrollment-display.mjs --execute` sets adult contact fields; `node scripts/fix-qil-registered-date.mjs --execute` sets `enrollment_date` to **2025-09-01** (Enrollments Registered). Contact profiles + **Programs** affiliation (`program_participant`): run **`scripts/175_split_customer_programs_affiliation.sql`**, then `node scripts/sync-qil-participant-contacts.mjs --execute`. Report: `scripts/reports/qilts/qil-import-YYYY-MM-DD.json`. Department **Enrollments** = names/courses/teachers only; payments under **Programs → Reports → Registrations**; **Financial Summary** uses billing totals.
...
**Finance / Payroll (July 2026):** Org payroll queue is **HR → Payroll** (`/workforce?tab=payroll`) — Finance removed from the main sidebar. Department heads still approve pay on department Financial → Payroll; approved lines appear as **Ready to pay**. Staff with `finance.manage` can **Mark paid** (status `paid` + `paid_at`). Childcare providers show a Childcare badge. Run **`scripts/187_finance_module_and_payroll_paid.sql`**. Event childcare hours: (1) Event Management → event → Childcare → **Log provider hours** (department from the event), or (2) Reports → Childcare Registrations → **Log hours** on a session card (standalone sessions require a department picker). Writes `department_staff_hour_logs.childcare_event_id`; run **`scripts/188_hour_logs_childcare_event.sql`**. Queue shows event names. Legacy `/finance` and `/finance/payroll` redirect to the HR Payroll tab. Key files: `lib/finance/org-payroll-queue.ts`, `lib/child-care/childcare-event-hours.ts`, `components/finance/finance-payroll-queue-panel.tsx`, `components/child-care/childcare-registrations-client.tsx`.
...
* **Program detail** (`/programs/[id]`) — Overview / Settings only. Sellable **Programs** (offerings) live on **HR → Departments → Programs** (`?tab=programs&year=…`); legacy `?tab=offerings` redirects there when the year has a department. Enrollments report lives on **HR → Departments → Enrollments** (`?tab=rosters&year=…`; alias `?tab=enrollments`); legacy `?tab=reports` redirects there when the year has a department. Inline **Edit** (name, subtitle, flyer, description, department, visibility, status) via Actions → Edit program or Overview Edit. Catalog ⋮ **Edit** opens this page. Key files: `program-detail-client.tsx`, `program-offerings-list-panel.tsx`, `program-detail-actions.ts`, `department-programs-panel.tsx`, `department-participants-panel.tsx`.
...
* **Service Needs** on **HR → Departments → [department] → Settings → Service Needs** (`?tab=settings&section=service-needs`). Department **Settings** also holds General / Registration / Notifications stubs (`department_program_settings`) and **Promo Codes** scoped to the whole department across years (`discount_codes.department_id`). Legacy `/programs/settings` and `/programs/settings/service-needs` redirect to `/workforce?tab=departments`. Key files: `components/departments/department-settings-panel.tsx`, `components/departments/department-promo-codes-settings-panel.tsx`, `components/programs/program-service-needs-settings-client.tsx`, `components/programs/edit/program-service-requirements-panel.tsx`. Apply SQL `scripts/190_department_settings_promo_codes.sql`.
...
* **Departments** is under **HR → Overview** (`/workforce?tab=departments`). Department names open `/workforce/departments/[id]`. Tabs: Overview (years/seasons), Programs, Enrollments (`?tab=rosters`), Applications, Schedule, **Financial** (Payroll / Expenses / Financial Summary), Reports (archived years). Employees live under Financial → Payroll (merged columns, no email/phone; name → contact; pencil → employment + **pay basis**). Legacy `/workforce/settings/departments`, `/hr/departments`, and `/workforce/departments` redirect to the Overview Departments tab. Programs sidebar starts at Catalog (not Departments). **Payments** is a tab on Programs → Reports (opens `/programs/registrations`).
...
* **HR Overview tabs (July 2026):** Sidebar HR is a direct link to `/workforce` (no flyout). Tabs: Overview, Departments, Employees, Volunteers, Childcare Providers, **Payroll**. Overview hosts people metrics; Payroll hosts the org Mark-paid queue (formerly Finance sidebar). Directory Applications use `?view=applications`; Positions use `?view=positions`. Key UI: `components/hr/hr-overview-client.tsx`, `components/finance/finance-payroll-queue-panel.tsx`. Legacy `/workforce/reports`, `/hr/reports`, `/finance`, `/finance/payroll` redirect into HR.
