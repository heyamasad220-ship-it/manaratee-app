...
* **Staff navigation unmerge (August 2026):** Top-level **Programs/ Events** is split. **Administration** is always on (after Directory): Departments, Employees, plus Volunteers and Child Care Providers when Programs or Event Management is enabled, plus Service Providers when Facilities is enabled. Routes stay under `/workforce/*`. **Programs** (when subscribed) flyout: **Academic** and/or **Seasonal** from `organizations.program_kinds` (`/programs/catalog?kind=`), then Financial Assistance and Reports. Unused program kinds are omitted. **Event Management** is its own rail item (Events, Master Calendar, Ticketing, Settings). Finance is not a rail item when Programs is on. Directory no longer lists Donors or Administration-owned roles (employees, volunteers, childcare providers, service providers). Key files: `lib/navigation/staff-module-nav.ts`, `components/layout/sidebar.tsx`, `lib/directory/directory-roles.ts`.
...
**Horizon Community Foundation demo seed (August 2026):** `node scripts/seed-horizon-community-foundation-demo.mjs --execute` loads fake Directory + Fund Development data for that org only (10 people, 1 org donor, 2 campaigns, 6 donors, pledges/payments, ask levels, prospects, wishlist). Names are mixed (American, Indian, Latino, African, Korean; one Arabic). Marker `HORIZON_DEMO_SEED_V1`. Reset: `--clean --execute`. Does not touch MAS Dallas. Staff header user menu shows **Admin** on this org instead of the signed-in email username.
...
**Canonical Pledge Details window (August 2026):** Staff add, edit, view, collect, remind, and delete pledges from one **Pledge Details** dialog (`components/donations/pledge-details-dialog.tsx`). **Add Pledge** and row click open the same window (title is always Pledge Details). One footer **Save** persists pledge fields plus any payment amount or payment-plan edits; **Cancel** closes without saving. Collection fields (Receive Payment, Payment Plan, Remind, Mark Contacted) and **Delete** stay hidden until a new pledge is saved, then remain in that window. Row **⋯** menus were removed from Pledges, campaign outstanding pledges, campaign pledges, and contact/donor pledge tables. Deep links (`?pledgeId=` / `?action=add`) still open this window on `/donations/campaigns/pledges`. Customer portal pledges are unchanged.
