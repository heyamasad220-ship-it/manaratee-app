...
* **Staff navigation unmerge (August 2026 / September 2026):** Top-level **Programs/ Events** is split. **Administration** is always on (after Directory): Departments, Employees, plus Volunteers and Child Care Providers when Programs or Event Management is enabled, plus Service Providers when Facilities is enabled. Routes stay under `/workforce/*`. **Programs** (when subscribed) has a flyout: **Overview**, **All programs**, **Registrations**, **Finance**, **Financial Assistance**, **Reports**, **Settings**. Overview (`/programs`) keeps Programs and Offerings KPI cards only. All programs is `/programs/list`. Offerings catalog `/programs/catalog` stays as a route (not in the flyout). Academic vs Seasonal is a **tag** on each program, not a nav split. **Event Management** is its own rail item (Overview, Events, Master Calendar, Check-in, Reports, Settings). Finance is not a rail item when Programs is on. Directory no longer lists Donors or Administration-owned roles (employees, volunteers, childcare providers, service providers). Key files: `lib/navigation/staff-module-nav.ts`, `lib/programs/programs-module-nav.ts`, `components/layout/sidebar.tsx`, `lib/directory/directory-roles.ts`.
* **Subscription catalog matches staff modules (August 2026):** Tenants subscribe to **Event Management**, **Programs**, **Vendor Hub**, **Venue Rentals**, **Fund Development**, and **Membership**. **Workforce**, **Finance**, **Facilities**, and **Community Calendar** are not sellable SKUs. Administration (`workforce`) is core for every org; Finance is included with Programs; Facilities (`spaces`) is included with Programs, Event Management, Venue Rentals, and Vendor Hub; Community Calendar is included with Vendor Hub or Event Management. Membership and Fund Development do not include Facilities or Community Calendar. SQL **`273_subscription_modules_match_staff_nav.sql`**. Keys: `lib/modules/module-catalog.ts`, `lib/modules/staff-module-labels.ts`.
...
**Fundraising Dinner Sep 12 2026 Square donations (September 2026):** `September12Donations.csv` (95 succeeded gifts, **$32,191**) imported onto campaign **Annual Fundraiser - September 2026**. Square donation reasons map to CRM giving groups; `Annual Fundraiser - September 12, 2026` is ungrouped campaign gifts ($5,700, including a $5,000 Syrian Forum USA sponsorship remark). Created **Tarbiya**, **Center**, and **CYP Usrat Al-Islah** giving groups (Tarbiya and Center linked to those departments). **CYP Usrat Al-Islah** is distinct from existing **CYP Usra**. Eight `campaign_groups` on the campaign. Payments write `campaign_id`, `campaign_group_id`, and `attributed_group_contact_id` so Campaign → Groups, Reports → Campaign Performance → Campaign Groups, and Reports → Donor Giving → Group Giving all show the breakdown. Fund **Annual Fundraiser - September 2026** under General Donation. Two MONTHLY rows ($130) stored as collected payments only (no recurring plans). Ticket checkout donations ($970) stay on ticket orders, not this ledger. Tag `FUNDRAISER_DINNER_DONATIONS_SEP2026_V1`. Script: `node scripts/import-fundraiser-dinner-donations.mjs` / `--execute`. Reports: `scripts/reports/fundraiser-dinner-donations-sep2026-dry-run.json`, `fundraiser-dinner-donations-sep2026-execute.json`. **Export CSV** on Campaign → Groups and Reports → Campaign Performance → Campaign Groups.
...
2. **Workforce routes:** Administration is the staff/core product (not a subscription SKU). Sidebar people lists live under Administration; module slug and paths remain `workforce` / `/workforce/*`. Align slug, routes, and folder names when safe (same class of rename as above — high blast radius).  
...
**Campaign workspace + phases (August 2026 — Phase A / September 2026):** Opening a campaign is a tabbed workspace (`?tab=`): **Overview**, **Event**, **Fundraising Plan**, **Pledges**, **Donations**, **Sponsorship**, **Groups**, **Wishlist**. Workspace tabs stay pinned below the staff header while the rest of the page scrolls. On **Overview**, the Campaign Goal KPI cards stay pinned with those tabs and wrap to extra rows instead of scrolling sideways. Fundraising Plan contains **Strategy | Prospects**. KPI cards (targeted gifts, target value, amount secured, gap) stay pinned with the workspace tabs; the Strategy/Prospects switch sits under the cards. Overview uses clear money language: **Committed** (valid pledges), **Collected** (payments), **Outstanding** (pledge balances). Each campaign has **one goal** (`campaigns.goal_amount`) — Goal Breakdown / campaign phases are no longer used. Existing phase rows can be cleared with `scripts/270_disable_campaign_goal_phases.sql` (table/columns kept unused). Pledges and Donations tabs filter the existing ledger by campaign (no duplicate financial records). Click a pledge row (or a Donations **Pledge Applied To** id) to open **Pledge Details** in place. Donation and pledge dates display as calendar dates only (no time). Donation **Status** is **Allocated** when the payment is applied to a pledge, **Unallocated** when the donor still has an open campaign pledge that this payment has not been applied to, and **—** for standalone gifts with no pledge to allocate against (refunded/voided statuses still win). Key files: `campaign-workspace-paths.ts`, `campaign-overview-tab.tsx`, `campaign-fundraising-plan-tab.tsx`. Migration `260_campaign_phases.sql` remains historical.
...
**Campaign strategy ask levels (August 2026 — Phase B):** Campaign → **Fundraising Plan → Ask Strategy** is a gift/ask chart (`campaign_ask_levels`, migration `261`). Donation ask amounts + target gift counts only (sponsorship packages stay under Sponsorship → Packages). Target Value = Ask × Target #. Table shows Prospects / Asked / Secured count / Amount Secured / Gap. Prospects and Asked counts (when > 0) open Fundraising Plan → Prospects filtered to donation prospects at that ask level; Asked uses stages Asked and Pledged (`CAMPAIGN_PROSPECT_ASKED_STAGES`). Secured uses valid linked/matching pledges, not prospect stage. Prospects may exceed target count. Nullable `pledges.ask_level_id` for later prospect→pledge conversion. Key files: `campaign-strategy-tab.tsx`, `campaign-ask-level-actions.ts`, `computeCampaignAskLevelMetrics`. Run `scripts/261_campaign_ask_levels.sql` after `260`.

**Campaign prospects (August 2026 — Phase C; unified August 2026):** Campaign → **Fundraising Plan → Prospects** (`campaign_prospects`). Shared outreach pipeline for **donation** and **sponsorship** asks. Ask type lives on the prospect record, not the Contact (`ask_type`: `donation` | `sponsorship`; existing rows default to donation). **Add Prospect** searches Directory contacts the same way Organizations does: people, organizations, and groups, matching name, email, phone, or primary contact name. Directory organization records (for example MSAADA) can be donation or sponsorship prospects. The create-contact link appears only after a search returns no match. Donation asks still use campaign ask levels; sponsorship asks pick an optional related event plus a campaign sponsorship package (or Custom / Undecided) and a suggested amount (package amount prefills, override allowed). Packages are configured on **Sponsorship → Packages**, not inline on the prospect form. Shared fields: assigned staff, stage, last contact, next follow-up, notes. Filter **Ask Type** (All / Donations / Sponsorships dropdown) plus an optional **Ask Level** filter (from the campaign’s configured donation ask levels). Table includes Ask Type badges and an Outcome column (pledge amount or sponsorship commitment). Follow-up shows **Today** / **Overdue**. **Log Activity** writes `campaign_prospect_activities` (email/phone/meeting/text/follow-up update `last_contacted_at`; notes do not). Stages stay Identified / Contacted / Pledged / Declined / No Response (`pledged` displays as **Committed** for sponsorships). Unique `(campaign_id, contact_id, ask_type)` so the same Contact can be a donation prospect and a sponsorship prospect in one campaign. Key files: `campaign-prospects-tab.tsx`, `campaign-prospect-actions.ts`, `campaign-fundraising-plan-tab.tsx`. Run `scripts/262_campaign_prospects.sql` then `scripts/284_campaign_sponsorship_prospects.sql`. Legacy `?tab=strategy` / `?tab=prospects` redirect to `?tab=plan` / `?tab=plan&section=prospects`.

**Prospect → pledge conversion (August 2026 — Phase D):** **Record Pledge** (donation prospects only) opens the canonical **Pledge Details** window and creates **one** `pledges` row with `campaign_id`, optional `ask_level_id`, and `campaign_prospect_id`. Prospect `suggested_ask_amount` is preserved. Stage → **Pledged** and `converted_pledge_id` is set. **View Pledge** opens the same window. Action: `convertCampaignProspectToPledgeAction`. UI: `components/donations/pledge-details-dialog.tsx`.

**Prospect → sponsorship conversion (August 2026):** Sponsorship prospects use **Create Sponsorship** (not Record Pledge). Creates one `campaign_sponsorships` row (contact, campaign, optional event/package, cash/in-kind/mixed, committed amount, status, payment status) and sets `converted_sponsorship_id`. Does **not** write a donation or pledge. Package benefits are copied onto `campaign_sponsorship_benefits` at conversion so later package edits do not change existing agreements. Committed sponsorships appear on campaign **Sponsorship**. Permissions reuse `donations.prospects.manage`. Key files: `campaign-sponsorship-actions.ts`, `campaign-sponsors-tab.tsx`, `campaign-sponsorship-dialog.tsx`. SQL: `scripts/284_campaign_sponsorship_prospects.sql`, `scripts/285_campaign_sponsorship_packages.sql`.
...
**Campaign overview insights (August 2026 — Phase G):** Campaign → Overview adds **Action Required** (overdue/upcoming follow-ups, unassigned, asked-without-pledge), **Team Summary** (by assignee), and **Campaign Groups** rollup. Deep-links into Fundraising Plan → Prospects filters (`followUp`, `assignee`, `stage`, `pledged`). Contact Financial panel shows **Fund Development** history (prospects, assignments, group gifts) gated by `donations.view`. Key files: `campaign-overview-insights.tsx`, `contact-fund-development-history.tsx`.
...
**Pledges summary cards (June 2026):** Pledges page stat cards match Donations Overview styling (colored left border, rounded icon badges). File: `app/(dashboard)/donations/campaigns/pledges/page.tsx`.

**Canonical Pledge Details window (August 2026):** Staff add, edit, view, collect, remind, and delete pledges from one **Pledge Details** dialog (`components/donations/pledge-details-dialog.tsx`). **Add Pledge** and row click open the same window (title is always Pledge Details). One footer **Save** persists pledge fields plus any payment amount or payment-plan edits; **Cancel** closes without saving. **Save** always closes the window after a successful add or payment (it does not stay open or reopen as details). If Contact search has no match, **Create “{name}”** opens Add Contact (person or organization; names with Foundation/Inc./LLC default to organization). Collection fields (Receive Payment, Payment Plan, Remind, Mark Contacted) and **Delete** appear only when you open an existing pledge. Row **⋯** menus were removed from Pledges, campaign outstanding pledges, campaign pledges, and contact/donor pledge tables. Deep links (`?pledgeId=` / `?action=add`) still open this window on `/donations/campaigns/pledges`. Customer portal pledges are unchanged.
...
| `/donations/campaigns/[id]` | Campaign workspace — Overview, Fundraising Plan (Strategy \| Prospects), Pledges, Donations, Sponsorship (Sponsors \| Packages), Groups, Wishlist; `?tab=` / `?section=` / `?group=` |
