...
**Vendor Network vendors table (August 2026):** `/vendor-hub/network/vendors` uses a vendor-focused layout (`vendorNetworkLayout` on `ContactsListView`): Contact (blue name link to CRM profile, email, phone underneath), Business Name (from approved vendor `applications.form_data.business_name` — contacts has no `company_name` column yet), Vendor Type (catalog type or latest payment/participant notes `category=`), Status, Last Activity. Removed Roles, Record Type, and Actions columns from this view only. **Pagination (August 2026):** server-side page of 20 vendors (role=`vendor` via `contact_roles!inner`) with `ListPagination`; search/status applied in the query so the full vendor list is not loaded up front.

**Vendor Network Participation History (August 2026):** `/vendor-hub/network/history` lists booth payments as Business Name (link to contact), Event, Event Date, Booth Type (payment notes `category=`), Amount. Sorted by event date. Key: `lib/vendor-hub/participation-history-queries.ts`, `components/vendor-hub/network/participation-history-client.tsx`.

**Vendor Hub Settings tabs (August 2026):** Payments and Public Page settings moved into the **General** tab as stacked cards. Remaining tabs: General, Booths, Applications, Notifications, Email Templates, Vendor Types. **Notifications** tab shows Vendor notification settings inline (staff/customer toggles); `/vendor-hub/settings/notifications` redirects to `?tab=notifications`. Vendor Types table: Name (click to sort A→Z / Z→A), Default fee, Actions — Status and Sort columns removed.
