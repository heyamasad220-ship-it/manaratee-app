...
**Campaign prospects (August 2026 — Phase C; simplified September 2026):** Campaign → **Prospects** (`?tab=prospects`) is a donation-only working list (`campaign_prospects.ask_type = donation`). **Add prospect** opens a popup to search the donor, type **Ask level**, search **Assigned to**, optionally set dates/notes, then **Save**. The table is read-only; click a row to open the same window to edit, remove, or **Record pledge** / **View pledge**. No stage or ask-type columns. Sponsorships are added on Campaign → **Sponsorship**, not this table. Unique `(campaign_id, contact_id, ask_type)` remains. Key files: `campaign-fundraising-plan-tab.tsx`, `campaign-fundraising-plan-table.tsx`, `campaign-fundraising-plan-dialog.tsx`, `campaign-prospect-actions.ts`. Legacy `?tab=plan` / `?tab=strategy` / `section=prospects` redirect to `?tab=prospects`.
...
| `/donations/campaigns/[id]` | Campaign workspace — Overview, Prospects, Pledges, Donations, Sponsorship (Sponsors \| Packages), Groups, Wishlist; `?tab=` / `?section=` / `?group=` |
