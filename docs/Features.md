...
* **Venue Rentals Dashboard vs Requests (July 2026):** `/bookings/overview` shows **confirmed upcoming** rentals only (KPI cards: upcoming / this week / next 30 days / balance due) plus **View all requests**. `/bookings/requests` loads the full queue (default status filter All) with KPI cards and search/status filters. **Queue conflict checks (July 2026):** list/dashboard conflict flags use one batched `resource_reservations` range query (`loadRentalConflictFlags`) instead of per-reservation round-trips — needed after bulk Google Form import (~150 rentals).
