...
* **Campaign workspace Event tab (September 2026):** Campaign detail **Event** (`?tab=events`) lists the same `internal_events` row linked by `ticketing_config.linkedCampaignId`. The tab shows the event name, date, and location; clicking the row opens `/event-management/[id]`. Pinned KPI cards for that event include tickets sold, remaining seats, checked in, ticket revenue, waitlist (when used), and **tickets sold by type**. Attach existing / Create event appear only when none is linked. No status tags or Check-in / Ticketing / Unlink buttons on the row. Ticketed events still appear on Event Management Overview and Events. If the org has Fund Development but not **Event Management**, the tab stays visible and shows a subscribe message (`ModuleNotSubscribed`) with Contact Manaratee (`support@manaratee.com`). Campaign managers (`donations.campaigns.manage`) may attach without full event manage. Keys: `campaign-events-tab.tsx`, `campaign-event-kpis.tsx`, `campaign-event-actions.ts`, `module-not-subscribed.tsx`.
