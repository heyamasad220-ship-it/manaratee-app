...
* **Ticketing Overview issued/revenue (August 2026):** Event Management → Ticketing → Overview counts **Issued** from all ticket types (including inactive historical offerings) and **Revenue** from every completed / partially refunded order (paged past the 1,000-row API limit). Closed imported events no longer show 0 / $0.00. Remaining is unlimited (`—`) when a type has no capacity cap. KPI cards show total events, active events, tickets issued, and revenue; the table lists active events (no Show filter). Key: `ticketing-overview-queries.ts`, `ticketing-overview-table.tsx`.
* **Ticketing Orders event filter (August 2026):** Event Management → Ticketing → Orders (`/event-management/ticketing/orders`) defaults the Events dropdown to **Active events**. Staff can still choose All events, Past events, or a specific event. `?event=` still deep-links a single event from the event workspace. Key: `ticketing-orders-client.tsx`.
* **Ticketing Events categories (August 2026):** Ticketing has an **Events** tab (`/event-management/ticketing/events`) with one continuous table matching Overview columns (Event with location/date, **Category** dropdown, Status, Issued, Remaining, Revenue) plus **Show** and **Category** filters. **Manage categories** adds/renames/hides/deletes. Overview is a sales snapshot with KPI cards (total / active events, tickets issued, revenue), an active-events table, and a **View all events** link to `/event-management/ticketing/events`. SQL **`287_ticketing_event_categories.sql`**. Keys: `ticketing-events-client.tsx`, `ticketing-event-sales-table.tsx`, `ticketing-event-category-actions.ts`.
* **Ticketing reports (July 2026):** Real ticket analytics under **Event Management → Ticketing → Reports** (`/event-management/ticketing/reports`) — always-on KPIs plus a **View** filter (**Days** / **Events** / **Customers**) and date range; CSV export matches the active view. Data from `ticket_orders` + `tickets` (no mock data). Event Management module **Reports** (`/event-management/reports`) is a separate sidebar item with a **Childcare** tab (`/event-management/reports/childcare`). Leftover `/programs/reports/childcare` redirects there. Key files: `lib/tickets/ticketing-reports-queries.ts`, `components/tickets/ticketing-reports-client.tsx`, `components/events/event-management-reports-nav.tsx`.
...
* **Event workspace Attendees — check-in & comms (August 2026):** **Scan / enter code** card checks in by ticket code (`checkInEventTicketByCode`) for staff with `events.checkin` or manage. **Check in** stays on the row; **Transfer** / **Resend** / **Refund** / **Mark paid** are manage-only inside the order sheet. Attendees tab prop fix: registration offerings no longer shadow the filter dropdown (`registrationOfferings` vs `ticketTypeFilterOptions`).
...
* **Event workspace Overview — sharing & net (August 2026):** Community Calendar card shows **Copy link**, **Download QR**, and **Open page** when visibility is Public (`internal-event-community-calendar-card.tsx`). Overview KPI cards sit on one colorful row (Status, Ticket revenue, Checked in, plus youth/staff/vendors/gifts when enabled). Registered and Event net are not KPI cards — registered stays on the Registration card; revenue / expenses / net stay on Finance. **Recent orders** is at the bottom. Department and event type show under the event name only (not repeated under Overview tabs).
...
* **Event workspace Settings tab (August 2026):** **Service needs** (volunteers / youth / vendors on `internal_events.requires_*` + `service_requirements`) + **Workspace modules** toggles + **Visibility & access** metadata (primary coordinator, audience, event tags, estimated attendance, internal notes) + registration checkout/questions/promos when registration requires offerings. Turning on Youth or Vendors in Service Needs also turns on those workspace tabs. Tasks + volunteer sign-ups remain on **Staff & Volunteers**. Overview shows coordinator name when set. Requires SQL **`252`**. Key: `internal-event-service-needs-settings.tsx`, `internal-event-features-settings.tsx`, `internal-event-meta-settings.tsx`, `updateEventWorkspaceMeta`, `internal-event-settings-workspace.tsx`.
...
* **Internal event delete guards (July 2026 / August 2026):** Delete lives on **Event Management → Events** (`/event-management/events`) and the event workspace (not Ticketing Overview). Server blocks delete when the event has **any** ticket orders/tickets (financial activity), **any** volunteer/vendor/childcare-provider sign-ups, or **any** childcare/youth registrations. Key files: `getInternalEventDeleteBlockers` in `lib/events/internal-event-actions.ts`, `components/events/internal-event-card-actions.tsx`.
* **Event workspace status (August 2026):** Workspace status is a compact dropdown: **Draft**, **Pending**, **Approved**, **Completed**, **Cancelled**. Pending Center requests (`submitted` / `awaiting_approval`) show as **Pending**; choosing **Approved** runs `approveInternalEventRequest` (space check + confirm). Event Management Overview **Attention required** opens this page. Edit pencil removed on workspace — click **Event details** to open the schedule/location editor. Key: `internal-event-status.ts`, `internal-event-status-select.tsx`, `internal-event-workspace.tsx`.
...
* **Event Management Overview vs Events (August 2026):** Sidebar first item is **Overview** (`/event-management`) — KPI cards + **Attention required** + **View all events**. Period filter (Today / This Week / This Month / All Events / Past Events) scopes KPIs only. **Events** (`/event-management/events`) is the view/manage catalog: search, Department, Status (Active / Draft / Past / All), Create event, row actions (edit / delete / copy). Row opens the event workspace. Key: `event-management/page.tsx`, `event-management/events/page.tsx`, `event-management-events-client.tsx`, `event-management-dashboard-panels.tsx`.
...
**Department Financial (August 2026):** **Financial** lives on the **department** workspace (not year/program). The **Reports** tab was removed — it repeated Financial. Leftover `?tab=reports` opens Financial. **Employees** is a top-level department tab (`?tab=employees`; staff roster; click a row → side-sheet **employee profile** in edit mode — position, job role, department, hire date, staff type/status, pay basis/rate, email/phone, recent pay periods & hour logs, link to contact profile; Remove blocked when department pay entries or hour logs exist). Leftover `?tab=financial&section=employees` opens Employees. **Schedule** moved to **Program Workspace** (`/programs/[id]?tab=schedule`): **Class times** (weekly `program_schedule_items` + sessions for that program; default **Week Board** of offering cards with instructor/space, plus **List** table; **Space** from `venue_id` / venue name) | **Activity planner** (`schedule_activities` Schedule Builder, locked to the open program; `?section=activity-planner`). Leftover department `?tab=schedule` opens the Programs doorway so staff pick a year. Schedule header CTAs: **Check space availability** → Facilities; **View Master Calendar** → Events. Operating **Financial** sub-tabs: **Payroll** (KPI cards: **Employees** + **Amount** only; Log hours / Create pay period actions; no pay-period list; Finance → Payroll queue for Mark paid) | Expenses | Financial Summary. Scoped to **open** years only (`draft`/`active`/`paused`). URLs: `/programs/[id]?tab=schedule`, `/programs/[id]?tab=schedule&section=activity-planner`, `?tab=employees`, `?tab=financial` (default Payroll), `?tab=financial&section=payroll|expenses|budget`. Legacy `?year=&tab=schedule` opens that program’s Schedule tab. Key files: `department-group-workspace-client.tsx`, `program-workspace-client.tsx`, `department-schedule-panel.tsx`, `department-payroll-panel.tsx` (`variant=roster|periods`), `department-employee-profile-sheet.tsx`, `finance-payroll-queue-panel.tsx`.
...
**Programs → Reports tabs (July 2026 / August 2026):** **Reports** is a Programs Home destination (not a sidebar flyout). Opens `/programs/reports/enrollments`. Shared report tab bar (`ProgramsReportsNav`): **Enrollments** | **Year comparison** | **Add-ons** | **Payment Summary** | **Waitlist** | **Attendance**. On a **program workspace**, Add-ons lives under **Finance** (not Reports). **Childcare** moved to **Event Management → Reports** (`/event-management/reports/childcare`). Registration, Transactions, and Payroll moved to Programs Home cards (**Registrations**, **Finance**). Bare `/programs/reports` redirects to Enrollments. Legacy `?tab=enrollment` redirects to Registrations; `?tab=transactions` → Transactions; `?tab=childcare` → Event Management Childcare; `?tab=enrollments` → Enrollments; `?tab=year-comparison` / `?tab=growth` → Year comparison; `?tab=tuition-plans` / `?tab=payment-summary` → Payment Summary; `?tab=addons` / `?tab=add-ons` → Add-ons. Leftover `/programs/reports/childcare` redirects to Event Management Reports. **`/programs`** is Programs Home (Overview), not a catalog redirect.
...
* **Service Needs** on **Event workspace → Settings** (`/event-management/[id]?tab=settings`). Volunteers / Youth / Vendors save on that event (`requires_volunteers` / `requires_childcare` / `requires_vendors` + `service_requirements`). Department **Settings** has one **Save** (name/color/description/terms) and **Delete department** at the bottom; delete is blocked when programs, offerings, or employees exist. **Program defaults**, **Registration**, **Notifications**, and **Promo Codes** live on Program Workspace **Settings** (`/programs/[id]?tab=settings&section=defaults|registration|notifications|promo-codes`). Registration/Notifications still save to `department_program_settings` (stubs). Promo codes remain department-scoped (`discount_codes.department_id`); sibling discounts lock to the open program. Leftover department `?section=year-defaults|registration|notifications|promo-codes` opens the Programs doorway. Leftover `?section=service-needs` opens department **Events**. Legacy `/programs/settings` redirects to `/workforce/departments`; `/programs/settings/service-needs` redirects to `/event-management`. Key files: `components/events/internal-event-service-needs-settings.tsx`, `components/programs/program-workspace-settings-panel.tsx`, `components/programs/program-policy-settings-panel.tsx`, `components/departments/department-settings-panel.tsx`, `components/departments/department-promo-codes-settings-panel.tsx`. Apply SQL `scripts/190_department_settings_promo_codes.sql`.
...
| `/donations/campaigns/[id]` | Campaign workspace — Overview, Fundraising Plan (Ask Strategy \| Prospects), Pledges, Donations, Sponsorship (Sponsors \| Packages), Groups, Wishlist; `?tab=` / `?section=` / `?group=` |
