...
**Campaign prospects (August 2026 — Phase C; unified August 2026):** Campaign → **Prospects** tab (`campaign_prospects`). Shared outreach pipeline for **donation** and **sponsorship** asks. Ask type lives on the prospect record, not the Contact (`ask_type`: `donation` | `sponsorship`; existing rows default to donation). **Add Prospect** searches Directory contacts (people, organizations, vendors). Donation asks still use campaign ask levels; sponsorship asks pick an optional related event + sponsorship package (or Custom / Undecided) and a suggested amount. Shared fields: assigned staff, stage, last contact, next follow-up, notes. Filter **All | Donations | Sponsorships**. Table includes Ask Type badges and an Outcome column (pledge amount or sponsorship commitment). Follow-up shows **Today** / **Overdue**. **Log Activity** writes `campaign_prospect_activities` (email/phone/meeting/text/follow-up update `last_contacted_at`; notes do not). Stages stay Identified / Contacted / Pledged / Declined / No Response (`pledged` displays as **Committed** for sponsorships). Unique `(campaign_id, contact_id, ask_type)` so the same Contact can be a donation prospect and a sponsorship prospect in one campaign. Key files: `campaign-prospects-tab.tsx`, `campaign-prospect-actions.ts`. Run `scripts/262_campaign_prospects.sql` then `scripts/284_campaign_sponsorship_prospects.sql`.
...
**Prospect → sponsorship conversion (August 2026):** Sponsorship prospects use **Create Sponsorship** (not Record Pledge). Creates one `campaign_sponsorships` row (contact, campaign, optional event/package, cash/in-kind/mixed, committed amount, status, payment status) and sets `converted_sponsorship_id`. Does **not** write a donation or pledge. Committed sponsorships appear on campaign **Sponsors**. Packages belong to events (`sponsorship_packages` + `sponsorship_package_benefits` for later fulfillment). Permissions reuse `donations.prospects.manage`. Key files: `campaign-sponsorship-actions.ts`, `campaign-sponsors-tab.tsx`, `campaign-sponsorship-dialog.tsx`. SQL: `scripts/284_campaign_sponsorship_prospects.sql`.
...
| `/donations/campaigns/[id]` | Campaign workspace — Overview, Strategy, Prospects, Pledges, Donations, Sponsors, Groups, Wishlist; `?tab=` / `?group=` |
