...
**Programs → Reports (July 2026):** Sidebar **Reports** is a single link to `/programs/reports` (Overview tab by default). Page tabs: Overview, **Registrations**, **Payment transactions**, **Attendance**, **Waitlist**. Attendance and Waitlist filter by offering; enable/disable remains on offering Overview (attendance/care packs) and Enrollment (waitlist). **Expenses** live in department workspace. **Financial Assistance** and **Payment Plans** under **Programs → Financial Assistance**.

**Programs → Reports → Payments list (July 2026):** `/programs/registrations` page title is **Payments** (interim). Column **Fee** (was Amount). **Status** temporarily used transaction outcomes — superseded by the registration pipeline design below.
...
**Program registration pipeline (July 2026):** Implementation started. Run **`scripts/182_program_registration_applications.sql`**. Customer **Apply** (`/customer/programs/[id]/apply`) with Returning vs New (returning auto-approved). Department workspace **Applications** tab for new-student evaluate (approve / not approve). Reports: **Registrations** + **Payment transactions**. Still pending: approve-other-offering UI, waitlist-on-full + offer deadline, gate Register on approval, FA-after-approval. Design: [`docs/programs-registration-pipeline-design.md`](./programs-registration-pipeline-design.md).

**Programs → Financial Assistance tabs (July 2026):** Moved enrollment FA awards and payment-plan installment reports from Reports onto `/programs/financial-assistance` (`?tab=financial-assistance` / `?tab=payment-plans`). Key files: `programs-fa-report-panels.tsx`, `module-applications-client.tsx` (`extraTabs`).

**Offering manage tabs (July 2026):** Overview / **Enrollment**. Overview shows **primary instructor** (and additional staff when present); Edit Overview to change instructor or add staff. Feature packs (attendance / care) on Overview. Waitlist toggle on Enrollment. Attendance & Waitlist views under **Programs → Reports**. Key files: `offering-manage-client.tsx`, `offering-overview-staff-fields.tsx`, `program-offering-paths.ts`.
...
**Department operating payments / Financial Summary (July 2026):** Replaces Google Sheets trackers for departments like Qur’an Institute for Ladies. **Tabs:** Rosters, Offerings (**same Programs Catalog UI**, filtered to the department), Schedule, Payroll, Expenses, Financial Summary (separate from Group giving). **Rosters:** enrollment list (student, course, teacher) without payment columns. **Offerings:** shared catalog of year/season programs; open a program to manage courses. **Schedule:** weekly class times + session/term list with links to edit. **Payroll:** teachers and childcare providers; log hours; create pay period for all; approve. **Expenses:** program expense rows for the department (moved from Programs → Reports). **Financial Summary:** custom start/end periods; revenue totals from Programs billing (no student payment detail UI); approved payroll expenses. SQL: `169`–`173`. Key files: `department-group-workspace-client.tsx`, `department-expenses-panel.tsx`, `department-offerings-panel.tsx`, `program-catalog-view.tsx`.
...
* **Offering manage** (`/programs/[id]/offerings/[offeringId]`) — Overview / Enrollment. Overview includes **Delivery**, feature packs, and **primary instructor** (edit to change instructor / add staff). Waitlist enable on Enrollment. Attendance & Waitlist: `/programs/reports?tab=attendance|waitlist`.
...
* **Departments** is under **Programs** in the sidebar (first item: Departments, then Catalog, Schedule, Financial Assistance, Reports, Settings). Department names open `/workforce/departments/[id]`. Tabs: Overview (year programs), Employees, Rosters, Offerings, Schedule, Payroll, Expenses, Financial Summary, Reports (archived years). Employees: name → contact; pencil → employment + **pay basis** (hourly rate or monthly salary). Legacy `/workforce/settings/departments` redirects to the list. **Payments** is a tab on Programs → Reports (opens `/programs/registrations`).
