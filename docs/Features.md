...
**Prospect → pledge conversion (August 2026 — Phase D):** **Record Pledge** in the Assign Donor dialog opens the canonical **Pledge Details** window and creates **one** `pledges` row (amount, date, frequency, category, fund, optional wishlist item) with `campaign_id`, optional `ask_level_id`, and `campaign_prospect_id`. After save, staff stay in Pledge Details (collection actions appear) and the Prospects list refreshes. Prospect `suggested_ask_amount` is preserved; actual amount is entered separately. Prospect stage → **Pledged** and `converted_pledge_id` is set. **View Pledge** opens the same window. Action: `convertCampaignProspectToPledgeAction`. UI: `components/donations/pledge-details-dialog.tsx`.
