...
* **Event Management Ticketing merge (September 2026):** Ticketing is no longer a separate Event Management sidebar item. **Overview** (`/event-management`) keeps operations KPIs and adds ticket sales cards (active ticketed, issued, revenue). **Events** (`/event-management/events`) is one list for every event, with Issued / Remaining / Revenue / Category on ticketed rows plus Ticketed and Category filters. **Check-in** is a sidebar item (`/event-management/check-in`) — org-wide door desk (phone camera QR, computer type-a-code). **Reports** tabs: **Orders** (`/event-management/reports/orders`) · **Tickets** (`/event-management/reports/tickets`) · **Childcare**. Unfinished org Ticketing Settings (default checkout / mock promo users) were not moved. Legacy `/event-management/ticketing/*` and `/tickets/*` redirect. Keys: `staff-module-nav.ts`, `event-management-dashboard-panels.tsx`, `event-management-events-client.tsx`, `event-management-reports-path.ts`.
* **Ticketing Overview issued/revenue (August 2026):** Event Management Overview counts **Issued** from all ticket types (including inactive historical offerings) and **Revenue** from every completed / partially refunded order (paged past the 1,000-row API limit). Closed imported events no longer show 0 / $0.00. Remaining is unlimited (`—`) when a type has no capacity cap. Key: `ticketing-overview-queries.ts`.
* **Ticketing Orders event filter (August 2026):** Event Management → Reports → **Orders** (`/event-management/reports/orders`) defaults the Events dropdown to **Active events**. Staff can still choose All events, Past events, or a specific event. `?event=` still deep-links a single event from the event workspace. Key: `ticketing-orders-client.tsx`.
...
* **Ticketing Events categories (August 2026 / September 2026):** Event Management → **Events** default columns are Event, Department, Date, Time, Location, Space, Status, Actions. Ticket sales columns (Issued, Remaining, Revenue) and a **Category** dropdown sit after Status when shown (`ticketing_event_categories`, SQL **`287`**). Add, rename, hide, or delete categories under **Event Management → Settings → Categories**. Event names are primary (blue) links. Staff can pick visible table columns from **Columns**. Keys: `event-management-events-client.tsx`, `event-management-events-columns.ts`, `ticketing-event-category-actions.ts`.
* **Ticketing reports (July 2026):** Real ticket analytics under **Event Management → Reports → Tickets** (`/event-management/reports/tickets`) — always-on KPIs plus a **View** filter (**Days** / **Events** / **Customers**) and date range; CSV export matches the active view. Data from `ticket_orders` + `tickets` (no mock data). Reports also has **Orders** and **Childcare** (`/event-management/reports/childcare`). Leftover `/programs/reports/childcare` redirects there. Key files: `lib/tickets/ticketing-reports-queries.ts`, `components/tickets/ticketing-reports-client.tsx`, `components/events/event-management-reports-nav.tsx`.
* **Event workspace Tickets (September 2026):** Event **Settings → Tickets** (formerly a top-level Tickets tab) uses a compact **Paid / Free** attendance picker. Mixed paid+free and open-public options were removed (use promo codes for comps). **Starts** / **Ends** sit beside attendance (date picker + 30-minute time list). Ticket types table is below. **Orders** stays a workspace tab. Legacy `?tab=tickets` / `?tab=ticketing` / `?tab=registration` open Settings → Tickets. Keys: `internal-event-registration-workspace.tsx`, `event-workspace-features.ts`, `date-picker-input.tsx`.
...
* **Event workspace Attendees tab (August 2026 / September 2026):** Event → **Orders** (formerly Attendees) lists seats with counters (Registered / Checked in / Not checked in / Canceled / Waitlisted), filters, check-in / undo (`setEventTicketCheckIn`), **Promote** for waitlisted tickets when waitlist is enabled (`promoteWaitlistedTicket`), **Add attendee** manual registration (`addManualEventAttendee`), **Mark paid** for pending orders (`completePendingEventTicketOrder`), **Refund** (full remaining or partial) for completed / partially refunded paid orders (`refundEventTicketOrder`), and **View orders**. Tabs, order KPI cards, and Check in stay pinned; only the attendee table scrolls. Clicking a row opens an order sheet with contact details, order/ticket info, and **Refund** / **Transfer** / **Resend** / **Mark paid**. **Check in** stays on the row. The Order column is hidden. Auto-waitlist when capacity is full on `createTicketOrder` if waitlist toggle is on. Requires SQL **`253_event_youth_checkin_waitlist.sql`** for `waitlisted` ticket status. **Attendee vs contact (August 2026):** Attendee column is the ticket holder (child name + code). Contact is the Directory contact (name + email + phone); the name links to `/directory/[id]`. Import/backfill SQL **`286`** clears purchaser email copied onto `tickets.attendee_email`. Key: `internal-event-attendees-tab.tsx`, `internal-event-attendee-order-sheet.tsx`, `internal-event-add-attendee-dialog.tsx`, `ticket-order-queries.ts`, `ticket-order-actions.ts`.
...
* **Event workspace Attendees — check-in & comms (August 2026 / September 2026):** Shared **Check in** (`TicketCheckInScanner`) on event Orders and Event Management → Check-in. **Phone only:** **Start camera** reads the ticket QR (`TicketQrCamera`, `jsQR` / `BarcodeDetector`). **Computer:** type the ticket code (no camera preview). Check-in by ticket code (`checkInEventTicketByCode` per event, `checkInOrgTicketByCode` org-wide) for staff with `events.checkin` or manage. **Check in** stays on the row; **Transfer** / **Resend** / **Refund** / **Mark paid** are manage-only inside the order sheet. Orders tab prop fix: ticket types no longer shadow the filter dropdown (`registrationOfferings` vs `ticketTypeFilterOptions`).
...
* **Campaign workspace Event tab (September 2026):** Campaign detail **Event** (`?tab=events`) lists the same `internal_events` row linked by `ticketing_config.linkedCampaignId`. The tab shows the event name, date, and location; clicking the row opens `/event-management/[id]`. Pinned KPI cards for that event include tickets sold, remaining, and checked in **with a per-type breakdown on each card**, plus ticket revenue, **ticket donations** (checkout gifts), and waitlist (when used). Attach existing / Create event appear only when none is linked. No status tags or Check-in / Ticketing / Unlink buttons on the row. Ticketed events still appear on Event Management Overview and Events. If the org has Fund Development but not **Event Management**, the tab stays visible and shows a subscribe message (`ModuleNotSubscribed`) with Contact Manaratee (`support@manaratee.com`). Campaign managers (`donations.campaigns.manage`) may attach without full event manage. Keys: `campaign-events-tab.tsx`, `campaign-event-kpis.tsx`, `campaign-event-actions.ts`, `module-not-subscribed.tsx`.
* **Ticketing Check-in (September 2026):** Event Management → **Check-in** (`/event-management/check-in`) is the org-wide door desk for every ticketed event, plus shortcuts into each event’s Orders tab. Phone: camera QR scan (`TicketQrCamera`). Computer: type-a-code only. Org-wide lookup: `checkInOrgTicketByCode`. Same check-in on the event Orders tab. Legacy `/event-management/ticketing/check-in` redirects here.
...
* **Internal event delete guards (July 2026 / August 2026 / September 2026):** Delete lives on the event workspace (not the Events list). The Events list Actions column is **Copy** only. Server blocks delete when the event has **any** ticket orders/tickets (financial activity), **any** volunteer/vendor/childcare-provider sign-ups, or **any** childcare/youth registrations. Key files: `getInternalEventDeleteBlockers` in `lib/events/internal-event-actions.ts`, `components/events/internal-event-card-actions.tsx`.
* **In-place create event + optional on-site approval (September 2026):** Create event stays on the page it was opened from (campaign Event, department Events, Event Management Overview/Events, Master Calendar). **Online** and **External Venue** skip Facilities and never wait for approval. **On-site (Center)** shows a **View facility calendar** link (opens in a new tab) so staff can check space and come back to finish — no room picker in the create window. Org setting **Approval required** lives on Event Management → Settings → General (`event_management_settings.approval_required`, default **off**, SQL **`291`**). When on, only on-site events go to Pending. Facilities Calendar can still create with a room picker because staff are already looking at spaces. Legacy `/event-management/create` and `/request` redirect to `/event-management/events?create=1`. Key: `create-internal-event-button.tsx`, `facility-event-request-drawer.tsx`, `event-management-settings.ts`.
...
* **Event Management Overview vs Events (August 2026 / September 2026):** Sidebar first item is **Overview** (`/event-management`) — KPI cards + **Attention required** + **View all events**. Period filter (Today / This Week / This Month / All Events / Past Events) scopes KPIs only. **Events** (`/event-management/events`) is the view/manage catalog: search, Department, Status (Active / Draft / Past / All), Create event. Columns: Event, Department, Date, Time, Location, Space, Status (**Draft** / **Published** / **Completed** when the date has passed), then Actions (**Copy** only). Rows sort newest date first. Row opens the event workspace. Key: `event-management/page.tsx`, `event-management/events/page.tsx`, `event-management-events-client.tsx`, `event-management-dashboard-panels.tsx`.
...
* **Unified Facilities event request drawer (July 2026 / September 2026):** The same booking drawer is used for create/edit. **Create from Event Management / campaign / department / Master Calendar** stays on that page (`CreateInternalEventButton`, `spaceMode=calendar-link`) — on-site has a **View facility calendar** link, not a room picker. **Facilities → Calendar** still opens the drawer with a room picker (`spaceMode=select`). **Online** and **External Venue** confirm immediately (no Facilities, no approval). **On-site** waits only when Event Management → Settings → **Approval required** is on (default off). Prefills department + requester name. Supports daily/weekly/monthly recurrence + exceptions. Legacy `/event-management/create` and `/request` redirect to Event Management Events with `?create=1`. **Edit** from event cards / workspace (`/event-management/[id]/edit`) opens the **same drawer**. Drawer is **booking-only** (date/time/location/setup/recurrence) — registration, staff, youth, and vendors are configured on the **event workspace** tabs. Staff Tools keeps `/customer/staff/events/request` (same drawer, calendar-link). Key files: `facility-event-request-drawer.tsx`, `create-internal-event-button.tsx`, `facility-event-edit-page-client.tsx`, `reservation-calendar.tsx`.
...
**Vendor Hub Reports (August 2026):** `/vendor-hub/reports` no longer uses hardcoded sample vendors/revenue. Tabs are **Vendor Sales**, **Booth Performance**, and **Participation History** (optional `?eventId=` on sales/booth; `?tab=history` and optional `?contact=`). Former Reports Overview KPIs/tables live on Vendor Hub **Overview**. Empty states when there are no assignments/payments. Key files: `lib/vendor-hub/vendor-hub-reports-queries.ts`, `components/vendor-hub/vendor-hub-reports-client.tsx`, `vendor-hub-reports-overview-panels.tsx`.
...
**Fund Development IA redesign (August 2026):** Sidebar is **Overview / Campaigns / Pledges / Donations / Reports / Settings**. Operational work (transactions, recurring plans, import/match, receipts & statements) lives under **Donations** (`/donations/payments/*`). **Reports** is an analytics landing page with Giving Summary, Donor Giving, Campaign Performance, Pledge Performance, and Recurring Giving. Date range and CSV export on Transactions / Giving Summary filter KPIs, charts, and the payment table together. Transaction columns are date, donor, amount, method, campaign, fund, receipt status, and status; method/campaign/fund filters use stored values. CSV export still includes type and group. Receipts has a Missing queue (`?status=missing`); year-end KPIs come from `donation_receipts` where `receipt_type = annual_statement`. No schema change — same `payments` / `pledges` / `recurring_donation_plans` / `donation_receipts` records. Legacy report URLs redirect. Key files: `donations-sidebar-children.ts`, `donation-ops-chrome.tsx`, `donation-reports-landing.tsx`, `donation-payments-panel.tsx`.
...
* `/donations/campaigns/pledges` — server-paginated table; filters: status (default **Open** = any outstanding balance), campaign, minimum pledged amount; statuses are **Open** and **Fulfilled** only (a partly paid pledge stays Open). Pledge summary cards reflect the same filters; KPI cards and table headers stay pinned while rows scroll; donor name opens contact profile in a modal (`ContactProfileDialog`). Legacy `/donations/pledges` redirects here. Pledge Performance analytics is `/donations/reports/pledges`.
...
* **Donations** (operations) — in-page tabs:
  * **Transactions** — `/donations/payments/transactions` (payment register + Receive Payment; date range, CSV export, campaign/fund/receipt columns)
...
* **Reports** (analytics) — landing cards at `/donations/reports` (optional `?range=` applies to Giving Summary):
  * **Giving Summary** — `/donations/reports/giving` (filtered KPIs stay pinned; Giving over time is a line chart — daily for 7/30/90 day ranges, monthly for last year / all time; Giving by method removed; read-only payment register)
  * **Donor Giving** — `/donations/reports/donors` (Individual / Household / Group). Report tabs and KPI cards stay pinned while the table scrolls. Open Group Giving from Reports. Group/Household table sections show the view title on the left (**Add Group** beside Group Giving) and the period dropdown on the right. Group name and Last Gift columns sort. Group table columns: Group, Primary Contact, Combined Total, Gifts, Last Gift. Household Giving lists only families with two or more adult donors. Group workspace is `/donations/groups/[id]`. Pledge and Outstanding Balance columns are omitted — pledge detail stays on **Pledges**.
...
| `/donations/campaigns/pledges` | Pledge list (filters: campaign, status default Open, min amount), add/edit/pay, last reminder/contacted columns, inline reminder actions, detail dialog; KPI cards and table headers stay pinned |
