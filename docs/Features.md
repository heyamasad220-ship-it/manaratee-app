...
* **Staff navigation unmerge (August 2026):** Top-level **Programs/ Events** is split. **Administration** is always on (after Directory): Departments, Employees, plus Volunteers and Child Care Providers when Programs or Event Management is enabled, plus Service Providers when Facilities is enabled. Routes stay under `/workforce/*`. **Programs** (when subscribed) is a **single rail item** — no Academic/Seasonal flyout. Clicking it opens Programs Home (`/programs`), titled **Overview**, with breadcrumb `Dashboard > Programs > Overview`. Colored whole-card links replace the old in-page module tabs (Programs, Offerings, Registrations, Finance, Financial Assistance, Reports). Clicking **Programs** in the breadcrumb from a nested page returns to Overview. Academic vs Seasonal is a **tag** on each program, not a nav split. **Event Management** is its own rail item (Overview, Events, Master Calendar, Check-in, Reports, Settings). Finance is not a rail item when Programs is on. Directory no longer lists Donors or Administration-owned roles (employees, volunteers, childcare providers, service providers). Key files: `lib/navigation/staff-module-nav.ts`, `lib/programs/programs-module-nav.ts`, `components/layout/sidebar.tsx`, `lib/directory/directory-roles.ts`.
...
* **Campaign workspace Event tab (September 2026):** Campaign detail **Event** (`?tab=events`) lists the same `internal_events` row linked by `ticketing_config.linkedCampaignId`. The tab shows the event name, date, and location; clicking the row opens `/event-management/[id]`. Pinned KPI cards for that event include tickets sold, remaining seats, checked in, ticket revenue, **ticket donations** (checkout gifts), waitlist (when used), and **tickets sold by type**. Attach existing / Create event appear only when none is linked. No status tags or Check-in / Ticketing / Unlink buttons on the row. Ticketed events still appear on Event Management Overview and Events. If the org has Fund Development but not **Event Management**, the tab stays visible and shows a subscribe message (`ModuleNotSubscribed`) with Contact Manaratee (`support@manaratee.com`). Campaign managers (`donations.campaigns.manage`) may attach without full event manage. Keys: `campaign-events-tab.tsx`, `campaign-event-kpis.tsx`, `campaign-event-actions.ts`, `module-not-subscribed.tsx`.
...
**Canonical Pledge Details window (August 2026):** Staff add, edit, view, collect, remind, and delete pledges from one **Pledge Details** dialog (`components/donations/pledge-details-dialog.tsx`). **Add Pledge** and row click open the same window (title is always Pledge Details). One footer **Save** persists pledge fields plus any payment amount or payment-plan edits; **Cancel** closes without saving. **Save** always closes the window after a successful add or payment (it does not stay open or reopen as details). Collection fields (Receive Payment, Payment Plan, Remind, Mark Contacted) and **Delete** appear only when you open an existing pledge. Row **⋯** menus were removed from Pledges, campaign outstanding pledges, campaign pledges, and contact/donor pledge tables. Deep links (`?pledgeId=` / `?action=add`) still open this window on `/donations/campaigns/pledges`. Customer portal pledges are unchanged.
