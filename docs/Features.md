...
* **Department workspace Events (July 2026):** HR → Departments → **Events** (`?tab=activity`) lists that department’s `internal_events` with open/edit/delete (manage permission). **Calendar** opens Event Management’s events calendar filtered by department; **Create event** opens the create form with department locked. Panel: `components/departments/department-events-panel.tsx`.
* **Event Management events calendar (July 2026):** `/event-management/calendar` shows all `internal_events` for the month (facility, online, external) with department filter — not the Facilities room grid. Sidebar **Calendar** points here; **Facility schedule** deep-links Facilities `?sources=internal_event` for space conflicts. Key files: `internal-events-calendar-client.tsx`, `internal-event-calendar-queries.ts`.
