...
* **Department workspace Events (July 2026):** Programs/ Events → **Departments** → **Events** (`?tab=activity`) lists that department’s `internal_events` with open/edit/delete (manage permission). **Calendar** opens **Departments → Master Calendar** filtered by department; **Create event** opens the create form with department locked. Panel: `components/departments/department-events-panel.tsx`.
* **Event Management events calendar (July 2026 / August 2026):** Master Calendar lives under **Programs/ Events → Departments → Master Calendar** (`/workforce/departments/calendar`) — all `internal_events` for the month (facility, online, external) with department filter; not the Facilities room grid. Legacy `/event-management/calendar` redirects here. **Facility schedule** deep-links Facilities `?sources=internal_event` for space conflicts. Key files: `internal-events-calendar-client.tsx`, `internal-event-calendar-queries.ts`, `departments-section-nav.tsx`.
* **Event location types (July 2026):** Create/Edit event form: **Center** (venue(s) + Facility setup + calendar block), **Online**, or **External Venue** (name + address). Facility setup only for Center. Migration **`210_internal_event_location_type.sql`** (`location_type`, `location_address`; sync skips calendar when `venue_id` is null). Legacy rows stay unset until edited.
...
* **Unified Facilities event request drawer (July 2026):** One side drawer on **Facilities → Calendar** for Center / Online / External Venue (always **submit for approval**, including event managers). Prefills department + requester name. Supports daily/weekly/monthly recurrence + exceptions (materializes one `internal_events` row per occurrence with shared `recurrence_config.seriesId`). **EM Master Calendar is read-only** with CTA **Create event** → Facilities. Department workspace **Create event** / **View Master Calendar** follow the same model. Legacy `/event-management/create` and `/request` redirect to Facilities. **Edit** from event cards / workspace (`/event-management/[id]/edit`) opens the **same drawer** (not the old full-page form). Drawer is **booking-only** (date/time/location/venue/setup/recurrence) — ticketing, volunteers, childcare, and vendors are configured on the **event workspace** tabs. **Flyer** upload lives on workspace Overview (`internal_events.flyer_url`, migration **`214_internal_event_flyer_url.sql`**, reuses `program-flyers` bucket). Staff Tools keeps `/customer/staff/events/request` (same drawer). Venue rental booking stays separate. Removed unused mock `NewBookingRequestDrawer` / `AddEventForm`. Key files: `facility-event-request-drawer.tsx`, `facility-event-edit-page-client.tsx`, `internal-event-module-setup-panel.tsx`, `internal-event-flyer-card.tsx`, `event-recurrence.ts`, `reservation-calendar.tsx`.
...
**Department Financial vs Reports (August 2026):** Operating **Financial** (Payroll / Expenses / Financial Summary) is scoped to **open** years only (`draft`/`active`/`paused`). Closed-year pay periods no longer appear there. New **Reports** tab (`?tab=reports`) filters by year/season (defaults to a closed year when present) with Payroll / Expenses / Financial Summary (read-only). Key files: `department-active-programs.ts` (`loadDepartmentOpenPrograms` vs `loadDepartmentWorkspacePrograms`), `department-payroll.ts`, `department-expenses-panel.tsx`, `department-reports-panel.tsx`, `department-group-workspace-client.tsx`.
...
**Programs → Reports → Enrollments (August 2026):** One row per participant (not family-grouped). Columns: **Contact**, **Participant**, **Date of birth**, **Age**, **Gender**, **Allergies**, **Emergency contact**, **Photo consent**, **Status** (Active/Cancelled). Filters match Registrations (Department | Program | Offering | Status; default Active offerings). DOB/gender/age from `people` via `child_person_id`; allergies and photo consent from enrollment notes (Allergies hides None / N/A / “Any allergies:” placeholders and keeps real values only); emergency contact from notes when present, otherwise registrant/parent name + phone. Key files: `app/(dashboard)/programs/reports/enrollments/page.tsx`, `components/programs/enrollments-report-table.tsx`, note parsers in `lib/programs/registration-report-helpers.ts`.
...
**Offering manage → overview + edit dialog (August 2026):** Offering route is an **overview** (single header breadcrumb trail via Header `breadcrumbExtras` (e.g. Dashboard › Programs/ Events › Departments › department name › offering name; last segment not clickable), clickable name + status, year · department subtitle, 3 StatCards: Primary instructor / Schedule / Enrollment, enrolled student names). Edit opens in `OfferingEditDialog` using the same basics form as Add (`OfferingBasicsForm`: **Type** Academic Year / Seasonal on top, then Name, Delivery, Status, dates, instructor, gender/ages, capacity) plus foldable **Advanced Settings**. Changing Type on edit updates `programs.program_kind`. **Questions:** Add question opens a dialog (prompt, answer type text box / text area / drop-down; drop-down values comma-separated). Run **`scripts/239_registration_question_select_options.sql`** for `select` type + `options`. **Billing Schedule** (monthly pricing): summary of billing day (from program start date), first/last billing date, duration, plus month checkboxes (4 per row; uncheck to skip e.g. Ramadan). Late enrollments bill from join month through last billing date only; skipped months are never charged. Run **`scripts/238_offering_billing_calendar_summary.sql`**. Dialog Save persists basics plus any opened Advanced handlers (staff, schedule, registration, pricing) and stays on overview (`router.refresh()`). Deep link `?edit=1` auto-opens the dialog. Preview / Share / Delete live on the Offerings list ⋯ menu; Delete also in the edit dialog footer. Key files: `offering-manage-client.tsx`, `offering-edit-dialog.tsx`, `offering-registration-questions-editor.tsx`, `program-billing-schedule-view.tsx`, `program-billing-utils.ts`, `offering-simple-schedule-form.tsx`, `program-sessions-editor.tsx`.

**Programs list row actions (August 2026):** Department **Offerings** table (`ProgramOfferingsListPanel`) — row click opens overview; **name** opens overview with `?edit=1` (edit dialog). Row **⋯** menu: Preview page, Share link, Delete (blocked when enrolled). Columns: Offering, Delivery, **Tuition**, **Primary Instructor**, **Days**, **Times**, Enrollment. Key files: `program-offerings-list-panel.tsx`, `department-programs-panel.tsx`, `department-programs.ts`.
