...
**Programs → Reports (July 2026):** Sidebar **Reports** is a single link to `/programs/reports` (Overview tab by default). Page tabs: Overview, **Payments** (opens `/programs/registrations` — to be renamed **Registrations**). **Expenses** moved to department workspace (next to Payroll). **Waitlist**, **Before & After Care**, and **Attendance** moved to offering manage. **Financial Assistance** and **Payment Plans** live under **Programs → Financial Assistance**.
...
**Program registration pipeline (design, July 2026):** Apply (customer or staff) with Returning vs New → returning auto-approved; new students evaluated by department head (approve / not approve / approve other offering) → after approval: optional FA, auto waitlist if full, seat open → auto offer + deadline → register (portal or staff) creates fee. Reports will show **Registrations** + **Payment transactions**. Existing enrollments treated as already registered. Full write-up: [`docs/programs-registration-pipeline-design.md`](./programs-registration-pipeline-design.md). **Not implemented yet.** Depends on offering-attributes migration for capacity/registration mode SSOT.
...
**Offering manage ops tabs (July 2026):** Offering workspace adds **Waitlist**, **Before & After Care**, and **Attendance** after **Staff** (temporary panels; offering-scoped where possible). Key file: `offering-operations-report-panels.tsx`, `offering-manage-client.tsx`.
...
**Department operating payments / Financial Summary (July 2026):** Replaces Google Sheets trackers for departments like Qur’an Institute for Ladies. **Tabs:** Rosters, Offerings, Schedule, Payroll, Expenses, Financial Summary (separate from Group giving). **Rosters:** enrollment list (student, course, teacher) without payment columns. **Offerings:** create/manage courses for the department’s programs (year/season runs); Catalog under Programs reads the same offerings. **Schedule:** weekly class times + session/term list with links to edit. **Payroll:** teachers and childcare providers; log hours; create pay period for all; approve. **Expenses:** program expense rows for the department (moved from Programs → Reports). **Financial Summary:** custom start/end periods; revenue totals from Programs billing (no student payment detail UI); approved payroll expenses. SQL: `169`–`173`. Key files: `department-group-workspace-client.tsx`, `department-expenses-panel.tsx`, `department-offerings*.tsx/ts`, `department-schedule*.tsx/ts`, `department-participants*.tsx/ts`, `department-payroll*.tsx/ts`, `department-budget*.tsx/ts`.
