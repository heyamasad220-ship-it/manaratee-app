...
**Department Financial (August 2026):** **Financial** lives on the **department** workspace (not year/program). The **Reports** tab was removed — it repeated Financial. Leftover `?tab=reports` opens Financial. **Employees** is a top-level department tab (`?tab=employees`; staff roster; click a row → side-sheet **employee profile** in edit mode — position, job role, department, hire date, staff type/status, pay basis/rate, email/phone, recent pay periods & hour logs, link to contact profile; Remove blocked when department pay entries or hour logs exist). Leftover `?tab=financial&section=employees` opens Employees. **Schedule** moved to **Program Workspace** (`/programs/[id]?tab=schedule`): **Class times** (weekly `program_schedule_items` + sessions for that program; **Space** column from `venue_id` / venue name) | **Activity planner** (`schedule_activities` Schedule Builder, locked to the open program; `?section=activity-planner`). Leftover department `?tab=schedule` opens the Programs doorway so staff pick a year. Schedule header CTAs: **Check space availability** → Facilities; **View Master Calendar** → Events. Operating **Financial** sub-tabs: **Payroll** (KPI cards: **Employees** + **Amount** only; Log hours / Create pay period actions; no pay-period list; Finance → Payroll queue for Mark paid) | Expenses | Financial Summary. Scoped to **open** years only (`draft`/`active`/`paused`). URLs: `/programs/[id]?tab=schedule`, `/programs/[id]?tab=schedule&section=activity-planner`, `?tab=employees`, `?tab=financial` (default Payroll), `?tab=financial&section=payroll|expenses|budget`. Legacy `?year=&tab=schedule` opens that program’s Schedule tab. Key files: `department-group-workspace-client.tsx`, `program-workspace-client.tsx`, `department-schedule-panel.tsx`, `department-payroll-panel.tsx` (`variant=roster|periods`), `department-employee-profile-sheet.tsx`, `finance-payroll-queue-panel.tsx`.

**Department Registrations tab (July 2026 / August 2026):** Enrollments + Applications live under **Registrations** (UI label; URL `?tab=students`). The page follows the program **Enrollment process** (Settings → Program defaults): **Application & Approval** shows compact attention cards (Needs Review, Awaiting Evaluation, Approved — Registration Pending, Enrolled, Balance Due) and two sub-tabs (**Applications** | **Enrollments**). **Direct Registration** hides Applications and shows Enrollments only (Enrolled, Pending Checkout, Balance Due, Waitlisted, Cancelled). Approved is an application status chip, not a separate tab. Application status, enrollment status, and payment status are independent — unpaid **Active** enrollments stay on the roster. Run **`scripts/280_program_enrollment_process.sql`**. Key files: `lib/programs/enrollment-process.ts`, `department-students-panel.tsx`, `department-applications-panel.tsx`, `department-participants-panel.tsx`, `program-defaults-settings-panel.tsx`. Applications table: **Program** column shows offering name only; column header filters for Participant, Program, New / Returning; status chips All / Needs Review / Evaluation / Approved / Waitlisted / Declined. **Click a row** to open the application form **dialog**. Batch Approve / Decline appears only after rows are selected. **Last Updated** shows staff who approved/declined (`evaluated_by` + `evaluated_at`) when present, otherwise last staff save (`updated_by_user_id` + `updated_at`) — not the applicant submit time (run **`scripts/237_program_application_updated_by.sql`**). **Un-approve** returns an approved-but-not-registered application to Needs Review (`submitted`). Answers stored in `program_applications.application_answers` (run **`scripts/236_program_application_answers.sql`**; includes `requested_offering_ids` for multi-course). Enrolled roster rows have **Move** next to **View registration** (same year/season; payments stay). Legacy URLs `?section=review|approved|roster` map to Applications / Enrollments. Key files: `department-students-panel.tsx`, `department-applications-panel.tsx`, `department-application-detail-sheet.tsx`, `program-application-form-fields.tsx`, `department-participants-panel.tsx`.
...
**Program Overview (August 2026):** Program workspace tabs (in the Programs module at `/programs/[id]`): **Overview | Offerings | Registrations | Schedule | Finance | Reports | Settings**. Header shows the program name with **Academic/Seasonal** and **status** badges (Draft / Active / Paused / Closed / Archived); the department name is a blue link under the title to `/workforce/departments/[id]`. **Overview** is a compact live dashboard: **Program health** KPIs (Enrollments / Offerings / Need review / Outstanding / Collected), **Needs attention** (applications, outstanding balances, capacity — not cancelled enrollments; refunds stay on Financial summary), an **Offerings** preview (instructor, enrollment, registration window), **Financial summary** (charges, discounts, FA, received, refunds, outstanding), and **Recent activity** (registrations, payments, applications). Links go to Offerings, Registrations, and Finance. **Schedule** is class times + activity planner for that program. **Finance** (`?tab=finance`) has **Transactions** (this program’s payment transactions; no donations, no program picker) and **Payment Summary** (`?section=payment-summary`; family balances). Leftover `?tab=reports&section=tuition-plans|payment-summary` opens Finance → Payment Summary. Org-wide Finance remains `/finance/transactions`; payroll stays on department **Financial**. **Reports** (`?tab=reports`) reuses Home reports already locked to this program: Enrollments | Add-ons | Waitlist | Attendance (`?section=`). Childcare lives on **Event Management → Reports**. Org-wide program reports remain `/programs/reports/...`. **Settings** sub-tabs: **General** (`ProgramBasicsSection`) | **Program defaults** (`ProgramDefaultsSettingsPanel`) | **Registration** | **Notifications** | **Promo Codes**. Registration/Notifications still save to `department_program_settings` (department-scoped stubs). Leftover department `?tab=settings&section=registration|notifications|promo-codes|year-defaults` opens the Programs doorway. Leftover `?section=service-needs` opens department **Events**. Department **Settings** is name/color, Description, and Terms (one **Save** at the bottom). **Delete department** is blocked unless the department has no programs, offerings, or employees. Department **Financial** (Payroll / Expenses / Financial Summary) stays on the department workspace. Key files: `program-workspace-client.tsx`, `program-workspace-finance-panel.tsx`, `program-workspace-reports-panel.tsx`, `program-workspace-settings-panel.tsx`, `program-policy-settings-panel.tsx`, `department-program-dashboard-panel.tsx`, `lib/programs/program-overview-metrics.ts`, `lib/programs/program-workspace-path.ts`, `department-program-overview-panel.tsx`, `department-schedule-panel.tsx`, `program-basics-section.tsx`.
