...
### Session refresh / middleware (September 2026)

Supabase only allows one refresh of a given refresh token at a time. Long sessions, extra browser tabs, or extra Cursor chats hitting the same login can trigger `Too many concurrent token refresh requests`. Middleware used to call `getUser()` on **every** path, including `/login`, so a stuck refresh 504'd Vercel (`MIDDLEWARE_INVOCATION_TIMEOUT`) and blocked sign-in.

`lib/supabase/middleware.ts` now:

* Skips session refresh on `/login`, `/forgot-password`, and `/auth/*` so the login page always loads
* Times out `getUser()` after 8 seconds on other routes (under Vercel's ~25s middleware limit)
* Clears poisoned `sb-*` cookies and sends protected routes to `/login` when refresh fails or times out

**If login 504s before this is deployed:** open a private/incognito window (or clear cookies for the site) and sign in there. That bypasses the stuck session cookie immediately.
...
* Role permission editor (open a role → grouped checkboxes; no all-roles matrix)
* Server-side protection
* Unauthorized page
* Permission-aware sidebar
* **Tiered staff navigation (July 2026):** Primary sidebar (`180px`, icon + label). **Manaratee logo** sits in the staff header above the breadcrumb row (large wordmark, links to Dashboard). Organization **name** then **logo** sit on the far right of the header bar (left of the user menu). Sidebar keeps a matching-height spacer (no duplicate logo) plus a breadcrumb spacer so **Dashboard** aligns with the breadcrumb row (`Dashboard > …`). Breadcrumb path sits on the row below the header. **Department workspace (July 2026):** `/workforce/departments/[id]` appends the department name on the **header** trail (`Dashboard > Programs/ Events > Departments > Education`) via `Header` `breadcrumbExtras` — no second in-page breadcrumb. **Page breadcrumbs (July 2026):** In-page trails (above page titles) use the same chevron style as the header (`Education > Program Name`) via shared `PageBreadcrumbs` (`components/navigation/page-breadcrumbs.tsx`). Parent segments are clickable so staff can step back to department, list, or module roots. Applied on Vendor Hub / Event Management shells, employee/group detail pages, and similar detail surfaces. **Offering manage** uses `Header` `breadcrumbExtras` (same pattern as department workspace) — single header trail only. Clicking a module opens a slide-out drawer with expandable groups; choosing a destination navigates and closes the drawer. **Module order (August 2026):** Dashboard → Contacts → **Programs/ Events** → Membership → Donations → Venue Rentals → Vendor Hub → **Community Calendar** (shared top-level when Vendor Hub and/or Event Management is enabled; `/community-calendar`) → **Facilities** (module slug `spaces`; formerly Bookings / Facility Manager) → Billing → Settings. **Facilities submenu:** Overview, Reservation Center, Calendar, Inventory, **Settings** (tabs: Spaces, Setup Styles at `/facilities/settings/*`). **Spaces cards (July 2026):** metric cards and filters removed; venues show in a 3-column card grid with color, flyer upload, description, capacity/location, and rentals badge (status removed from UI). Run **`scripts/204_venue_color_flyer.sql`**. **Per-day hours & rates (July 2026):** Edit Venue uses a Sunday–Saturday table (open toggle, start/end, flat, hourly) stored in `rental_space_pricing`; toggle label **Available for rental**. Optional seed from legacy peak/non-peak: **`scripts/205_seed_venue_day_pricing.sql`**. Membership is already implemented (`/membership`); if missing from the rail, enable it for the org (repair SQL `scripts/165_ensure_membership_sidebar.sql`, or Platform Admin → Organizations → modules). Flat list (no People/Operations regrouping). Shared chrome heights: `lib/layout/staff-dashboard-chrome.ts`. **Event Management visibility (July 2026):** Org-enabled modules always appear in the staff rail (sub-nav still permission-gated). Loader recovers missing `modules` embeds, normalizes slugs, and keeps product modules even if `is_active` was flipped off. Org **Super Admin** gets full sidebar permissions. Enabling a product module seeds Admin/Super Admin permissions. Repair script: `scripts/138_ensure_event_management_sidebar.sql`. Key files: `components/layout/sidebar.tsx`, `lib/organizations/load-organization-sidebar-modules.ts`, `lib/organizations/sidebar-nav-context.ts`, `lib/modules/organization-module-access.ts`.
* **Staff navigation unmerge (August 2026 / September 2026):** Top-level **Programs/ Events** is split. **Administration** appears after Directory only when **Programs** or **Event Management** is subscribed (it is a capability of those modules, not a core rail item): Departments, Employees, plus Volunteers and Child Care Providers, plus Service Providers when Facilities is enabled. Fund Development–only and Membership–only tenants do not see Administration. Routes stay under `/workforce/*`. **Programs** (when subscribed) has a flyout: **Overview**, **All programs**, **Registrations**, **Finance**, **Financial Assistance**, **Reports**, **Settings**. Overview (`/programs`) keeps Programs and Offerings KPI cards only. All programs is `/programs/list`. Offerings catalog `/programs/catalog` stays as a route (not in the flyout). Academic vs Seasonal is a **tag** on each program, not a nav split. **Event Management** is its own rail item (Overview, Events, Master Calendar, Ticketing, Reports, Settings). Finance is not a rail item when Programs is on. Directory no longer lists Donors or Administration-owned roles (employees, volunteers, childcare providers, service providers). Key files: `lib/navigation/staff-module-nav.ts`, `lib/programs/programs-module-nav.ts`, `components/layout/sidebar.tsx`, `lib/directory/directory-roles.ts`.
* **Subscription catalog matches staff modules (August 2026 / September 2026):** Tenants subscribe to **Event Management**, **Programs**, **Vendor Hub**, **Venue Rentals**, **Fund Development**, and **Membership**. **Workforce**, **Finance**, **Facilities**, **Community Calendar**, and **Sign Ups** are not sellable SKUs. Administration (`workforce`) is included with Programs or Event Management; Finance is included with Programs; Facilities (`spaces`) is included with Programs, Event Management, Venue Rentals, and Vendor Hub; Community Calendar and Sign Ups appear with Vendor Hub or Event Management. Membership and Fund Development do not include Administration, Facilities, Community Calendar, or Sign Ups. SQL **`273_subscription_modules_match_staff_nav.sql`**, **`298_administration_programs_events_capability.sql`**. Keys: `lib/modules/module-catalog.ts`, `lib/modules/staff-module-labels.ts`.
* **Module-based subscription pricing (August 2026):** Super Admin sells those six product modules with integer-cent monthly prices and a configurable multi-module discount (`module_discount_rules`). Monthly total = selected module prices − discount, unless a custom/grandfathered amount is locked on `organization_subscriptions`. Default Enabled is retired; new tenants do not auto-receive product modules. `/admin/plans` redirects to `/admin/modules`. Historical `plans` rows remain. Stripe platform SaaS billing is still a TODO. SQL **`274_module_based_subscription_pricing.sql`**. Catalog reads `modules.monthly_price_cents` only — do not select legacy `price_monthly` / `monthly_price` (those columns are missing on some databases). Super Admin organization module selection is per-module (no persona/preset cards). Super Admin Modules uses one Add / Edit dialog; included capabilities are checkboxes stored on `modules.included_capability_slugs` (SQL **`275`**). Super Admin pages use a dedicated main-column scrollbar (`app/(platform)/layout.tsx`) so the Modules catalog, discount card, and organization sheet tabs stay reachable. Keys: `lib/billing/module-subscription-pricing.ts`, `lib/billing/organization-subscription-service.ts`. APIs: `/api/platform/modules`, `/api/platform/module-discount-rules`, `/api/platform/organizations/[organizationId]/subscription`.
* Subscription-aware modules
* **Roles & Permissions subscription filter (June 2026 / September 2026):** Settings → Roles & Permissions lists roles in a table; open a role to set grouped permission checkboxes (saves immediately). Only modules enabled on the org appear (`lib/permissions/permission-definitions.ts`, filtered via `loadOrganizationEnabledModuleSlugs`). Core modules (Settings, Contacts) always appear; product modules (e.g. Donations only for MAS Dallas) gate their permission groups. Staff permissions appear only when Administration is on (Programs or Event Management). **Facility Manager** and **Facility Coordinator** roles are hidden unless the org has **Facilities** (`spaces`) or **Venue Rentals** (`bookings`) enabled (`filterOrganizationRolesForOrganization` in `lib/permissions/facilities-access.ts`). Key: `roles-permissions-client.tsx`.
* **Org Super Admin + Admin system roles (August 2026):** Creating an organization always inserts **Super Admin** and **Admin** into `organization_roles` (full permissions). Platform Admin creating an org does **not** become an org member; the contact email is invited as Super Admin. `admin@manaratee.com` is kept as `platform_admins` / `profiles.is_platform_admin`. Enter Organization marks that row `platform_support_access` so it is hidden from Settings → Users and from the `organization_users` view. Run **`scripts/271_org_system_roles_and_platform_admin.sql`** then **`scripts/272_hide_platform_admin_org_memberships.sql`**. Keys: `organization-system-roles.ts`, `organization-role-actions.ts`, `/api/organizations/create`.
* **Multi-org login (August 2026):** One Auth user can belong to many orgs (for example viewer at MAS Dallas and Admin at a demo org). Invite/magic-link sets `user_metadata.organization_id` to the invited org. After password setup, staff roles in that org open `/dashboard` there; otherwise the first staff membership wins over a viewer membership. Customer portal still has the organization switcher.
...
* **Bookings Calendar merge (July 2026):** Under **Facilities** (module slug `spaces`; sidebar label **Facilities**), **Space Availability** and **Schedule** are a single sidebar item **Calendar** → `/facilities/calendar` (ops master calendar: full titles, blocks, event planning). **List view (September 2026):** Day | Grid | List. List (`?view=list`) is an agenda grouped by day (`SUNDAY, SEPTEMBER 19, 2026`), with time, space, holder, title, and PoC (event coordinator, rental billing contact, or program instructor). Click the start date and the end date separately to pick a custom range (`?date=` + `?endDate=`; default 30 days). Keys: `reservation-calendar.tsx`, `reservation-calendar-list.tsx`, `lib/reservations/calendar-list.ts`. **Shared scheduling foundation (July 2026):** Venue Rentals, internal Events, and Program sessions all feed one schedule — stored rows in `resource_reservations` (rentals/events/holds/closures/maintenance) plus **per-session expand** of `program_schedule_items` (not one long program block). Module calendars are filtered views of the same data via `?sources=` (`venue_rental`, `internal_event`, `program_facility`); Facilities Calendar (no filter) shows everything. Redirects: `/bookings/calendar` → Facilities `?sources=venue_rental`. **Events → Master Calendar** (`/event-management/calendar`) is the read-only department events calendar (+ **Create event** in place)—it does **not** redirect to Facilities Calendar. Legacy `/workforce/departments/calendar` redirects here. Programs sidebar **Calendar** removed (Aug 2026) — `/programs/calendar` redirects to Facilities. **Setup/cleanup:** `setup_minutes` / `cleanup_minutes` on `rental_reservations` and `internal_events` expand the occupied window written to `resource_reservations` (migration **`209_shared_scheduling_foundation.sql`**). Org defaults + optional per-space overrides for new rentals: migration **`222_venue_rental_setup_cleanup_buffers.sql`** (Venue Rentals → General + Facilities → Spaces). **Event location types (July 2026):** `internal_events.location_type` (`facility` | `online` | `external`) + `location_address` (migration **`210_internal_event_location_type.sql`**); online/external do not write facility calendar rows. **Programs ↔ venues:** optional `program_schedule_items.venue_id`; conflict checks on save use the shared facility schedule. **Basic vs advanced Facilities:** enabling Venue Rentals, Events, or Programs implies `spaces` (calendar + space settings + conflict checking); Overview / Reservation Center / Inventory stay advanced (shown when Venue Rentals is enabled). Shared venue picker: `components/reservations/facility-venue-select.tsx`. Separate rental / event / program forms preserved — no combined generic form. **Overview:** `/facilities/overview` is a **read-only** facilities landing for facility staff — **Confirmed rentals** + **Setup briefs needing review**. Approvals/payments stay in Venue Rentals. **Double-booking prevention:** rental submits, event creates, and program schedule saves check `resource_reservations` plus program sessions (prefer `venue_id`, fallback location name match). **Inventory:** `/facilities/inventory` — run **`207`** then **`208`**. **Setup Styles:** Facilities → Settings → Setup Styles. **Event Types:** Venue Rentals → Settings → Event Types. **Organization Master Calendar (planning only):** separate org-wide collaboration calendar (not Facilities). Vision doc: `docs/organization-master-calendar-vision.md` — do not implement until requested.

* **Event Management source of truth (September 2026):** Event Management, Fund Development campaigns, department Events, and Community Calendar listings for those events share one UUID on `internal_events`. Campaign link is `internal_events.campaign_id` (JSON `ticketing_config.linkedCampaignId` kept in sync). Childcare signup rows must belong to an event or program (`source_type` + `source_id`). SQL **`300_event_management_source_of_truth.sql`**. Keys: `lib/events/event-campaign-id.ts`, `lib/events/event-datetime.ts`.
* **Bazaars live in Vendor Hub (September 2026):** Bazaars are not Event Management workspaces. Create/edit is only in Vendor Hub (no EM picker, ticketing, or program link). Vendors are always on; childcare is never on. A lightweight `internal_events` row (`source_module = vendor_hub`) holds the date/place for an optional on-site Facilities reservation and Community Calendar listing. Those rows are hidden from Event Management lists, Overview Need Vendors, Master Calendar, and department Events. Staff `/event-management/[id]` URLs for bazaar holds redirect to Vendor Hub. Facilities calendar still shows the space hold and opens Vendor Hub. Graduation / open house with a few vendor tables stays on Event Management **Need Vendors**. SQL **`305_bazaar_owned_internal_events.sql`**. Keys: `ensure-bazaar-internal-event.ts`, `internal-event-source.ts`.
* **Sign Ups (September 2026):** Top-level staff menu **Sign Ups** when Vendor Hub or Event Management is on (not a subscription SKU). Flyout: **Overview**, **Notifications**, **Reports**, **Settings**. Overview (`/sign-ups/overview`) lists events with `internal_events.requires_volunteers` from Event Management and Vendor Hub. The name opens the real event. Volunteer times and slots, and who signed up, are on the bazaar **Sign-ups** tab and the Event Management **Volunteers** tab. Each has **Slots** and **Volunteers**. Upcoming is the default. Reports (`/sign-ups/reports`) lists every signup: event name, volunteer name, email, phone, slot, and time (`service_participations` pending or confirmed; slot is `volunteer_role`, time is `assignment_meta.shiftLabel`). Notifications and Settings are placeholders. A bazaar **Sign-ups** tab is always on. The bazaar appears on Overview after slots are saved. Legacy `/sign-ups/groups` redirects to Overview. `/sign-ups/messages` redirects to Notifications. `/sign-ups/events`, `/sign-ups/upcoming`, and `/sign-ups/previous` redirect to Overview. Keys: `lib/sign-ups/sign-up-overview-queries.ts`, `lib/sign-ups/sign-up-reports-queries.ts`, `lib/events/volunteer-windows.ts`, `components/events/event-volunteers-panel.tsx`.
* **Event Management Ticketing area (September 2026):** Event Management flyout includes **Ticketing** (`/event-management/ticketing`) with in-page tabs **Overview** · **Events** (ticketed events only) · **Orders** · **Check-in** · **Settings**. Settings holds ticket **Categories** plus default checkout / org promos / check-in app users. Org-wide door desk is Ticketing → Check-in (`TicketCheckInScanner`). Event workspace no longer has an Orders tab; `?tab=orders` / `?tab=attendees` open Ticketing → Orders filtered to that event. Event Management Reports is **Tickets** · **Childcare**. Legacy `/event-management/check-in`, `/event-management/reports/orders`, and `/event-management/settings/categories` redirect here. Keys: `staff-module-nav.ts`, `ticketing-tab-nav.tsx`, `ticketing-orders-client.tsx`, `ticketing-settings-client.tsx`.
* **Ticketing Overview issued/revenue (August 2026 / September 2026):** Event Management Overview shows ticket **Revenue** from every completed / partially refunded order (paged past the 1,000-row API limit). Issued counts stay on Events and Reports, not Overview. Remaining is unlimited (`—`) when a type has no capacity cap. Key: `ticketing-overview-queries.ts`.
* **Ticketing Orders event filter (August 2026 / September 2026):** Event Management → Ticketing → **Orders** (`/event-management/ticketing/orders`) lists the newest **50** orders with page numbers. The Events control is a checkbox list (no All / Active / Past groups); leave it empty to see every event, or check one or more events. `?event=` still pre-checks a single event. Legacy `/event-management/reports/orders` redirects here. Key: `ticketing-orders-client.tsx`.
* **Ticketing order cancel/refund (September 2026):** Orders table has no row ⋮ menu (order ID opens the detail sheet). **Actions** sits next to Order status and only **Export** (selected rows, or all filtered rows if none selected). Order detail **Cancel or refund** lets staff pick tickets, refund a custom/suggested amount, add a note, and choose whether to email the customer. Selected tickets are voided; remaining tickets stay valid. Unpaid orders cancel without Stripe. Paid Stripe Connect charges refund on the org account; pay-at-event is local only. Note is stored in `ticket_orders.metadata.staff_refund_note`. Keys: `ticket-order-detail-panel.tsx`, `ticket-order-cancel-refund-dialog.tsx`, `refundEventTicketOrder`.
* **Ticketing Events categories (August 2026 / September 2026):** Event Management → **Events** default columns are Event, Department, Date, Time, Location, Space, Status, Actions. Ticket sales columns (Issued, Remaining, Revenue) and a **Category** dropdown sit after Status when shown (`ticketing_event_categories`, SQL **`287`**). Add, rename, hide, or delete categories under **Ticketing → Settings**. Ticketed-only list is Ticketing → Events; it defaults to **Active events** (filter to Past or All). Status is **Draft**, **Active**, or **Past** (Past is automatic after the date). Event names are primary (blue) links. Staff can pick visible table columns from **Columns**. Keys: `event-management-events-client.tsx`, `ticketing-events-client.tsx`, `ticketing-event-category-actions.ts`.
* **Ticketing reports (July 2026 / September 2026):** Real ticket analytics under **Event Management → Reports → Tickets** (`/event-management/reports/tickets`) — always-on KPIs plus a **View** filter (**Days** / **Events** / **Customers**) and date range; CSV export matches the active view. Data from `ticket_orders` + `tickets` (no mock data). Reports also has **Childcare** (`/event-management/reports/childcare`). Orders live under Ticketing → Orders. Leftover `/programs/reports/childcare` redirects there. Key files: `lib/tickets/ticketing-reports-queries.ts`, `components/tickets/ticketing-reports-client.tsx`, `components/events/event-management-reports-nav.tsx`.
* **Event workspace Tickets (September 2026):** Event **Settings → Tickets** (formerly a top-level Tickets tab) uses a compact **Paid / Free** attendance picker. Mixed paid+free and open-public options were removed (use promo codes for comps). **Starts** / **Ends** sit beside attendance (date picker + 30-minute time list). Ticket types table is below. Orders are under **Ticketing → Orders** (not an event workspace tab). Legacy `?tab=tickets` / `?tab=ticketing` / `?tab=registration` open Settings → Tickets. Keys: `internal-event-registration-workspace.tsx`, `event-workspace-features.ts`, `date-picker-input.tsx`.
* **Event workspace Tickets tab (August 2026):** Event **Settings → Tickets** (formerly Registration / Tickets workspace tab): sales open/close as **Starts** / **Ends**; ticket types are Name / Price / Quantity. Enter/Tab on Quantity adds another row. Save via `updateInternalEventModules`.
* **Event workspace Attendees tab (August 2026 / September 2026):** Per-event Orders moved to **Ticketing → Orders**. Legacy `?tab=orders` / `?tab=attendees` redirect there with `?event=`. The former workspace attendee table (`internal-event-attendees-tab.tsx`) is no longer a workspace tab. Org-wide check-in is Ticketing → Check-in. **Attendee vs contact (August 2026):** Attendee is the ticket holder (child name + code). Contact is the Directory contact. Import/backfill SQL **`286`**. Keys: `ticketing-orders-client.tsx`, `ticket-order-queries.ts`, `ticket-order-actions.ts`.
* **Event workspace Registration offerings (August 2026):** Per-type **Visibility** (public / unlisted / private), **Min / Max per order**, and optional **Sales start / end** dates on **Settings → Tickets** (`event-ticketing-fields.tsx`, `ticket-types.ts`, migration **`252`**). Empty type dates inherit the event Starts / Ends window. `getTicketOfferingSaleStatus` enforces windows when `createTicketOrder({ enforceSaleWindows: true })` (public checkout path).
* **Event workspace Attendees — check-in & comms (August 2026 / September 2026):** Shared **Check in** (`TicketCheckInScanner`) on Ticketing → Check-in. **Phone only:** **Start camera** reads the ticket QR (`TicketQrCamera`, `jsQR` / `BarcodeDetector`). **Computer:** type the ticket code (no camera preview). Check-in by ticket code (`checkInEventTicketByCode` per event, `checkInOrgTicketByCode` org-wide) for staff with `events.checkin` or manage.
* **Event workspace Finance — campaign link (August 2026 / September 2026):** Optional **linked donations campaign** on `internal_events.campaign_id` (JSON `ticketing_config.linkedCampaignId` kept in sync); Finance tab picker + pledge/gift summary (`event-finance-queries.ts`, `updateEventLinkedCampaign`, `internal-event-finance-tab.tsx`). SQL **`300`**.
* **Campaign workspace Event tab (September 2026):** Campaign detail **Event** (`?tab=events`) lists the same `internal_events` row linked by `internal_events.campaign_id`. The tab shows the event name, date, and location; clicking the row opens `/event-management/[id]`. Pinned KPI cards for that event include tickets sold, remaining, and checked in **with a per-type breakdown on each card**, plus ticket revenue, **ticket donations** (checkout gifts), and waitlist (when used). Attach existing / Create event appear only when none is linked. No status tags or Check-in / Ticketing / Unlink buttons on the row. Ticketed events still appear on Event Management Overview and Events. If the org has Fund Development but not **Event Management**, the tab stays visible and shows a subscribe message (`ModuleNotSubscribed`) with Contact Manaratee (`support@manaratee.com`). Campaign managers (`donations.campaigns.manage`) may attach without full event manage. Keys: `campaign-events-tab.tsx`, `campaign-event-kpis.tsx`, `campaign-event-actions.ts`, `module-not-subscribed.tsx`. SQL **`300`**.
* **Ticketing Check-in (September 2026):** Event Management → Ticketing → **Check-in** (`/event-management/ticketing/check-in`) is the org-wide door desk for every ticketed event, plus shortcuts into Ticketing → Orders for that event. Phone: camera QR scan (`TicketQrCamera`). Computer: type-a-code only. Org-wide lookup: `checkInOrgTicketByCode`. Legacy `/event-management/check-in` redirects here.
* **Event workspace Vendors — Vendor Hub (August 2026 / September 2026):** Event Management **Need Vendors** is for non-bazaar events (graduation, open house with a few tables). Full bazaars live in Vendor Hub, not this tab. A leftover bazaar hold that still points at a ticketed EM event can show a Vendor Hub link (`getVendorHubLinkForInternalEvent`).
* **Event workspace Reports — activity (August 2026):** **Recent activity** card on Reports (check-ins, new registrations, staff confirmations) via `buildEventRecentActivity` in `event-recent-activity.ts`. Event Overview **Recent orders** shows a few latest orders/refunds (`buildEventRecentOrders`).
* **Event workspace Overview — sharing & net (August 2026 / September 2026):** Workspace tabs and Overview KPI cards stay pinned while the page scrolls. The event name is a blue link for staff who can manage — it opens the same create/edit booking form on this page. Overview KPI cards are Type (One-time / Recurring), Schedule, Location, Childcare, Volunteers, and Vendors (Needed / Not needed). Recurring includes same-name series such as weekly Learn, Love, Live the Quran. Flyer, description, event details, and Community Calendar live on **Settings → General**. Community Calendar **Copy link** / **Download QR** / **Open page** show when visibility is Public (`internal-event-community-calendar-card.tsx`). Registered and Event net are not KPI cards — registered stays on the Overview Tickets card; revenue / expenses / net stay on Finance. **Recent orders** is at the bottom. Department and event type show under the event name only.
...
* **Event ticket Stripe refunds (August 2026 / September 2026):** Staff **Refund** on the event Orders tab (amount) and Ticketing order detail **Cancel or refund** (per-ticket) call `refundEventTicketOrder`. Ticketing dialog supports ticket checkboxes, a refund amount (suggested from selected ticket prices), a staff note, and an email checkbox. Stripe Connect charges refund that amount on the org account (`createStripeTicketRefund`); pay-at-event / staff **Mark paid** orders are refunded in the database only. Selected tickets are voided; unselected tickets stay valid (`partially_refunded`). Refunding the remaining balance sets `refunded`, voids leftover tickets, and restores inventory. Purchaser email uses `canceled` / `partial_refund` / `refunded` when the checkbox is on. Webhook `charge.refunded` applies `charge.amount_refunded` (full or partial). Finance, overview, and ticketing reports use net remaining (`ticket_orders.refunded_amount_cents`). If Checkout completes after the order was already canceled, the charge is refunded instead of completing the order. SQL **`258_ticket_order_refunded_amount.sql`**. Keys: `ticket-stripe.ts`, `ticket-order-actions.ts`, `ticket-refund-math.ts`, `internal-event-refund-dialog.tsx`, `ticket-order-cancel-refund-dialog.tsx`.
* **Event documents (August 2026):** Settings → **Features** → Event documents uploads PDFs/images (`event_documents`, SQL **`254_event_documents.sql`**, `program-flyers` storage). **Public** files appear on the community event page. Key: `event-document-actions.ts`, `internal-event-documents-card.tsx`.
* **Event communications overrides (August 2026):** Event Settings → **Checkout** stores optional confirmation/reservation subject + extra message on `ticketing_config.communications`. Applied by `sendEventOrderConfirmationEmail` (public registration, Stripe complete, Mark paid, Resend). Keys: `internal-event-settings-workspace.tsx`, `ticket-confirmation-email.ts`, `ticket-types.ts`.
...
* **Event Workspace redesign (August 2026 / September 2026):** Tabs use progressive disclosure: **Overview | Staff | Volunteers | Youth | Vendors | Finance | Reports | Settings** (hidden when that module is off). Ticket types live on **Settings → Tickets**. Features in `workspace_features` + Settings toggles; attendance mode in `ticketing_config.attendanceMode` (`paid` | `free`; legacy `paid_and_free` / `open_public` still parse and map to Paid / Free). Overview is a command center (KPIs, attention alerts, ops cards). Finance = ticket revenue + `event_expenses` ledger. Reports = attendee CSV + financial summary. SQL **`252_event_workspace_redesign.sql`**. Keys: `event-workspace-features.ts`, `event-overview-metrics.ts`, `internal-event-overview-dashboard.tsx`, `internal-event-registration-workspace.tsx`, `internal-event-finance-tab.tsx`, `internal-event-reports-tab.tsx`, `internal-event-features-settings.tsx`.
* **Event workspace Settings tab (August 2026 / September 2026):** Settings has **General | Tickets | Features | Checkout**. **General** is flyer, description, event details, and Community Calendar (`internal-event-general-settings.tsx`). **Tickets** is attendance (Paid/Free), Starts/Ends, and ticket types. **Features** is service needs, workspace modules, visibility/access, and documents. **Checkout** is questions/promos when the event has paid or free tickets. Turning on Youth or Vendors in Service Needs also turns on those workspace tabs. Turning on Volunteers shows the **Volunteers** tab. The **Staff** workspace switch is paid staff only. Overview shows coordinator name when set. Requires SQL **`252`**. Key: `internal-event-service-needs-settings.tsx`, `internal-event-features-settings.tsx`, `internal-event-meta-settings.tsx`, `updateEventWorkspaceMeta`, `internal-event-settings-workspace.tsx`.
...
* **Internal event delete guards (July 2026 / August 2026 / September 2026):** Delete is available on the event workspace and on the Events list (three-dot **Actions** → Copy or Delete). Server blocks delete when the event has **any** ticket orders/tickets (financial activity), **any** volunteer/vendor/childcare-provider sign-ups, **any** childcare/youth registrations, or a linked Vendor Hub bazaar (`vendor_hub_events.internal_event_id` is `ON DELETE RESTRICT`). Key files: `getInternalEventDeleteBlockers` in `lib/events/internal-event-actions.ts`, `components/events/internal-event-card-actions.tsx`.
* **In-place create event + optional on-site approval (September 2026):** Create event stays on the page it was opened from (campaign Event, department Events, Event Management Overview/Events, Master Calendar). **Online** and **External Venue** skip Facilities and never wait for approval. **On-site (Center)** shows a **View facility calendar** link (opens in a new tab) so staff can check space and come back to finish — no room picker in the create window. Org setting **Approval required** lives on Event Management → Settings → General (`event_management_settings.approval_required`, default **off**, SQL **`291`**). When on, only on-site events go to Pending. **Facilities Calendar Create event** jumps to **today** in Day view (no create popup); staff click an open time slot to open the form after checking availability. Legacy `/event-management/create` and `/request` redirect to `/event-management/events?create=1`. Key: `create-internal-event-button.tsx`, `facility-event-request-drawer.tsx`, `event-management-settings.ts`.
* **Event workspace status (September 2026):** Workspace status is a compact dropdown: **Draft**, **Pending**, **Live**, **Completed**, **Cancelled**. There is no **Approved** tag. **Pending** appears only while an on-site event is waiting (and only if Event Management → Settings → **Approval required** is on). Choosing **Live** from Pending runs `approveInternalEventRequest`. Key: `internal-event-status.ts`, `internal-event-status-select.tsx`, `internal-event-workspace.tsx`.
* **Event workspace Volunteers (September 2026):** Event Management **Volunteers** (`?tab=volunteers`) appears when `requires_volunteers` is on. A Vendor Hub bazaar always shows **Sign-ups**. Both open the same panel: **Slots** (one default row with hourly start, end, job name, and openings; **Add slot** on that row adds more jobs) and **Volunteers** (name, email, phone, slot, time). There is no sign-up toggle and no separate times card. One **Save** at the bottom writes slots and sets `requires_volunteers` when at least one time window is present, which lists the event on Sign Ups → Overview. Clearing every window and saving turns that off. **Staff** (`?tab=staff`) is paid staff only. Saved on `service_requirements.volunteers.windows`. Key: `event-volunteers-panel.tsx`, `volunteer-windows.ts`.
* **Department workspace Events (July 2026 / September 2026):** Programs/ Events → **Departments** → **Events** (`?tab=activity`) lists that department’s `internal_events` with open/edit/delete (manage permission). CTAs: **View Master Calendar**; **Check space availability** → Facilities Calendar; **Create event** opens in place on the department page (no Facilities redirect). On-site events show a **View facility calendar** link so staff can check rooms and come back to finish. Panel: `components/departments/department-events-panel.tsx`.
* **Events Master Calendar (July 2026 / August 2026):** Master Calendar lives under **Event Management → Master Calendar** (`/event-management/calendar`) as a **sidebar menu item** (not an in-page tab). Events list is **Event Management → Events** (`/event-management/events`). Shows all `internal_events` for the month (facility, online, external) with department filter; not the Facilities room grid. Legacy `/workforce/departments/calendar` redirects here. **Facility schedule** deep-links Facilities `?sources=internal_event` for space conflicts. Key files: `internal-events-calendar-client.tsx`, `internal-event-calendar-queries.ts`, `event-management-section-path.ts`.
* **Event Management Overview vs Events (August 2026 / September 2026):** Sidebar first item is **Overview** (`/event-management`) — four KPI cards on one row (upcoming **one-time** and **recurring** series counts, Need Childcare / Volunteers / Vendors) plus a **This month** table of upcoming events (event name, **Ticketed** tag, date, childcare / volunteers / vendors needed). Ticket **Revenue** lives on Ticketing → Overview. **Upcoming** counts future-only events (start time has not begun; cancelled/declined excluded). Recurring series such as Tuesday Night Live count as **one** recurring event, not each meeting date. Period filter (Today / This Week / This Month / All Events / Past Events) still scopes Need Childcare / Volunteers / Vendors. **Attention required** stays below the table. **Events** (`/event-management/events`) is the view/manage catalog: **One-time** and **Recurring** tabs, search, Department, Status (Active / Draft / Past / All), Create event. One-time lists each date. Recurring shows **one row per series** (same name or `recurrence_config.seriesId`), with Schedule, next/last date, and remaining meetings; the name opens the **next upcoming** meeting (`/event-management/[id]`). Columns: Event, Department, Contact (requester), Phone, Email, Date, Time, Location, Space, Status (**Draft** / **Published** / **Completed** when the date has passed), ticket columns when relevant, then Actions (three-dot menu: **Copy** / **Delete**). Use **Columns** to show or hide optional fields. Rows sort soonest upcoming first (past dates stay hidden on Active). Searching a Recurring series shows KPI cards for remaining recurrences, schedule, and time. Key: `event-management/page.tsx`, `event-management/events/page.tsx`, `event-management-events-client.tsx`, `event-management-events-filters.ts`, `event-management-dashboard-panels.tsx`.
* **Internal Events Google Form import (September 2026):** `InternalEvents.xlsx` department requests → Event Management for MAS Dallas. One-time rows only (36 form one-time − 5 Other = 31 imported). Recurring programs skipped. Center Programs → Center; Quran Institute → Quran Institute for Ladies. Created departments **CYP**, **Scouts**, **Dawah Outreach**. Created rooms Main Lobby, Conference room, New Youth Lounge, Pool, Pool Room, Open space between the 2 buildings. Requesters are Directory contacts + event coordinator (no staff logins). Past → `completed`, future → `confirmed`; Community Calendar stays private. Overnight when the form end clock is at or before start. Tag `INTERNAL_EVENTS_XLSX_V1`. Script: `node scripts/import-internal-events-xlsx.mjs` / `--execute`. Reports: `scripts/reports/internal-events-xlsx-import-dry-run.json`, `internal-events-xlsx-import-execute.json`.
* **Public recurring gatherings import (September 2026):** Drop-in series into Event Management as one row per meeting, walk-in (`open_public`), no signup. **Learn, Love, Live the Quran** weekly Wed 9–10 AM MPH (last Wed of month potluck 9–11 AM); **Thursday Halaqa for Ladies** weekly Thu 11 AM–2 PM MPH (first Thu 2026-05-28 through last Thu of May 2027); **Tuesday Night Live** weekly Tue 7–8:30 PM Youth Lounge (middle school girls, **not** on Community Calendar); **Crochet Clique** four first-Saturdays every two months; **Men Talk** Jun–Jul Fridays plus named speaker nights. Public series `community_calendar_status = published`. **Her Space** skipped (no dates on the form). Tag `INTERNAL_EVENTS_RECURRING_PUBLIC_V1`. Script: `node scripts/import-internal-events-recurring-public.mjs` / `--execute`.
* **MUHSEN / Sabiqoon / Men in the Making (September 2026):** **MUHSEN Sunday School 2026-2027** is an Education academic program (not Event Management) with registration and a one-time Tuition fee plan. Offering **Ages 5-12** (capacity 5), Sundays 9:30 AM–1:00 PM Youth Lounge, lead Aliya Alison Lee. Tuition amount was not on the facility form — set it on the offering fee plan before taking payments. **Sabiqoon** (5 Sundays 2026-05-31–2026-06-28, Youth Lounge) and **Men in the Making** (Saturdays 2026-07-04–2026-09-05) are walk-in Event Management series like Tuesday Night Live, published to Community Calendar. Sabiqoon clock time was missing on the form (imported as 11:00 AM–1:00 PM). Men in the Making overlapping form windows were stitched at Aug 1 (Jul 4–25 MPH+Pool Room+Pool 12:00–2:45 PM; Aug 1–Sep 5 MPH+Banquet Hall+Pool 12:00–3:00 PM). Tag `MUHSEN_SABIQOON_MITM_V1`. Script: `node scripts/import-muhsen-sabiqoon-men-in-the-making.mjs` / `--execute`.
...
* **Unified Facilities event request drawer (July 2026 / September 2026):** The same booking drawer is used for create/edit. **Create from Event Management / campaign / department / Master Calendar** stays on that page (`CreateInternalEventButton`, `spaceMode=calendar-link`) — on-site has a **View facility calendar** link, not a room picker. **Facilities → Calendar → Create event** goes to today’s Day view; click an empty slot (or grid cell) to open the create drawer with that room and time (`spaceMode=select`). **Online** and **External Venue** confirm immediately (no Facilities, no approval). **On-site** waits only when Event Management → Settings → **Approval required** is on (default off). Prefills department + requester name. Schedule is **One-time**, **Recurring** (daily/weekly/monthly rule), or **Custom dates** (add meeting dates one at a time; same time of day; still listed on Events → Recurring as `Custom dates`). Legacy `/event-management/create` and `/request` redirect to Event Management Events with `?create=1`. **Edit** from event cards / workspace (`/event-management/[id]/edit`) opens the **same drawer**. Drawer is **booking-only** (date/time/location/setup/recurrence) — registration, staff, youth, and vendors are configured on the **event workspace** tabs. Staff Tools keeps `/customer/staff/events/request` (same drawer, calendar-link). Key files: `facility-event-request-drawer.tsx`, `create-internal-event-button.tsx`, `facility-event-edit-page-client.tsx`, `reservation-calendar.tsx`, `internal-event-actions.ts`, `event-recurrence.ts`.
...
| My Tickets | `event-management` (or `ticketing`) |
| Profile → Applications (Volunteer / Childcare) | `programs` or `event-management` |
| Profile → Applications (Vendor) | `vendor-hub` |
| Dashboard / Profile | always visible |

Key files: `lib/customer/customer-portal-modules.ts` (client-safe), `lib/customer/customer-portal-modules-server.ts` (server loaders/guards), `components/customer/customer-nav.tsx`, `app/(customer)/layout.tsx`. Disabled module routes redirect to `/customer/dashboard`. **Dashboard** (`/customer/dashboard`) is the customer home page: compact welcome header (first name, email, member since, Edit Profile) plus module-aware previews. With Fund Development enabled it shows Lifetime Giving / Active Pledges / Outstanding Balance, **Needs Your Attention** (outstanding pledges; hidden when none), up to **2 featured campaigns**, and compact **Recent Activity** from existing payments and pledges. Empty attention/activity/campaign sections are hidden rather than filled. Other modules can add sections later without a plugin framework (`getCustomerDashboardSectionFlags` in `lib/customer/customer-dashboard-data.ts`). Campaigns have no featured-flag column; the dashboard picks up to two active campaigns (prefer goal + description). **My Transactions** (`/customer/transactions`): customer read-only mirror of contact **Financial** (KPIs, Financial by Module, Recent Transactions, Financial Summary / open balances) via `ContactFinancialPanel` `variant="customer"` + `loadCustomerMyTransactionsSummaryAction`. Visible when donations, programs, bookings, or membership is enabled. Giving UI lives under **Donations → Giving Opportunities**; donations tab **Giving History** lists donation payments. Customer portal **My Donations** (`/customer/donation`) uses Donation Options on Giving Opportunities (campaigns live on the Dashboard, not this tab), dynamic summary cards (Lifetime Giving, Active Pledges, Outstanding Pledge Balance, and Next Payment only when scheduled), and polished empty states. Customer sidebar branding uses the active org `logo_url` with **organization name** in bold below the logo (falls back to name-only or Manaratee logo). **Profile** submenu (Family, Notification Preferences, Applications) appears only after the donor opens Profile. **Applications** lists Vendor, Volunteer, and Childcare only when the matching product is subscribed: Volunteer and Childcare need **Programs** or **Event Management** (staff review is Administration); Vendor needs **Vendor Hub**. A Fund Development–only org does not see Applications. Direct `/customer/apply/*` URLs redirect to the dashboard when the module is off. **Notification Preferences** (`/customer/profile/notifications`) shows toggles only for org-enabled modules (`lib/customer/customer-notification-preferences.ts`); Donations module includes payment completed, payment charges, failed transactions, pledge reminders, and SMS payment reminders, plus org-wide newsletter.

For a donations-only org (e.g. MAS Dallas), enable only `donations` in platform admin → organization modules.
...
**Bazaar create/edit (August 2026 / September 2026):** Create/Edit Bazaar Event is core details only: name, dates, hourly start and end (one list: 11 AM, 12 PM, 1 PM), location, description, optional on-site spaces, primary contact, and Community Calendar. Organizer name is the tenant organization (for example MAS Dallas) and is stored on `organizer_name` at save. Event type, vendor applications, booth setup, payments, public info, and internal notes are not on this form. Booth setup and payments are on the bazaar **Settings** tab. Persists `organizer_contact_id`, `organizer_name`, and `venue_id` on `vendor_hub_events`. On-site spaces are optional: check one or more facilities and each is held on the Facilities calendar (`internal_event_venues`; `vendor_hub_events.venue_id` stays the first space). Off-site leaves every space unchecked. There is no Event Management picker, ticketing, or program link. Run **`scripts/227_vendor_hub_event_organizer_venue.sql`**, **`scripts/300_event_management_source_of_truth.sql`**, and **`scripts/305_bazaar_owned_internal_events.sql`**.

**Vendor Hub Reports (August 2026 / September 2026):** `/vendor-hub/reports` no longer uses hardcoded sample vendors/revenue. Tabs are **Orders** (`?tab=vendor-sales`), **Booth Performance**, and **Participation History** (optional `?eventId=` on orders/booth; `?tab=history` and optional `?contact=`). **Orders** is one row per booth assignment/payment. **Type** is what the vendor sells (coffee, clothing, hot food) from `vendor_hub_vendor_types`; **Booth type** is the physical space (stage, lobby, corner, regular, outdoor). Vendor name, email, and phone are separate columns, with a Columns popup and CSV export. The bazaar event **Orders** tab is the same report filtered to that event. Lifetime revenue, vendor counts, and booth utilization live here. Vendor Hub **Overview** is an attention queue (onboarding, next-bazaar booth inventory, recent orders). Empty states when there are no assignments/payments. Vendor names prefer application `business_name`, then `contacts.full_name`. **Booth Performance** uses organization default booth types from Settings when the event has that layout (totals from numbered inventory, not Eventbrite ticket-type counts). Key files: `lib/vendor-hub/booth-orders.ts`, `booth-orders-queries.ts`, `components/vendor-hub/booth-orders-table.tsx`, `vendor-hub-reports-queries.ts`, `vendor-hub-reports-client.tsx`.

**Bazaar event Orders tab (August 2026 / September 2026):** Event workspace tabs are **Overview | Orders | Evaluations | Messages**. Former **Reservations** and **Payments** tabs removed (old URLs redirect to Orders). Orders is the same report as Reports → Orders, filtered to this bazaar (row click still edits booth/fee). Route: `/vendor-hub/events/[eventId]/booths`. Key: `lib/vendor-hub/booth-orders-queries.ts`, `components/vendor-hub/booth-orders-table.tsx`, `components/vendor-hub/events/bazaar-event-vendors-client.tsx`, `lib/vendor-hub/vendor-hub-nav.ts`.

**Bazaar event Overview (September 2026):** The event name is a link that opens Edit Bazaar Event. KPI cards are **Booth registrations** (reserved/total, with remaining booths), **Revenue collected**, and **Evaluations pending**. Below that, **Booths by type** and **Recent orders** are for this bazaar only. Flyer, public share link, and copy/cancel/delete are on **Settings → General**. Booth pricing and booth types are on **Settings → Booths**. The **Sign-ups** tab is always shown, with **Slots** and **Volunteers**. There is no sign-up toggle; saving slots sets `requires_volunteers`, which lists the bazaar on Sign Ups → Overview. Clearing every slot and saving turns that off. Key: `bazaar-event-overview-client.tsx`, `bazaar-event-settings-client.tsx`, `bazaar-event-name-button.tsx`.

**Vendor org application intake (August 2026 / September 2026):** Customers apply at `/customer/apply/vendor` when **Vendor Hub** is subscribed (also listed under Profile → Applications). Fields are driven by `lib/vendor-hub/vendor-application-fields.ts` (toggle/add definitions to grow the form later). Staff share **Copy apply link** and review at **Vendor Hub → Network → Onboarding** (`/vendor-hub/network/onboarding`) — colorful status cards (Total / Pending / Approved / Rejected), status dropdown filter (no All/Pending/Approved/Rejected pill tabs), and one Submissions table. Columns: Applicant, Business Name, Business Type, Email, Submitted, Status (no Type/Actions). Rows open `/applications/[id]`. Approval grants the vendor role → Vendor Network. Apply is blocked only by an **approved or pending application** — a `vendor` contact role alone does not block re-apply (`hasApprovedOrgVendorApplication`). **One-time import cleanup (org-scoped script, not product default):** `node scripts/delete-imported-vendor-applications.mjs --org-id <uuid>` (dry-run) / `--execute` deletes that org’s approved vendor_hub vendor applications while keeping contacts, vendor roles, and participation history. Key: `components/customer/vendor-apply-client.tsx`, `vendor-application-form.tsx`, `app/(customer)/customer/apply/vendor/page.tsx`.
...
**Vendor Hub Overview (August 2026 / September 2026):** `/vendor-hub` uses the Fund Development dashboard layout. The staff header (Manaratee logo, organization, account) and breadcrumbs stay fixed on every Vendor Hub page. KPI cards: **Active events**, **Onboarding pending**, **Booth requests**, and **Booths still open**. **Action Required** lists onboarding, booth requests, unpaid booths, and open booths when those counts are above zero. **Active events** lists today and upcoming bazaars. **Quick Actions** are blue links (Create bazaar, Vendor Network, Vendor onboarding, Reports). Booths by type and recent orders stay on the bazaar workspace and Reports. Lifetime sales stay on Reports. Key: `vendor-hub-chrome.tsx`, `vendor-hub-dashboard-client.tsx`.

**Community Calendar (August 2026 / September 2026):** Shared top-level nav **Community Calendar** → `/community-calendar` (staff). Shown when the org has **Vendor Hub and/or Event Management**. Lists **Public** `internal_events` (`community_calendar_status`; SQL **`247`**, **`300`**, **`305`**). Published bazaars appear from their Vendor Hub date/place hold; staff bazaar cards open Vendor Hub (other events open Event Management). Visibility UI is **Private** / **Public** only (legacy `community_visible` still readable; saving as Public writes `published`). Featured block shows organization name, full date, day/time, and location with address; staff can **drag the featured flyer** to set crop (`flyer_focal_x` / `flyer_focal_y`, SQL **`249`**). Publish from event **Settings → General** → Community Calendar card, or bazaar create/edit. Legacy `/vendor-hub/community-calendar` redirects. **Public (no-login) browse:** `/o/[orgSlug]/community-calendar` — featured nearest upcoming + flyer, category circles from org **event types**, Browse tabs All / Today / This weekend, 4-column cards (flyer, name, day/time, location, Free or ticket prices). Ticketed Event Management cards → `/o/[orgSlug]/events/[eventId]` (view + buy tickets via join/sign-in). Bazaar cards on the public calendar are listings only (not ticket checkout). Copy public link from staff Community Calendar or Settings → Users. Key: `lib/community-calendar/*`, `public-community-calendar-view.tsx`, `community-featured-flyer.tsx`, `internal-event-community-calendar-card.tsx`.
...
**Add vendor to event (August 2026):** On the event Orders tab, **Add vendor** searches the **Vendor Network** only. If the vendor is missing, **Create new vendor** searches Contacts (or creates a contact), then creates the vendor profile + `vendor` role. After selection, enter business name, vendor type, optional booth, fee/payment, products/services, and notes for the event. Click a participating vendor row to **edit** booth/fee/type/notes for that event, or **remove** them with an optional fee refund (records `payment_type=refund`, cancels assignment/participant). Key: `components/vendor-hub/vendor-picker.tsx`, `components/vendor-hub/events/create-vendor-dialog.tsx`, `components/vendor-hub/events/add-event-vendor-dialog.tsx`, `components/vendor-hub/events/edit-event-vendor-dialog.tsx`, `lib/vendor-hub/vendor-search-actions.ts`, `lib/vendor-hub/add-event-vendor-actions.ts`.

**TicketOrders.csv Event Management import (August 2026):** Historical Eventbrite `TicketOrders.csv` → Event Management for MAS Dallas. Dry-run default; `--execute` writes `internal_events`, `event_ticket_types`, `ticket_orders`, `tickets`, and matches contacts by email (`find_or_create_contact_for_org`). Eventbrite repeats the order total on every ticket line — the importer groups by email + event + order date + amount. Skips vendor/bazaar booth events (already in Vendor Hub), QLH / Quran Institute / MAS Sunday School, `TRANSACTION_CHARGE` fees, and `DONATION` lines (those amounts are the same order total, not itemized gifts). Attaches **Annual Fundraising Dinner - Crystal Banquet** (Sep 12 2026) to the existing dinner; maps Kids Program / Babysitting onto the existing offerings. Historical events are `completed`, unpublished on Community Calendar, and have no facility reservation (external/online so they do not block rooms). Attendee name is the Eventbrite ticket holder; purchaser name/email/phone stay on the order contact (SQL **`286`** clears copied attendee emails). Idempotent via `metadata.importTag=TICKET_ORDERS_CSV_V1` + `importKey`. Script: `node scripts/import-ticket-orders-csv.mjs` / `--execute` / `--csv`. Reports: `scripts/reports/ticket-orders-import-dry-run.json`, `ticket-orders-import-execute.json`.

**Fundraising Dinner Sep 12 2026 door report (September 2026):** `September122026Fundraiser.csv` itemizes TICKET vs DONATION lines, check-in times, and child names for the Crystal Banquet dinner. Importer updates the 12 existing TicketOrders.csv orders on **Annual Fundraising Dinner**, inserts the remaining orders, applies door check-in, and stores checkout gifts on `ticket_orders.metadata.checkoutDonationCents` (not campaign ledger gifts). Event Overview / Finance / campaign Event tab show a **Ticket donations** KPI separate from Ticket revenue. Tag `FUNDRAISER_DINNER_SEP2026_V1`. Script: `node scripts/import-fundraiser-dinner-report.mjs` / `--execute`. Result: 114 orders, 246 tickets ($10,795 + fees on the original 12), $970 ticket donations, 190 checked in. Reports: `scripts/reports/fundraiser-dinner-sep2026-dry-run.json`, `fundraiser-dinner-sep2026-execute.json`.

**Fundraising Dinner Sep 12 2026 Square donations (September 2026):** `September12Donations.csv` (95 succeeded gifts, **$32,191**) imported onto campaign **Annual Fundraiser - September 2026**. Square donation reasons map to CRM giving groups; `Annual Fundraiser - September 12, 2026` is ungrouped campaign gifts ($5,700, including a $5,000 Syrian Forum USA sponsorship remark). Created **Tarbiya**, **Center**, and **CYP Usrat Al-Islah** giving groups (Tarbiya and Center linked to those departments). **CYP Usrat Al-Islah** is distinct from existing **CYP Usra**. Eight `campaign_groups` on the campaign. Payments write `campaign_id`, `campaign_group_id`, and `attributed_group_contact_id` so Campaign → Groups, Campaigns → Campaign Groups, and Donors → Group Giving all show the breakdown. Fund **Annual Fundraiser - September 2026** under General Donation. Square **Recurring Type** was originally stored only in the payment memo, so every gift looked one-time on Campaign → Donations. Repair `--link-recurring --execute` creates `recurring_donation_plans` for Square `MONTHLY` rows plus remark-only commitments such as Susan Almasri “50 monthly for one year” (3 plans / $180). Campaign Donations now has a **Type** column, a **Recurring** table (plans for this campaign; org-wide list remains Donations → Recurring), and pinned KPI cards for one-time total, recurring count, monthly recurring, one-time gift count, and donors. Ticket checkout donations ($970) stay on ticket orders, not this ledger. Tag `FUNDRAISER_DINNER_DONATIONS_SEP2026_V1`. Script: `node scripts/import-fundraiser-dinner-donations.mjs` / `--execute` / `--link-recurring --execute`. Reports: `scripts/reports/fundraiser-dinner-donations-sep2026-dry-run.json`, `fundraiser-dinner-donations-sep2026-execute.json`, `fundraiser-dinner-recurring-link-execute.json`. **Export CSV** on Campaign → Groups and Reports → Campaign Performance → Campaign Groups.
...
**MAS Fall Bazaar Oct 10 2026 Eventbrite booth orders (September 2026):** `FallBazaar101026.csv` (25 Eventbrite `TICKET` rows = booth purchases, not admission tickets) → new **MAS Fall Bazaar** on Oct 10, 2026 11:00 AM–7:00 PM. Vendor Hub bazaar with a date/place hold (`internal_events.source_module = vendor_hub`, not an Event Management workspace), 25 paid vendor participants, and 25 payments (**$4,525**; Stripe/PayPal). Booth inventory was remapped from Eventbrite ticket names onto **organization default booth types** (SQL **`303`**). **Mocktail / smoothie (truck or cart)** at **$175** is its own Settings type (SQL **`304`**); Iman Jassim’s Eventbrite mocktail order sits on MOCK-1, not Ice cream truck. The 16 Eventbrite “regular hall” purchases fill Regular table (12) then Open table (no clothing). Hot meal uses **Hot meal (Outdoor)**. Stage has 3 tables (1 assigned, 2 open). Tag `FALL_BAZAAR_101026_V1`. Script: `node scripts/import-fall-bazaar-101026.mjs` / `--execute`. Reports: `scripts/reports/fall-bazaar-101026-dry-run.json`, `fall-bazaar-101026-execute.json`. Bazaar: `/vendor-hub/events/1342449e-38ca-4882-9fb4-1ae4e6120472`.

**Bazaar table selection + MAS hall layout (September 2026):** Approved vendors pick a **numbered table** in My Bazaars. Charge is booth type **price + selection_fee** (SQL **`301`**, `reserve_vendor_booth`). Hall regular / blue (no clothing) / stage: **$150 + $10 = $160**. Corners T15/T25: **$150 + $25 = $175**. Lobby $300, coffee $300 (one slot, lobby or truck), **mocktail/smoothie $175** (truck or cart), outdoor trucks, and between-buildings spots use listed prices (general merch tables also +$10). SQL **`302`** + **`304`** seed **organization default booth types** (with `selection_fee` and `default_booth_numbers`) and rebuild template **Default bazaar layout** from those types. Booth Types settings can edit table extra / table numbers and **Save as default template**. Apply that template when creating a bazaar. Blue tables: attribute **No clothing / no racks**. Not a clickable map yet — the hall drawing can be the flyer; vendors choose T15 from the list. Checkout donation and Eventbrite-style guest pay link are still pending. Keys: `lib/vendor-hub/booth-pricing.ts`, `scripts/301_vendor_booth_table_selection.sql`, `scripts/302_default_booth_types_from_layout.sql`, `scripts/304_mocktail_smoothie_booth_type.sql`.
...
**Events list (August 2026 / September 2026):** `/vendor-hub/events` is titled **Events** (sidebar and breadcrumb match). One row of KPI cards: Events, Upcoming, Next, On the calendar. The page title and those cards stay fixed under the logo header while the table scrolls. The table columns are Event name, Date, Venue, Vendors. Event name is a primary-color link to the event workspace. Row actions: Edit (drawer), Copy event, Delete (confirm). Venue prefers linked Facilities venue name, else free-text location. Imported events set to **MAS Dallas Islamic Center** via `node scripts/set-bazaar-events-mas-dallas-venue.mjs --execute`.

**Vendor Network vendors table (August 2026 / September 2026):** `/vendor-hub/network/vendors` uses a vendor-focused layout (`vendorNetworkLayout` on `ContactsListView`). Columns, in order: **Business Name**, **Vendor Type**, **Primary Contact**, **Phone**, **Email**, **Last Activity**, **Status**. Phone and email are their own columns. Primary Contact is the name only, in regular weight, with no initials. Column header filters: Business Name, Vendor Type, Primary Contact (name), Phone, Email, and Status (top search box and All Status dropdown removed). **Last Activity** has a sort icon (Newest first / Oldest first; default newest). Default Status filter is **Active**. **Export CSV** uses the same column order. **Add Vendor** opens contact search → create contact if missing → vendor profile + `vendor` role (`CreateVendorDialog`). Pagination over filtered vendor-role contacts.

**Vendor Hub Participation History (August 2026):** `/vendor-hub/reports?tab=history` (Reports tab) shows one row per vendor: Business Name (link to vendor profile), **Primary Contact**, **Email**, **Phone** (`(###) ###-####`), Vendor Type, total Events, Last Event, Last Event Date, Last Amount Paid (latest non-refund payment). **Export CSV** downloads all rows (`vendor-participation-history-YYYY-MM-DD.csv`). Event counts include participation + payments. Sorted by last event date. Legacy `/vendor-hub/network/history` redirects here (`?contact=` preserved). Key: `lib/vendor-hub/participation-history-queries.ts`, `components/vendor-hub/network/participation-history-client.tsx`.
...
**Vendor Hub default vs event booth settings (August 2026):** `/vendor-hub/settings?tab=booths` is **organization defaults** (pricing UI, default booth types, attributes, templates) — no event picker. Default types include **table selection extra** and **default table numbers** (SQL **`302`**). **Save as default template** rebuilds **Default bazaar layout** from those types. Event workspace **Settings** tab at `/vendor-hub/events/[eventId]/settings` holds event booth pricing + booth types only (no attributes). Use **Copy from defaults** to clone org default booth types (and attribute links) onto the event. Defaults = `vendor_hub_booth_types` rows with `event_id IS NULL` + `organization_id`. Run **`scripts/234_vendor_hub_default_booth_types.sql`** then **`302_default_booth_types_from_layout.sql`**. Key: `BoothTypesSettingsPanel`, `lib/vendor-hub/default-booth-type-actions.ts`, `components/vendor-hub/events/bazaar-event-settings-client.tsx`.

**Vendor profile page (August 2026 / September 2026):** `/vendor-hub/network/vendors/[contactId]` — Vendor overview opens in **edit mode** with **Save** at the bottom-right of that card. Business name (contact name fallback), primary contact, email, phone, vendor type from Settings catalog (`applications.form_data.vendor_type_id`), social, products. Vendor Network **Vendor Type** is that catalog (Art & Photography, Hot Food, Ice Cream, and so on), not the booth. The column is a dropdown and saves immediately. Every vendor-type dropdown is the same width. The column filter uses checkboxes so several types can be selected together. **Apply** saves that selection and closes the list. **Export CSV** downloads the vendors matching those filters. September 2026: **Ice Cream** was added to the catalog (`scripts/306_ice_cream_vendor_type.sql`). Types for vendors already in the network were filled from the August bazaar export (newest order, main product). Food-truck booths are Food Truck. Savor Coffee Bar and Smart Drinks are Juice / Smoothies. People with no product answer were left as None. Header **Primary contact** name opens the Directory/CRM profile (`contactProfilePath`). Duplicate **Open CRM contact** button removed. Participation history and documents (fixed kinds: food license / insurance / sales tax / other). Storage bucket `application-documents` + `application_documents.document_kind` via **`scripts/230_vendor_profile_documents.sql`**. September 2026: vendor-type saves and this page were timing out because vendor row-level security scanned every contact. `scripts/307_vendor_hub_application_rls_perf.sql` switches those checks to `auth_user_contact_ids()`. Vendor Network title and tabs stay fixed under the logo header while the list scrolls. Vendors list: **row click** opens this profile in a dialog on the Vendors tab (`VendorProfileDialog`); closing it leaves the list, filters, and scroll where they were. The direct URL still opens the full page. Contact Financial: Vendor Hub in by-module chart + summary from `vendor_hub_payments`. Key: `lib/vendor-hub/vendor-profile-queries.ts`, `vendor-profile-actions.ts`, `components/vendor-hub/network/vendor-profile-client.tsx`, `vendor-profile-dialog.tsx`, `vendor-network-section-shell.tsx`, `contacts-list-view.tsx`.
...
**Directory module (August 2026):** User-facing **Contacts** sidebar is now **Directory**. Canonical table remains `contacts` (one person/organization record, many roles). Person/organization **Overview** does not show a Roles card; role badges remain on the profile header. Flyout: Overview, People, Families, Organizations, then populated role views, then Reports and Settings. Most role views appear only when the current tenant has records. **Service Providers** (plumbers, pest control, and other contractors the organization uses — not Vendor Hub event vendors) appears only when **Facilities** (`spaces`) is enabled, at `/directory/role/service-providers`. Legacy `/resources/service-providers` redirects there. Role views include lightweight lookup columns (department/position, lifetime giving, membership type, household/children, vendor type, rental history) without loading full operational records. Donor amount columns require `donations.view`. **Sponsor** is a manual role on the same contact (`scripts/269_directory_sponsor_role.sql`) — not a separate identity table. `/directory` Overview has global search, tinted People/Families/Organizations count cards, and color-coded Active Directory Categories. People / Organizations / Families moved out of Reports. Giving groups (`contact_type = group`) live only under Fund Development (Group Giving), not Directory. Campaign fundraising teams stay on Campaign → Groups. Reports are analytics (growth, overlapping role distribution, completeness, possible duplicates). Universal **+ Add** creates Person, Organization, or Family and can assign multiple roles. Membership **Add member** searches Directory first. Legacy `/contacts/...` list URLs redirect. Profile links use `/directory/[id]`. Key: `lib/directory/*`, `components/directory/*`, `components/layout/sidebar.tsx`. Optional SQL: `scripts/268_directory_module_label.sql`. Sponsor CHECK: `scripts/269_directory_sponsor_role.sql`.

**Directory People list (`ContactListRow is not defined`, August 2026):** `/directory/people` showed a red error box and an empty table because the client imported TypeScript types from `lib/contacts/contact-list-actions.ts` (`"use server"`). Next.js 16 treated `ContactListRow` as a missing runtime export. List types now live in `lib/contacts/contact-list-types.ts`; client components import types from that file and only import `fetchContactsList` / `fetchContactListStats` from the server-actions module. Directory reports use the same split: types in `lib/directory/directory-report-types.ts`, action in `directory-report-actions.ts` (a type re-export of `DirectoryRoleCountMap` from the actions file failed the Vercel/Turbopack production build).

**Giving groups moved out of Directory (August 2026):** CRM `contact_type = group` records are Fund Development only — they track donations attributed to a department, committee, or other collective (members’ individual gifts roll up). Directory no longer lists Groups; `/directory/groups` and `/directory/groups/[id]` redirect to **Donors → Group Giving** (`/donations/reports/donors?view=group`) and the giving workspace (`/donations/groups/[id]`). **Add Group** is on the Group Giving table section. Membership Groups stay at `/membership/groups`. Campaign Groups stay on Campaign → Groups.

**Donor Giving report columns (August 2026):** Individual, Household, and Group Giving tables on `/donations/reports/donors` no longer show **Pledge** or **Outstanding Balance**. Pledge status and balances stay on **Pledges**. Individual CSV/PDF export matches (no pledge columns). **Group Giving (September 2026):** table columns are Group, Primary Contact, Combined Total, Gifts, and Last Gift (Members / Group Gifts / Member Gifts removed). **Household Giving (September 2026):** lists only families with two or more adult Directory contacts who each donated in the period.

**Contacts list UI (June 2026):** Removed **All Contacts** (`/contacts` redirects to `/contacts/people`). Sidebar lists **People**, **Organizations**, **Reports**, and **Settings** (**Families** removed from the sidebar July 2026 — household directory lives under **Reports → Families**). User-facing **Affiliation** terminology replaced with **Roles** in Contacts → Settings automatic-role rules (contact profiles do not show a Roles section — role badges stay in the header and sync from activity only). Discount tags: **Employee / Staff / Member / Volunteer / Full-Time Employee** are system-managed (auto from Workforce or Membership; not pickable on profile). **Donor** and custom tags remain manually assignable on **individual and organization** Overview → Edit. Opening Edit does not auto-save the tag dropdown (empty Select events are ignored; `setPersonDiscountTag` returns an error instead of throwing). Sync preserves system tags when staff change a manual tag. Key: `lib/discount-tags/discount-tag-assignment.ts`. People/Organizations/Groups lists: search + add only (role/status dropdown filters removed); table columns **Contact** (name styled as link), **Email**, **Phone**, **Created by** (not stored yet — shows —), **Last modified**, **Status** (Active/Inactive only). No **Actions** column — edit, merge, delete, and **View Details** live on the contact profile sticky header actions menu. Removed intro banners and stat cards on type-specific list pages. Removed Teams column and team filter from `ContactsCrmList`. Team assignment remains on individual contact profiles where HR teams are enabled. **Organizations list (July 2026 / August 2026):** first column renamed **Organization** with column **sort** + **filter** (name); **Primary Contact** moved between organization name and Email; top search bar removed (use column filter); server-side sort via `fetchContactsList` (`sortBy`, `nameFilter`). **Roles** column filter + **Export CSV** export all matching rows (not just the current page), including address and primary contact. Filename `directory-organizations-YYYY-MM-DD.csv`. Same Export CSV control is on People. Reuses `fetchContactDirectoryExportAction`. **Groups list (July 2026):** same pattern — **Group** column with sort/filter, Primary Contact after name, no top search bar. **Individual name casing (July 2026):** ALL CAPS / all-lowercase individual contact names can be rewritten to proper case (`ABEER ZOUBI` → `Abeer Zoubi`) via `node scripts/proper-case-individual-contact-names.mjs` (dry-run; add `--execute` to apply). Organizations and groups are skipped. Create/update of individual contacts also applies the same rule. Helper: `lib/contacts/proper-case-name.ts`.

**Phone display (August 2026):** Tables and CSV/PDF exports show US 10-digit numbers (and numbers with a leading `1` / `+1`) as `(###) ###-####`. Stored values stay as entered. Shorter or non-US numbers are left unchanged. Helper: `lib/ui/format-phone.ts`; table cells use `components/ui/phone-text.tsx`. Form inputs are not auto-formatted.
...
**Contacts Groups record type (June 2026 / July 2026 / August 2026):** CRM `contact_type = group` remains for giving attribution but **is not a contact profile and is not listed in Directory**. Detail UI is **`/donations/groups/[id]`** for Group Donation / Membership Group collectives. When the collective is a **Department** (linked via `linked_department_id`, or unique same-name match — auto-linked on open), Group Giving redirects to the shared **Department workspace** at **`/workforce/departments/[id]`**. **Department-level tabs:** **Overview** (KPI snapshot + **active** programs list; closed/paused years stay on **View all programs** / the Programs tab; click a year → `/programs/[id]`), **Programs** (catalog including closed years), **Events** (`?tab=activity`), **Group giving** when linked, **Financial**, **Settings**. **Year-level tabs** (require `?year=`): **Overview** (live report), **Offerings** (`?tab=programs`), **Registrations** (`?tab=students`), **Settings** (`?tab=settings&year=` — configure name/dates/flyer). Breadcrumbs: Departments → department → Programs → year name. Year-only tabs without `?year=` redirect to Overview. Legacy aliases unchanged (`offerings` → overview, finance/students remaps). Financial assistance stays at org/committee level. Apply **`scripts/167_giving_group_category.sql`**. Key files: `department-group-workspace-client.tsx`, `department-overview-panel.tsx`, `donation-group-path.ts` (`isDepartmentYearWorkspaceTab`).

**Programs Catalog offerings (July 2026 / August 2026):** `/programs/catalog` (**Offerings** on Programs Home) and customer **`/customer/programs`** show **active** `program_offerings` (not year/season rows). Cards: **program name on top** (bold), **offering name** as blue link; flyer falls back to parent program flyer; family filters: **Gender** (All / Male / Female), **Audience** (All / Youth / Adult; **Age** only when Youth). **Public (no-login) catalog:** `/o/[orgSlug]/programs` — same cards/filters; only programs with `visibility = public`; offering click → `/join/[orgSlug]?next=/customer/programs/...` to register. Staff copy link on Offerings + Settings → Links. Branding edited on offering Overview / program Configure. Apply **`scripts/191_offering_catalog_branding.sql`**. Key files: `offering-catalog-view.tsx`, `offering-catalog-queries.ts`, `public-offering-catalog-queries.ts`, `program-catalog-filters.tsx`, `app/o/[orgSlug]/programs/page.tsx`.

**Department Head portal access (July 2026 / August 2026):** Department Heads (`staff.is_department_head` + `department_id`, set on CRM Employment) get **Staff Tools** even without org-wide `staff.view`. Staff Tools shows **My department** → department workspace. Admin sidebar injects **My department** when the user is a head but lacks `staff.view` (full Workforce roster stays admin-only). Do **not** grant org-wide `programs.view` / `programs.manage` / `staff.view` just to make a head work — that would show every department. Application evaluation requires `canManageDepartment` (or `programs.manage` for orphan years). Key files: `lib/departments/department-headship.ts`, `lib/auth/staff-tools-eligibility.ts`, `app/(customer)/customer/staff/page.tsx`, `components/layout/sidebar.tsx`.

**How to set up a department head (August 2026 / September 2026):** 1) Directory person (existing or create). 2) Add them as an employee on that department. 3) On Employment details, check **Department Head (Director)** and **Save changes**. 4) Type their **work email** on that same sheet (personal email stays on the contact). If the mailbox is not in Settings → Users yet, an invitation is sent and assigned to this person. They should **log in with the work email** when one is assigned. SQL **`186`** is already applied; no new org role is required.

**Program Lead (September 2026):** One person per year/season who can open **that** program workspace and see all of its offerings, without org-wide `programs.view`. This is narrower than Department Head (whole department) and wider than offering **Coordinator** / Primary instructor (one or more classes in **My Classes**). Assign them on Program Workspace → Settings → General → **Program Lead** (department employees). Their contact profile shows a **Program workspace** card. Staff Tools and the admin sidebar show **My program** when they lack `programs.view` and are not already using **My department**. Org-wide `/programs` lists send them to their first led program (`?tab=offerings`). Do **not** invent a new job role for camp group leads who cover two offerings — assign Coordinator (or Primary instructor) on each class. Run **`scripts/290_program_lead_contact.sql`**. Key files: `lib/programs/program-leadship.ts`, `lib/programs/program-lead-actions.ts`, `lib/programs/program-access.ts`, `components/programs/program-lead-settings-card.tsx`, `components/contacts/contact-program-workspace-panel.tsx`.

**How to set up a program lead (September 2026):** 1) Add them as an employee of the department that owns the year/season. 2) Open that program → Settings → General. 3) Choose **Lead name** and click **Save** at the bottom (same button as name, dates, and publishing). 4) They should log in with their work email when one is assigned. No extra org role is required.

**Department Financial (August 2026 / September 2026):** **Financial** lives on the **department** workspace (not year/program). The **Reports** tab was removed — it repeated Financial. Leftover `?tab=reports` opens Financial. **Employees** is a Financial sub-tab (`?tab=financial&section=employees`; staff roster; **Add employee** searches Directory and can create a contact if they are missing; click a row → side-sheet **employee profile** in edit mode — position, job role, **Department Head (Director)**, department, hire date, staff type/status, pay basis/rate, personal email, typed work email, phone, recent pay periods & hour logs, link to contact profile; Remove blocked when department pay entries or hour logs exist). Pay basis is **Hourly**, **Monthly salary**, or **Unpaid (volunteer)** — unpaid keeps saved rates so you can switch someone back to paid later, and payroll skips them. The roster defaults to **Active** employees **assigned to a class** this year (department head stays visible); switch Assignment to **All employees** to see unassigned staff such as a teacher with no offering. Status Inactive is still manual — do not auto-inactivate unassigned teachers. Leftover `?tab=employees` opens Financial → Employees. **Schedule** moved to **Program Workspace** (`/programs/[id]?tab=schedule`): **Class times** (weekly `program_schedule_items` + sessions for that program; default **Week Board** of offering cards with instructor/space, plus **List** table; **Space** from `venue_id` / venue name) | **Activity planner** (`schedule_activities` Schedule Builder, locked to the open program; `?section=activity-planner`). Leftover department `?tab=schedule` opens the Programs doorway so staff pick a year. Schedule header CTAs: **Check space availability** → Facilities; **View Master Calendar** → Events. Operating **Financial** layout: one sticky combined KPI row under the main **Financial** tab (Employees, Revenue, Expenses, Profit — calculated from open-year payments, approved payroll, and operating costs; click a card to jump to that sub-tab), then sub-tabs **Employees | Payroll | Expenses | Financial Summary**, then the active sub-tab content (Payroll: Log hours / Create pay period; Finance → Payroll queue for Mark paid; no local KPI cards). Org **Finance → Payroll** still shows its own queue KPIs. Scoped to **open** years only (`draft`/`active`/`paused`). URLs: `/programs/[id]?tab=schedule`, `/programs/[id]?tab=schedule&section=activity-planner`, `?tab=financial` (default Payroll), `?tab=financial&section=employees|payroll|expenses|budget`. Legacy `?year=&tab=schedule` opens that program’s Schedule tab. Key files: `department-group-workspace-client.tsx`, `department-finance-kpi-row.tsx`, `lib/departments/department-finance-kpis.ts`, `program-workspace-client.tsx`, `department-schedule-panel.tsx`, `department-payroll-panel.tsx` (`variant=roster|periods`), `department-employee-profile-sheet.tsx`, `lib/departments/staff-pay-basis.ts`, `finance-payroll-queue-panel.tsx`. Run **`scripts/293_staff_pay_basis_unpaid.sql`**.

**Department Registrations tab (July 2026 / August 2026):** Application & Approval programs have a top-level **Applications** tab (`?tab=applications`) after Offerings; **Registrations** (`?tab=students`) is the enrollment roster only (no Applications | Enrollments sub-tabs). **Applications** KPIs: Pending (submitted + evaluation statuses), Approved — Registration Pending. Status chips: All / Pending / Approved / Waitlisted / Declined / Withdrawn. **Direct Registration** hides the Applications tab; leftover `?tab=applications` and `?tab=students&section=applications|review|approved` open Registrations. Application-based Registrations KPIs: Enrolled / Waitlisted / Cancelled. Direct Registration KPIs: Enrolled / Pending Checkout / Waitlisted / Cancelled. The roster is operational (participant, offering name with a Change tag to move, teacher, Status, optional Parent / Guardian, registered date) — amounts, paid, balance, and payment status live on **Finance → Payment Summary**. Adult rows put email/phone under Participant (contact link); youth rows show the child name only and Parent / Guardian with the registrant’s contact/email/phone. Empty demographic cells show **—**. Registrations KPIs are clickable and filter the roster (`?status=enrolled|waitlisted|cancelled|pending|all` and `?offering=`). Default columns: Participant, Offering, Teacher, Status, Parent / Guardian, Registered, Actions. Optional columns (email, phone, DOB, age, gender, allergies, photo consent, program) use **Columns** (browser localStorage). Toolbar: Search, Offering, Teacher, Status, Filters, Columns, **Export CSV** (current view vs full registration data), Clear filters. Status defaults to **Active**. The roster table is paginated (default 20 per page, same `ListPagination` as Applications). Approved is an application status chip, not a separate tab. Application status and enrollment status are independent — unpaid **Active** enrollments stay on the roster. Customer **Register** on Application & Approval programs requires an unused approved application for that offering; unapproved families are sent to Apply. **Copy registration link** includes the approved offering. Summer Camp 2026 is Direct Registration (SQL **`281`**); QIL stays Application & Approval. Run **`scripts/280_program_enrollment_process.sql`**. Key files: `lib/programs/enrollment-process.ts`, `department-students-panel.tsx`, `department-applications-panel.tsx`, `department-participants-panel.tsx`, `program-defaults-settings-panel.tsx`, `lib/programs/program-registration-actions.ts`. Applications table: **Program** column shows offering name only; column header filters for Participant, Program, New / Returning. **Pending** is one queue (`submitted` plus evaluation statuses) — there is no separate Needs Review tab or KPI. **Click a row** to open the application form **dialog**. Batch Approve / Decline / Withdraw appears only after rows are selected. **Last Updated** shows staff who approved/declined (`evaluated_by` + `evaluated_at`) when present, otherwise last staff save (`updated_by_user_id` + `updated_at`) — not the applicant submit time (run **`scripts/237_program_application_updated_by.sql`**). **Un-approve** returns an approved-but-not-registered application to Pending (`submitted`). **Withdraw** (row, dialog, or batch) marks submitted, evaluation, waitlisted, or approved-not-registered applications as `withdrawn` so they leave Pending and Approved — Registration pending; they cannot register with that application. Already-registered students withdraw from **Registrations**. Answers stored in `program_applications.application_answers` (run **`scripts/236_program_application_answers.sql`**; includes `requested_offering_ids` for multi-course). Enrolled roster rows change offering with **Change** next to the offering name (dropdown only while changing; same year/season; payments stay). Cancelled rows keep a link to the offering. Overview application attention links go to **Applications**. Key files: `department-students-panel.tsx`, `department-applications-panel.tsx`, `department-application-detail-sheet.tsx`, `program-application-form-fields.tsx`, `department-participants-panel.tsx`, `program-workspace-client.tsx`.

**Department Programs doorway (August 2026 / September 2026):** Department workspace **Programs** tab lists that department’s years/seasons including closed (name — status — offering count — enrollment count). **Series KPI cards** group years by program series (name minus the year, plus aliases so Summer Camp 1/2, Fall October/November, Spring Week 1/2, and Winter Break / Ready Set Pray share one card; Education still groups Sunday School, QLH, SQA, and so on). Each card shows **active** and **closed** year counts only (draft/paused omitted). A card appears only when the series has **two or more** active/closed years (one-off camps like RIJAAL or Special Needs stay in All years, not as cards). Cards stretch across the row with season/program colors and icons (waves for Summer, leaf for Fall, snowflake for Winter, flower for Spring). Below them the tab is a **two-column** layout: years list on the left, enrollment-over-years chart on the right. Click a card to filter the list and fill the chart; click again or **Show all programs** to clear. Short labels live in `seriesShortLabelFromRaw` (`lib/programs/year-comparison.ts`). Department **Overview** lists **active** programs only; **View all programs** opens this tab. Clicking a row opens `/programs/[id]` in the Programs module. Leftover `?year=` department URLs redirect there. Offering manage is `/programs/[id]/offerings/[offeringId]`; department-scoped offering URLs redirect into Programs. Key files: `department-programs-doorway-panel.tsx`, `program-workspace-client.tsx`, `lib/programs/program-workspace-path.ts`, `lib/programs/year-comparison.ts`.

**Department Years/Seasons = Programs Catalog (July 2026 / August 2026):** Program **management** lives in the Programs module (`/programs/[id]`). The department **Programs** tab is a doorway summary only. Sellable **Offerings** for a selected program live on that program’s **Offerings** tab. `/programs/[id]` with a `department_id` is the year workspace (no redirect back to Administration). Orphan programs (no department) keep standalone detail. Staff UI: Program = `programs` row; Offering = `program_offerings` row (`lib/programs/program-display-labels.ts`).
...
**Department Overview + Programs tab (July 2026 / August 2026 / September 2026):** Department-level tabs: **Overview | Programs | Events | Group giving | Financial | Settings**. **Overview** is a department snapshot: one KPI row for **active** programs, **active** offerings (not cancelled), employees, students (unique enrolled people on those offerings), and upcoming events. Under the cards, Overview is a **two-column** layout: **active** programs list on the left and **Enrollment over time** on the right (same split as the Programs tab). Collected / payroll / net live on the **program** Overview, not here. Cards link to Programs, Financial → Employees, or Events. **Description** and **Terms and Conditions** (rich text `departments.terms_html` + optional PDF `departments.terms_pdf_url`; run **`scripts/241_department_terms.sql`**; PDF uploads reuse `program-flyers` via `uploadDepartmentTermsPdf`) live on **Settings**. Settings has name and color on one row, **Director Name** (picks a department employee as `staff.is_department_head`), then Description and Terms. One **Save** at the bottom. **Delete department** is also at the bottom and is blocked when any programs, offerings, or employees exist. Program/offering, event, and bazaar flyers still use `FlyerThumbnail` / `ProgramFlyerField`. **Programs** = summary doorway with series KPI cards (active/closed counts; click filters the list and shows enrollment over years) then the year list (name — status — offerings — enrollments); click a year opens the Programs module workspace (`/programs/[id]`). Programs also **auto-close** after `end_date`. Closed programs remain in the doorway list. Operating lists include `closed` via `DEPARTMENT_WORKSPACE_PROGRAM_STATUSES`; **Financial** still uses open years only for live ops. Run **`scripts/199_program_status_closed.sql`**. Key files: `department-overview-panel.tsx`, `flyer-thumbnail.tsx`, `program-flyer-field.tsx`, `department-programs-doorway-panel.tsx`, `department-terms-actions.ts`, `rich-text-editor.tsx`, `lib/ui/rich-text.ts`, `department-group-workspace-client.tsx`, `department-year-actions.ts`, `department-year-auto-close.ts`, `department-active-programs.ts`.

**Program Overview (August 2026):** Program workspace tabs (in the Programs module at `/programs/[id]`): **Overview | Offerings | Applications (Application & Approval only) | Registrations | Schedule | Finance | Reports | Settings**. Header shows the program name with **Academic/Seasonal** and **status** badges (Draft / Active / Paused / Closed / Archived); the department name is a blue link under the title to `/workforce/departments/[id]`. **Overview** is a compact live dashboard: **Program health** KPIs (Enrollments / Offerings / Need review / Outstanding / Collected), **Needs attention** (applications, outstanding balances, capacity — not cancelled enrollments; refunds stay on Financial summary), **Financial summary** (charges, discounts, FA, received, refunds, outstanding), and **Recent activity** (registrations, payments, applications). The offerings table lives on the **Offerings** tab, not Overview. Application attention links go to **Applications**; enrollment links go to **Registrations**; finance links go to **Finance**. **Schedule** is class times + activity planner for that program. Class times default to a visual **Week Board** (day columns + offering cards; missing teacher/room called out; cards open the offering). **List** is the administrative table. Recurring times still belong to each offering — this tab is an aggregate view. **Finance** (`?tab=finance`) has **Transactions** (this program’s payment transactions; no donations, no program picker), **Payment Summary** (`?section=payment-summary`; family balances), and **Add-ons** (`?section=addons`; purchased extras). Leftover `?tab=reports&section=tuition-plans|payment-summary` opens Finance → Payment Summary. Leftover `?tab=reports&section=addons|add-ons` opens Finance → Add-ons. Org-wide Finance remains `/finance/transactions`; payroll stays on department **Financial**. **Reports** (`?tab=reports`) is analytics for this program: **Overview** (enrollment summary + by offering; counts drill into Registrations), **Trends**, **Attendance**. Leftover `?section=waitlist` opens Overview. Waitlisted people are managed from Registrations (`status=waitlisted`). Childcare lives on **Event Management → Reports**. Org-wide program reports remain `/programs/reports/...` (including the org Enrollments and Waitlist tables). **Settings** sub-tabs: **General** (program details, enrollment defaults, and unique registration policy — dates and eligibility appear once) | **Notifications** | **Promo Codes**. Registration payment/form/cancellation fields and Notifications still save to `department_program_settings` (department-scoped stubs). Leftover `?section=defaults|registration|year-defaults` open General. Leftover department `?tab=settings&section=registration|notifications|promo-codes|year-defaults` opens the Programs doorway. Leftover `?section=service-needs` opens department **Events**. Department **Settings** is name/color, Description, and Terms (one **Save** at the bottom). **Delete department** is blocked unless the department has no programs, offerings, or employees. Department **Financial** (Payroll / Expenses / Financial Summary) stays on the department workspace. Key files: `program-workspace-client.tsx`, `program-workspace-finance-panel.tsx`, `program-workspace-reports-panel.tsx`, `program-workspace-settings-panel.tsx`, `program-policy-settings-panel.tsx`, `department-program-dashboard-panel.tsx`, `lib/programs/program-overview-metrics.ts`, `lib/programs/program-workspace-path.ts`, `department-program-overview-panel.tsx`, `department-schedule-panel.tsx`, `components/programs/schedule/*`, `lib/programs/weekly-schedule-board.ts`, `lib/programs/offering-schedule-color.ts`, `program-basics-section.tsx`.

**Program Schedule Week Board (August 2026):** Program Workspace **Schedule → Class times** defaults to a card-based weekly timetable (Mon–Fri, plus Sat/Sun when scheduled). Each card shows time, offering name, instructor, and room; if a class time has no instructor of its own, the offering **primary instructor** is shown. “Teacher not assigned” / “Room not assigned” appear only when that value is still missing. Cards use a stable pastel accent per offering ID and open `/programs/[id]/offerings/[offeringId]`. **List** keeps the administrative table (Day, Time, Offering, Instructor, Space). Offering Schedule remains the source of truth — no duplicate schedule records. View choice persists in session storage. Key files: `department-schedule-panel.tsx`, `components/programs/schedule/weekly-schedule-board.tsx`, `schedule-class-card.tsx`, `lib/programs/weekly-schedule-board.ts`, `lib/programs/offering-schedule-color.ts`, `lib/departments/department-schedule.ts`.

**Program Registrations vs Reports (August 2026):** Program Workspace **Registrations** is the single operational list (who is registered). Clickable Enrolled / Waitlisted / Cancelled cards filter the table; **Reports** no longer duplicates that roster. Movable rows show the offering name plus a **Change** tag — the drop-down appears only when changing. Workspace Reports is **Overview | Trends | Year comparison | Attendance**: enrollment summary, enrollment by offering (capacity/available only when limited), monthly registration counts, year-to-year participants/families (new vs returning), and existing attendance. Offering enrolled/waitlisted/cancelled numbers open Registrations with those filters. Org-wide `/programs/reports/enrollments` and `/programs/reports?tab=waitlist` are unchanged. Key files: `department-students-panel.tsx`, `department-participants-panel.tsx`, `program-workspace-reports-panel.tsx`, `program-workspace-enrollment-reports.tsx`, `program-year-comparison-panel.tsx`, `lib/programs/registration-table-columns.ts`, `lib/programs/program-enrollment-summary.ts`, `lib/programs/program-enrollment-summary-queries.ts`, `lib/programs/program-workspace-path.ts`.

**MUHSEN Sunday School 2026-2027 (September 2026):** Separate Education academic program (not mixed into Sunday School 2026-2027). Registration + payments: offering **Ages 5-12** (capacity 5), Sundays 9:30 AM–1:00 PM Youth Lounge, lead Aliya Alison Lee. One-time Tuition fee plan is $0 until staff set the amount (not on the facility request form). Script: `node scripts/import-muhsen-sabiqoon-men-in-the-making.mjs` / `--execute`.

**Sunday School 2026–2027 import (August 2026):** Workbook `Sunday_School_Master_Registration_Report.xlsx` (**Master Registration Report** + Rosters + Registrations + Payments) → Education → year **`Sunday School 2026-2027`** → offerings **Age 4-6**, **Age 7-9**, **Age 10-14**. Parent identity and fees/discounts/due come from Master (**Financial Row = Yes**). **Money received** (amounts + dates) comes from the **Payments** sheet: `REGISTRATION_AMOUNT` → tuition schedules on the family host charge. `ADDONS_AMOUNT` (legacy $5 transaction fee) is **not** imported as an add-on; already-imported transaction-fee charges were removed with **`scripts/289_remove_program_transaction_fee_addons.sql`**. Master “Registration Paid” is not used for timeline amounts. Fee: **$120**/student with **5% sibling discount** ($114 for 2nd+); staff families **50% staff discount**; Habiba Hassan **3% full-payment** on $234 list, paid $240 (extra $6) — do not treat her $2,042.82 subscription payment as tuition. Skip Zachie/Ihab Neel and withdrawn/cancelled (Walaa Hatamleh, Rosemary Admiral). Monthly rows → 9 `program_payment_plans`. Idempotent tag `SUNDAY_SCHOOL_2026_27_V2`. Script: `node scripts/import-sunday-school-2026-2027.mjs` / `--execute`. Repair already-imported payments: `node scripts/repair-sunday-school-payments-from-sheet.mjs` / `--execute`. Report: `scripts/reports/sunday-school-2026-2027-*.json`.
...
**QIL 2026–2027 payments + registrations (August 2026):** Member-portal export `QIPayments.csv` (63 rows, \$8,525 succeeded / \$825 refunded, **\$7,700 net**) → year **`Quran Institute for Ladies 2026-2027`**: **56 enrolled** + **3 cancelled** (full refunds for reporting). Paid students leave **Approved** (`application.enrollment_id` set; 59 applications linked). CSV **Registered Members** spelling is canonical (contacts renamed, e.g. Imene Latreche, Abeer Zoubi, Malak Ahmed Hmimy). Fully refunded: Heba El Manawy (Memorization Baqara), Madihah Ahmed, Anhar Ahmed (payer Hanan Ahmed). Family payers: Sarfraz Amin → Israa; Rowaida → Malak Ahmed Hmimy (\$900 split). Staff/FA coupons from remarks (`STAFF-QIL2627-50`, `FA-QIL2627-100`). If a student is approved for more than one Tajweed/Recitation class, payment is applied to Recitation Improvement first. Run **`scripts/277_program_charge_schedule_refunded_status.sql`**. Tag `QIL_2026_27_PAYMENTS_V1`. Script: `node scripts/import-qil-payments-2026-2027.mjs` / `--execute` (`--csv` optional). Report: `scripts/reports/qil-2026-2027-payments-*.json`.

**QIL 2026–2027 final roster + payments (September 2026):** `QI-FinalList.xlsx` (Teacher / Course / English Name / Phone / Email) is the canonical enrolled roster for year **`QIL 2026-2027`**. `QI-Payments0915.csv` (67 rows, \$9,025 succeeded / \$975 refunded, **\$8,050 net**) overlays tuition. **112 enrolled seats** / **96 students**. Names not on the list were cancelled (Ayah Zahra, Belqes Ali, Sahar Shqaidef). Extra Ajurrumiyyah seats off the list were cancelled (Marwa, Meriem). Sadia Shakeel is on the list without a course — her Recitation seat was kept. **Israa Alaomairi** is the Preparing for Ijaza student; **Sarfraz Amin** is her husband/payer (Excel listed him). **Noor Abushaaban** and **NoorElaine Abushaaban** are the same person — kept **NoorElaine** on Recitation (\$400) and Al-Ajurrumiyyah. **Rajaa Aljaber’s 7 Recitation Improvement students** (Beyza, Leah, Ozlem, Robina, Wafa, Hanan, Doaa) were first placed on Amneh Ismail’s Recitation class because Excel “Rajaa Aljaber” matched a Directory contact of that name instead of staff **Rajaa Eljaber**; they now sit on **Recitation Improvement (Non-Arabic Speaking)** with Rajaa. The importer resolves teachers from active primary-instructor contacts first. Unpaid students on the list are enrolled with fee-plan balances. Tag `QIL_2026_27_FINAL_V1`. Script: `node scripts/import-qil-final-2026-2027.mjs` / `--execute` (`--file` / `--csv` optional). Report: `scripts/reports/qil-2026-2027-final-*.json`.

**QIL historical payments (August 2026):** `New_PAYMENT_TRANSACTION_REPORT.csv` → closed years **QIL 2022-2023** / **2023-2024** / **2024-2025** (offerings from CSV course names; enrollments + `program_charges` from Stripe rows grouped by Transaction ID). **QIL 2025-2026** left unchanged. **QIL 2026-2027** only new Stripe IDs matched to existing offerings (no new classes): Suzan Qawouq \$450 + \$50 refund on Recitation Improvement. Skipped June/July 2026 Recitation Improvement intensives, Ijaza Lesson, Junior, Taekwondo, donations/tickets, and `processing` rows. Tag `QIL_HISTORICAL_PAYMENTS_V1`. Script: `node scripts/import-qil-historical-payments.mjs` / `--execute`. Report: `scripts/reports/qil-historical-payments-*.json`. **QIL 2024-2025 gap fill (August 2026):** `QIL24-25.csv` added **29** missing enrollments (no changes to students already on the year; no new offerings). Year is now **82 enrolled** + **7 cancelled**. Tag `QIL_2024_25_GAP_V1`. Script: `node scripts/import-qil-2024-2025-gap.mjs` / `--execute`. Report: `scripts/reports/qil-2024-2025-gap-*.json`.

**QIL 2026–2027 registration window (August 2026):** Year program **`QIL 2026-2027`** and all **18 offerings** use `enrollment_close_date` **2026-12-31** (most offerings had Aug 16/17 and showed **Registration closed** because they do not inherit program dates). Script: `scripts/279_qil_2026_2027_enrollment_close.sql`.

**QIL 2026–2027 approved roster (August 2026):** Finalized workbook `QIApproved.xlsx` (Teacher / Offering / Student) → year **`Quran Institute for Ladies 2026-2027`** as **`program_applications` status=`approved`** (Registrations → **Approved**). Matches existing English contacts even when spelling differs; creates missing student contacts; adds missing teachers (e.g. **Asmaa Salim**); creates offerings **Al Nouraniyyeh**, **Tajweed Level 1/2 (Non-Arabic Speaking)**; renames **Memorization 2** → **Memorization (Yusif - Annahl)**. Tag `QIL_2026_27_APPROVED_V1`. Script: `node scripts/import-qil-approved-2026-2027.mjs` / `--execute` (`--file` optional). Report: `scripts/reports/qil-2026-2027-approved-*.json`. **Prune (August 2026):** Approved-but-not-registered applications whose student is on neither `QIApproved.xlsx` nor `QIPayments.csv` → `not_approved` (15 applications / 13 students). Enrolled rows are left alone. Tag `QIL_2026_27_PRUNE_APPROVED_V1`. Script: `node scripts/prune-qil-approved-2026-2027.mjs` / `--execute`. Report: `scripts/reports/qil-2026-2027-prune-approved-*.json`. **Al-Ajurrumiyyah is free:** remaining approved students for that offering were enrolled ($0 paid charges). Tag `QIL_2026_27_AJURRUMIYYAH_FREE_V1`. Script: `node scripts/enroll-qil-ajurrumiyyah-2026-2027.mjs` / `--execute`. **Partials parked on Recitation Improvement (August 2026):** Al-Ajurrumiyyah has no tuition. Meriem Douma ($225/$25) and Narmeen Alfahal ($450/$50) were moved from Tajweed (Beginner) / Tajweed (Advanced) onto Recitation Improvement so those balances show under that offering until the class name is corrected; their $0 Ajurrumiyyah seats stay. SQL **`scripts/282_qil_ajurrumiyyah_partials_to_recitation.sql`**.
...
**Education historical enrollments import (August 2026):** Cleaned workbook `EducationPrograms.xlsx` (member rows) + original `EduPrograms2.csv` (parent email/phone) → **Education** year programs plus a new **Istiqamah Institute** department. One program per product per year with a single default offering named like the program. **People + enrollments only** (no charges). Skipped already-loaded years (`QLH 2024-2025`, `QLH 2025-2026`, `Sunday School 2026-2027`), cancelled rows, fall camps, and empty `Sunday School 25-26` CSV rows. Kept the QLH 2023/2024 split (61 in 2022-23, 53 in 2023-24). Parents matched by CSV email then phone via `find_or_create_contact_for_org`; minors as `people` under the parent. Idempotent tag `EDU_HISTORICAL_V1`. Script: `node scripts/import-edu-historical-enrollments.mjs` / `--execute` (`--xlsx` / `--csv` optional). Report: `scripts/reports/import-edu-historical-*.json`. Department / Programs list cards now use exact per-program enrollment counts so totals stay accurate past the API’s 1,000-row select cap.
...
**Program kinds (July 2026 / August 2026):** Two product modes — **`academic`** (year + offerings) and **`seasonal`** (camp/season; may have multiple offerings for age/gender bands). Column `programs.program_kind` (SQL **`193`**). Programs Home lists both together with an Academic/Seasonal **tag**; org packaging still gates **create**. **Org packaging (Aug 2026):** `organizations.program_kinds` = `academic` | `seasonal` | `both` (default both; SQL **`246`**). **Phase 6:** editable on Platform Admin → Organizations → Modules under **Programs** (Academic and Seasonal toggles; one or both) via `PATCH /api/platform/organizations/[id]/program-kinds`. Tenant **Billing** no longer includes a Program modes card (September 2026). Create UI and APIs hide/reject modes outside the entitlement. **Policy (hard validation):** academic = full-program registration + monthly tuition allowed; seasonal = session/day-pass packages, no monthly academic tuition — `lib/programs/program-kind-policy.ts`. **Phase 2 (Aug 2026 / September 2026):** Kind terminology via `getHierarchyLabels` / `getReportHierarchyLabels` (Year↔Season; offerings are **Offerings** for both kinds); department catalog **Add Year** / **Add Season** entry CTAs; year workspace tab labels follow selected program kind; create can lock kind with `?kind=`; Registrations + Payment Summary reports add **Type** (Academic/Seasonal) filter with matching labels. **Phase 3 (Aug 2026):** Same Type filter + kind-aware labels on Enrollments, Add-ons, and Transactions (`org-reports-client.tsx`; Academic/Seasonal selection hides donation rows). **Phase 4 (Aug 2026):** Report Type presets sync to URL `?kind=` (`hooks/use-program-kind-report-preset.ts`); report nav keeps the preset across tabs; Attendance + Waitlist include Type filter. **Phase 5 (Aug 2026):** Known-kind staff UI (create/edit form, year configure/defaults, offerings section, detail header, registration detail, year breadcrumb) uses `getHierarchyLabels`. Seasonal create makes one leaf offering by default; staff can add more offerings. Key: `program-kind.ts`, `program-kind-policy.ts`, `program-display-labels.ts`, `program-kind-report-preset.ts`, `organization-program-kinds.ts`, `department-programs-catalog-panel.tsx`, `program-form.tsx`, `programs-registrations-table.tsx`.

**Department-home program navigation (July 2026):** Department-linked years/seasons open manage under **`/workforce/departments/[id]/programs/[programId]/offerings/[offeringId]`** so the sidebar stays on **Programs/ Events → Departments**. Legacy `/programs/.../offerings/...` redirects when `department_id` is set. Helper: `programOfferingManageHref(..., { departmentId })`. Offering manage lives in the Programs module. Breadcrumbs: Department → Programs tab → offering. Department **Add program** includes Academic vs Seasonal type; Seasonal skips inherit defaults and creates a new camp product (no Tajweed placeholder).
...
**Programs → Reports tabs (July 2026 / August 2026 / September 2026):** **Reports** is a Programs Home destination (not a sidebar flyout). Opens `/programs/reports/enrollments`. Shared report tab bar (`ProgramsReportsNav`): **Enrollments** | **Year comparison** | **Camp enrollment** | **Add-ons** | **Payment Summary** | **Waitlist** | **Attendance**. On a **program workspace**, Add-ons lives under **Finance** (not Reports). **Childcare** moved to **Event Management → Reports** (`/event-management/reports/childcare`). Registration, Transactions, and Payroll moved to Programs Home cards (**Registrations**, **Finance**). Bare `/programs/reports` redirects to Enrollments. Legacy `?tab=enrollment` redirects to Registrations; `?tab=transactions` → Transactions; `?tab=childcare` → Event Management Childcare; `?tab=enrollments` → Enrollments; `?tab=year-comparison` / `?tab=growth` → Year comparison; `?tab=camp-enrollment` / `?tab=camps` → Camp enrollment; `?tab=tuition-plans` / `?tab=payment-summary` → Payment Summary; `?tab=addons` / `?tab=add-ons` → Add-ons. Leftover `/programs/reports/childcare` redirects to Event Management Reports. **`/programs`** is Programs Home (Overview), not a catalog redirect.
...
**Programs → Reports → Year comparison (August 2026):** Year-to-year enrollment growth by **program series** (e.g. QLH 2022–23 through 2026–27 grouped as Quran for Little Hearts) and by **department**. Counts unique **participants** and registrant families on enrolled/active seats. **Returning** = in the previous year of the same series (or any program in the department when Program is All). Also shows new / dropped families, participants per family, and % change. Line chart of enrolled participants and stacked bars (new vs returning families). Filters: Department | Type | Program | Year. **Year** and **Program** names open that year’s workspace on **Reports → Year comparison**. When several programs share a year, the year link opens the largest one. The same report is locked to the open series on `/programs/[id]?tab=reports&section=year-comparison`. CSV export. Route `/programs/reports/year-comparison`. Key files: `lib/programs/year-comparison.ts`, `lib/programs/year-comparison-queries.ts`, `components/programs/year-comparison-report.tsx`, `components/programs/program-year-comparison-panel.tsx`. Summer Camp 1 / Camp 2 years group as **Summer Camp**.

**Programs → Reports → Camp enrollment (September 2026):** Household-first camp history for Recreational Camps. Tabs: **Enrollment trend** (unique families by year × Fall/Spring/Summer/Winter), **Program summary** (table of each camp with new / returning / dropped vs the previous camp — Camp 1 then Camp 2 counts as two programs), **Family participation** (family, program count, last program, last date). 2026 operational **Summer Camp 2026** is split by session weeks (Weeks 1–4 = Camp 1, Weeks 5–8 = Camp 2). Route `/programs/reports/camp-enrollment`. Key files: `lib/programs/camp-enrollment.ts`, `lib/programs/camp-enrollment-queries.ts`, `components/programs/camp-enrollment-report.tsx`.

**Historical camp enrollments import (September 2026):** Workbook `Camp_Enrollment_Growth.xlsx` (Registrations) → department **Recreational Camps**, closed seasonal programs 2022–2026 (Fall / Spring / Summer 1 & 2 / Winter / Youth Intensive / Special Needs / RIJAAL). People + enrollments only (no charges). Parents matched by email then phone; minors as `people` under the parent. **Skipped** 2026 Camp One / Camp Two (already on operational Summer Camp 2026). Idempotent tag `CAMP_HISTORICAL_V1`. Script: `node scripts/import-camp-enrollments-historical.mjs` / `--execute`. Then fold camp households: `node scripts/sync-summer-camp-households.mjs --camp-parents --execute`.
...
**Programs → Payments list (July 2026 / August 2026):** `/programs/registrations` tracks registration balances. Row actions: Receive payment, **Edit registration**, **Mark financial assistance**, Custom payment plan, Add notes, **Withdraw & settle**. Top filters: **Department** | **Program** | **Offering** | **Status** (Active/Closed; default Active). Table columns (family/contact view): **Registration date**, **Contact** (name/email/phone), **# Participants**, **Participants** (stacked names), **Registration fee**, **Total paid**, **Additional fees**, **Status** (Active/Cancelled), Actions. Registration fee is the list amount; no discount columns (registration fee is not discounted). Rows group by registrant contact + program. Family payment rollups (**Program Fees** as months × monthly fee, **Additional Fees** + type, **Received**, **Balance**, paid/partial/unpaid/refunded) live on program **Finance → Payment Summary** (org-wide list remains **Reports → Payment Summary**). Helpers: `lib/programs/registration-report-helpers.ts`. KPI **Active Enrollment** counts only non-terminal enrollments on **currently active** offerings. **Open Balances** counts unpaid non-terminal enrollments in the filtered set. Key files: `programs-registrations-table.tsx`, `app/(dashboard)/programs/registrations/page.tsx`, `payment-summary-report-panel.tsx`.

**Programs → Reports → Payment Summary (August 2026 / September 2026):** Replaces the former **Tuition Plans** installment grid. One row per registration family (registrant contact + program). **Free courses** (fee plan `free` / $0 tuition) and complimentary $0 enrollments are omitted — this list is balances, not roster. An enrollment with money still appears even if the offering is tagged free. Program **Registrations → Enrollments** does not show amounts or payment status. Top filters: **Program** | **Offering** | **Status** (All / Balance due / Paid / Partial / Unpaid / Refunded). KPI cards: **Balance Due** (family count; click to filter), **Received**, **Outstanding**. Report tabs and those KPI cards stay pinned while the table scrolls. Enrollment load pages past Supabase’s 1,000-row default so imported QIL payments on older seats are included. Columns: **Participants** (adults: contact link + email/phone; youth: child names), **Offering**, **Program Fees** (`10 × $114.00` from `program_payment_plans`, else registration total), **Additional Fees** (same months × fee style for lunch/childcare), **Type** (Lunch, Childcare, etc. — transaction fees are omitted), **Received**, **Balance**, **Status** (Paid / Partial / Unpaid / Refunded), **Contact** last (minors / payer-is-not-participant only; hidden on all-adult self-registration lists such as QIL). CSV export. Org-wide route remains `/programs/reports/tuition-plans`. On a program workspace it lives under **Finance → Payment Summary** (`?tab=finance&section=payment-summary`). Key files: `lib/programs/payment-summary-report.ts`, `lib/programs/payment-summary-report-helpers.ts`, `components/programs/payment-summary-report-panel.tsx`, `program-workspace-finance-panel.tsx`.

**Programs → Reports → Add-ons (August 2026):** One row per purchased add-on (not family-grouped). Tracks materials, lunch, uniforms, field trips, childcare extras, and other non-tuition charges. **Transaction fees are excluded** on every program (Finance → Add-ons and org-wide `/programs/reports/addons`) and are not created for new imports. Imported Sunday School transaction-fee charges (65 charges / \$535) were deleted from the ledger (lines + payment schedules) so they no longer appear in Finance → Transactions or contact payment history. Run **`scripts/289_remove_program_transaction_fee_addons.sql`**. Tuition / registration payments are unchanged. Columns: **Contact**, **Participant**, **Program**, **Offering**, **Add-on Type**, **Quantity**, **Amount Due**, **Amount Paid**, **Balance**, **Status** (Paid / Partial / Unpaid / Refunded). Filters: Department | Program | Offering | Program Status (default Active) | Add-on Type | Payment Status. Source: `program_charges` add-on/fee rows plus non-tuition charge lines (skips registration/tuition/discount/voided/transaction fees). Org-wide route remains `/programs/reports/addons`. On a program workspace it lives under **Finance → Add-ons** (`?tab=finance&section=addons`); leftover `?tab=reports&section=addons` opens that Finance tab. Key files: `app/(dashboard)/programs/reports/addons/page.tsx`, `components/programs/addons-report-table.tsx`, `lib/programs/addons-report.ts`, `lib/programs/addon-display.ts`, `program-workspace-finance-panel.tsx`.
...
2. **Workforce routes:** Administration is a Programs / Event Management capability (not a subscription SKU and not core). Sidebar people lists live under Administration when that capability is on; module slug and paths remain `workforce` / `/workforce/*`. Align slug, routes, and folder names when safe (same class of rename as above — high blast radius).  
...
**Programs → Financial Assistance tabs (July 2026 / August 2026):** Canonical hub is **Finance → Financial Assistance** (`/finance/financial-assistance`): Overview, Submissions, Templates, **Reports** (staff FA awards: participant, year/season, program, original fee, assisted fee, plan; **Remove** restores original fee and supersedes the award). Enrollment payment rollups live under program **Finance → Payment Summary** (org-wide **Reports → Payment Summary**; not FA). Legacy FA **Payment Plans** tab redirects to `/programs/reports/tuition-plans`. Legacy `/programs/financial-assistance` and `?tab=financial-assistance` redirect appropriately. Awards stored in `program_enrollment_fa_awards` when staff use Mark financial assistance — run **`scripts/185_program_enrollment_fa_awards.sql`** (includes note backfill). Opening Reports imports past FA from notes/charge lines only for enrollments with **no** award row yet (so Remove is not re-imported). Contact profile Program enrollments + Financial show original fee and FA plan. Key files: `fa-awards.ts`, `programs-fa-report-panels.tsx`, `payment-summary-report.ts`.

**Offering manage → overview + edit dialog (August 2026 / September 2026):** Offering route is an **overview** (header breadcrumb: Departments › department › **program name** › **Offerings** › offering name; last segment not clickable; parents link to department overview, program overview, and year Offerings list), clickable name + status, year · department subtitle, 3 StatCards: Primary instructor / Schedule / Enrollment, enrolled student names). **Session enrollment:** Weeks listed with live headcounts from `program_registration_session_access`; Camp 1 (Weeks 1–4) / Camp 2 (Weeks 5–8) unique-kid rollups when those date windows apply; click a week (`?session=`) for that session’s roster. **Capacity:** offering capacity is **per session** when weeks exist (Enrollment hint “Up to N per session”; week rows show enrolled/N). Registration cannot add a kid to a week that is already full — run **`scripts/244_session_capacity_per_offering.sql`**. **Full-camp priority:** staff toggle **Accept selected weeks** (`selected_sessions_open`, SQL **`245`**). Off = only complete Camp 1 and/or Camp 2 packages enroll; selected/partial weeks waitlist. On = selected weeks may enroll and waitlist auto-promotes FIFO when all preferred weeks still have seats. **`245` patches `promote_waitlist`:** no early program-capacity block (per-session via grant); resolve offering from waitlist `offering_id` then default. Edit opens in `OfferingEditDialog` using the same basics form as Add (`OfferingBasicsForm`: **Type** Academic Year / Seasonal on top, then Name, Delivery, **Facility / space** when in-person or hybrid, Status, dates, instructor, gender/ages, capacity) plus foldable **Advanced Settings**. Changing Delivery to **Online** when a room is assigned shows **Release this space?** — Save then clears `program_schedule_items.venue_id` / `location` and those times leave the Facilities calendar. Online classes are not expanded onto the calendar even if a leftover room remains. Changing Type on edit updates `programs.program_kind`. **Questions:** Add question opens a dialog (prompt, answer type text box / text area / drop-down; drop-down values comma-separated). Run **`scripts/239_registration_question_select_options.sql`** for `select` type + `options`. **Pricing:** Fees keep Amount; when any fee **Recurrence** is **Monthly**, a **Payment Options** section appears under Fees (above Billing Schedule / Discounts) with checkboxes for **Pay in Full** (`$X at registration`) and **2 Semester Payments** (`$X/2 × 2`) — not month-by-month. One-time fees hide Payment Options. Choices persist on the fee plan notes marker + plan type when saving. **Billing Schedule** (monthly pricing): summary of billing day (from program start date), first/last billing date, duration, plus month checkboxes (4 per row; uncheck to skip e.g. Ramadan). Late enrollments bill from join month through last billing date only; skipped months are never charged. Run **`scripts/238_offering_billing_calendar_summary.sql`**. Dialog Save persists basics plus any opened Advanced handlers (staff, schedule, registration, pricing) and stays on overview (`router.refresh()`). Deep link `?edit=1` auto-opens the dialog. Preview / Share / Delete live on the Offerings list ⋯ menu; Delete also in the edit dialog footer. Key files: `offering-manage-client.tsx`, `offering-edit-dialog.tsx`, `offering-session-enrollment.ts`, `session-package-priority.ts`, `selected-sessions-priority-actions.ts`, `offering-registration-questions-editor.tsx`, `offering-simple-pricing-editor.tsx`, `offering-pricing-mapper.ts`, `program-billing-schedule-view.tsx`, `program-billing-utils.ts`, `offering-simple-schedule-form.tsx`, `program-sessions-editor.tsx`, `program-schedule-actions.ts` (`setOfferingScheduleFacility`).

**Move students between offerings (August 2026):** Program **Registrations** roster uses an **Offering** drop-down (current class name on the trigger; menu shows name · instructor · headcount so duplicate class names can be told apart). Offering overview Enrolled students still has **Move** and the destination dialog. Both keep the same enrollment (payments, FA awards, notes, history). Does not cancel and recreate. Same year/season only. Blocks archived destinations, limited offerings that are full, cancelled/withdrawn/transferred/expired enrollments, and a student already actively enrolled on the destination. Closed offerings are allowed. Session week access is cleared (not remapped); historical attendance stays on the old offering; tuition is not requoted. Not shown on the session-week roster. Requires `programs.manage` or department-head manage. Key files: `move-enrollment-offering-actions.ts`, `move-enrollment-offering-dialog.tsx`, `department-participants-panel.tsx`, `offering-manage-client.tsx`.

**Programs list row actions (August 2026):** Department **Offerings** table (`ProgramOfferingsListPanel`) — row click opens overview; **name** opens overview with `?edit=1` (edit dialog). **Drag handle** reorders offerings (persists `program_offerings.sort_order` via `reorderProgramOfferings`; run SQL **`243`**). Row **⋯** menu: Preview page, Share link, **Duplicate** (name dialog; copies options/pricing/sessions/billing via `duplicateProgramOffering`), **Cancel offering** (dialog: unenrolls students, optional refund of money received, writes off remaining balances, removes waitlist, sets `status=cancelled`, releases the class space; run SQL **`283`**), Delete (blocked when enrolled). After duplicate/delete/reorder/cancel, parent `DepartmentProgramsPanel` silently refetches via `onOfferingsChanged` (list is client state; `router.refresh` alone does not update it). Columns: Offering, Delivery, **Program Fee**, **Primary Instructor** (filter icon; All / No instructor / each teacher), **Days**, **Times**, Enrollment, **Offering Status** (filter icon; default **Active**, plus Cancelled and All). Fee type label **Program Fee** (DB `component_type` remains `tuition`) via `FEE_TYPE_LABELS` / `FEE_COMPONENT_TYPE_LABELS`. Key files: `program-offerings-list-panel.tsx`, `department-programs-panel.tsx`, `department-programs.ts`, `program-offering-actions.ts` (`cancelProgramOffering`). Cancelled offerings stay on the staff list but are hidden from family catalog/register/apply and cannot be a Move destination.`, `lib/programs/program-offering-actions.ts`, `lib/programs/program-offering-duplicate-actions.ts`, `lib/programs/offering-pricing-simple-types.ts`.

**Add Offering dialog fields (August 2026):** Shared `OfferingBasicsForm` with Edit — Type (Academic Year / Seasonal), Name, Delivery, start/end + enrollment dates (default from year), primary instructor, gender, min/max age, capacity, optional one-time tuition fee, and open-enrollment toggle. Instructor picker lists **employees of the department** (`staff.department_id`). **Edit Save always writes the primary instructor** via `setOfferingPrimaryInstructor` (updates/reactivates the offering-level `program_staff_assignments` row; department `staff` rows are eligible even without an Employee contact role). The edit form no longer resets mid-edit when staff options finish loading. Key files: `offering-basics-form.tsx`, `program-offerings-list-panel.tsx`, `offering-edit-dialog.tsx`, `program-staff-assignment-actions.ts`, `program-staff-assignment-queries.ts`.

**Primary instructor uniqueness (August 2026):** Some QIL offerings had several active `primary_instructor` rows (approved-roster import added teachers without deactivating the previous one). The Offerings list showed the newest name; the offering overview showed the oldest — Recitation Improvement 17/20 listed Fadia Salameh but opened as Amneh Ismail. Cleanup keeps the newest offering-level primary and deactivates extras. Unique index `program_staff_assignments_one_offering_primary` (SQL **`278`**) enforces one active offering-level primary. List, overview, and Move picker share `pickPrimaryInstructorAssignment`. Run **`scripts/278_one_primary_instructor_per_offering.sql`**.
...
**Department operating payments / Financial Summary (July 2026):** Replaces Google Sheets trackers for departments like Qur’an Institute for Ladies. **Department tabs:** Programs, Events, Group giving, **Financial** (sticky combined KPI row, then sub-tabs Employees, Payroll, Expenses, Financial Summary — separate from Group giving); year workspace keeps Registrations. **Enrollments:** enrollment list (student, year/season, course, teacher, parent/guardian) with filters, cancelled/withdrawn toggle, Export CSV — no payment columns. **Schedule** lives on **Program Workspace** (`/programs/[id]?tab=schedule`): **Class times** (visual **Week Board** of recurring offering times + instructor/space, optional **List** table, plus session/term list; CTAs for Facilities availability and Master Calendar) and **Activity planner** (Schedule Builder for `schedule_activities`, locked to that program). **Payroll:** teachers and childcare providers; log hours; create pay period for all; approve; department heads can **Edit** hours/amount and **Delete** pay lines (including approved). **Expenses:** program expense rows for the department (moved from Programs → Reports). **Financial Summary:** custom start/end periods plus a simple **By month** table (student payments, payroll, profit per calendar month); revenue from Programs billing (no student payment detail UI); approved payroll expenses. URLs: `?tab=financial` (default Payroll), `?tab=financial&section=employees|expenses|budget`; leftover `?tab=employees` opens Financial → Employees; legacy `?tab=payroll|expenses|budget` still resolve. SQL: `169`–`173`. Key files: `department-group-workspace-client.tsx`, `department-expenses-panel.tsx`, `department-programs-panel.tsx`, `program-catalog-view.tsx`.
...
**Department Head / Director (July 2026 / August 2026 / September 2026):** Mark an employee **Department Head (Director)** on their department Employment details sheet (requires a department). Run **`scripts/186_staff_department_head.sql`**. Their contact profile shows a **Department workspace** card (like teacher **Program assignments** → Manage) with **Open workspace** → `/workforce/departments/[id]` (all tabs). Access is scoped via `canViewDepartment` / `canManageDepartment` (`lib/departments/department-access.ts`). Heads **edit and do everything** for that department only: payroll, employees, settings, events (create/edit from the department Events tab / Facilities calendar with the department locked), and that department’s programs, offerings, registrations, payments, schedule, and staff assignments. Org-wide Programs lists, Finance (`/finance/transactions`, `/finance/payroll`), and the Departments list stay admin-only — heads without `programs.view` are sent to **My department**. Program workspace access uses `canAccessProgram` / `canManageProgram` (`lib/programs/program-access.ts`), which also includes **Program Lead** for that year/season. Event workspace access uses `canViewInternalEvent` / `canManageInternalEvent` (`lib/events/event-access.ts`). Key UI: `department-employee-profile-sheet.tsx`, `contact-department-workspace-panel.tsx`, `contact-employee-panel.tsx`.

**Department-scoped access (July 2026 / August 2026):** Department workspace mutations/views with a known `departmentId` use `canViewDepartment` / `canManageDepartment` from `lib/departments/department-access.ts` (org `staff.view`/`staff.manage` **or** active Department Head for that department). Applies to payroll, budget periods, babysitting, year programs (`canManageDepartment` **or** `programs.manage`), department staff assign/update, Directory search when adding employees (`searchContactsForHrPickerAction`), and giving-group link helpers. Creating/deleting departments on the org list remains org-wide `staff.view`/`staff.manage`. Heads cannot create or delete departments. SQL for head flag: `186`.

**Horizon Community Foundation demo seed (August 2026):** `node scripts/seed-horizon-community-foundation-demo.mjs --execute` loads fake Directory + Fund Development data for that org only (10 people, 1 org donor, 2 campaigns, 6 donors, pledges/payments, ask levels, prospects, wishlist, 3 sponsorship packages + 4 sponsors on Annual Community Impact 2026), plus **Youth Programs** and **Community Engagement** departments, academic **Horizon Scholars 2026–27** (Fall Tutoring Circle + Spring STEM Lab), seasonal **Summer Adventure Camp** and **Winter Break Camp**, and events **Horizon Volunteer Day** + **Family Welcome Night**. Names are mixed (American, Indian, Latino, African, Korean; one Arabic). Marker `HORIZON_DEMO_SEED_V1`. Reset: `--clean --execute`. Programs/events only (leaves donations in place): `--programs --execute`. Packages/sponsors only (leaves pledges and payments in place): `--sponsorships-only --execute` (Presenting $10k / Community Partner $5k / Table Host $2.5k; Lakeside paid, Whitaker partial, Alvarez unpaid, Grace mixed cash + in-kind). Cash sponsorship commitments add to Overview **Committed**; they do not change **Collected**. Does not touch MAS Dallas. Staff header user menu shows **Admin** on this org instead of the signed-in email username. If MAS Dallas department names still appear on Horizon, run **`scripts/276_restore_department_organization_ids.sql`** in Supabase (preview SELECT, then UPDATE). **Applied August 2026:** MAS Dallas departments (Center, Education, Quran Institute for Ladies, Recreational Camps, Rising Star Academy, Swimming, Tarbiya, Youth) were restored from Horizon back to org `e057e00a-e4e3-4adf-9af5-f465db1894be`. The list page only queries the selected org, so MAS Dallas showed “No departments yet” until this restore.

**Settings → Users list fix (June 2026 / August 2026):** `/settings/users` loads **staff** members via `fetchOrganizationUsersForSettings()` (service role + `settings.users.view` + `staffOnly`). Customer-portal logins (`organization_members.role = viewer`, created on join/rental/instructor portal access) stay in Directory as contacts and do **not** appear here. The client does not call that action on mount (that pattern made Next.js refresh the route forever). Row menu supports **Change Role**, **Edit Profile** (name + login email), **Send Reset Email** (Supabase recovery link to `/auth/confirm`), and **Delete** (removes org membership; blocks self-delete and last Super Admin). Actions require org system admin or `settings.users.manage`; audit log entries: `member.profile_updated`, `member.password_reset_sent`, `member.removed`. Key file: `lib/organizations/organization-users-actions.ts`.

**Work emails / assigned Directory person (August 2026 / September 2026):** Organization-owned mailboxes (for example `finance@masdfw.org`) are staff logins, not a second personal email. Personal email stays on `contacts.email` and is used for My Account. Assign a Users row to one Directory person (`organization_members.assigned_contact_id`); reassign by changing the person — do not create a second contact or a second Users row for the same address. The department employee sheet has a **typed work email** field (not a Users drop-down): Save assigns an existing login, or invites a new Admin staff login and assigns it. When someone leaves, mark the employee inactive or remove them from the department (that unassigns the work email) and keep the Directory contact plus personal login. Staff Tools: a work login uses membership + the assigned person; a personal login whose contact already has a work email assigned does **not** get Staff Tools from the employee role (membership admin on the personal login is unchanged). Settings → Users invite may still assign a Directory person. Run **`scripts/288_organization_member_assigned_contact.sql`**. Key files: `lib/organizations/work-email-assignment.ts` (`setContactWorkEmailAction`), `lib/organizations/work-email-lookups.ts`, Users `Assigned to` column, employee drawer personal vs work email. Audit: `member.work_email_assigned`.

**Settings → Links (August 2026):** Customer join and public catalog/calendar URLs moved from Users to **Settings → Links** (`/settings/links`). Users is staff accounts only. Key: `organization-join-link-card.tsx`, `app/(dashboard)/settings/links/page.tsx`.
...
**Pledge reassignment (June 2026):** **Pledge Details** on **Campaigns → Pledges** (`/donations/campaigns/pledges`) and contact profile **Financial → Pledges** includes an **Assigned to** picker (person, organization, or group). Saving reassigns the pledge to the selected contact’s donor record and moves linked **payments** and **pledge reminders** with it; affiliation sync runs on both old and new contacts. Use this to move historical pledges from an individual to a group (e.g. Quran Institute). Key files: `lib/donations/pledge-admin-actions.ts` (`updatePledgeAction`, `reassignPledgeContact`), `components/donations/pledge-contact-picker.tsx`, `components/donations/pledge-details-dialog.tsx`.

**Contact Financial → cross-module summary (June 2026 / layout July 2026 / September 2026):** Contact profile **Financial** tab is the money hub (not a second ledger). Profile tabs stay **Overview | Financial | Activity** — product modules are not extra profile tabs. **Overview** keeps KPI cards (Total payments received, Outstanding Balance, Last Payment), Financial by Module, and Recent Transactions. **Financial** inner navigation follows the tenant subscription: a **programs-only** org flattens to **Charges | Payments | Refunds | Payment Methods** (no Recurring gifts, Pledges, giving Statements, or Membership). A **donations-only** org flattens to **Recurring | Pledges | Refunds | Payment Methods**. When two or more billing modules are on, Financial shows **All** plus **Fund Development / Programs / Venue Rentals / Vendor Hub / Membership** (only subscribed modules). **All** is the shared ledger (payments, refunds, payment methods). Fund Development holds recurring gifts, pledges, and giving statements. Programs holds open charges and program payments. Mixed orgs still default a programs-only contact into **Programs** instead of empty Pledges. `?tab=financial&module=programs` (or `fund-development`, `venue-rentals`, `vendor-hub`, `membership`, `all`) deep-links the inner section. Recurring on this tab is donation recurring plans, not program tuition. The empty Invoices placeholder was removed. Right-rail Statements appear only on Fund Development. Membership is a Financial inner section when that module is on, not a permanent rail card. Open Balances still lists unpaid pledges, venue rental lines, and program fee balances. Transactions show actual payments (no pledges) with Refund / Download / Email Receipt; **Link to Pledge** only when Fund Development is subscribed. Donation **Recurring** still uses donor-scoped plan actions (Edit, Change Card, Receive Payment, Pause/Resume, Cancel, New Plan). **Pledges** use `DonorPledgesTab`. Payment methods stay shared (staff Financial + customer Profile). Key files: `components/contacts/contact-financial-panel.tsx`, `lib/contacts/contact-financial-nav.ts`, `lib/contacts/contact-financial-actions.ts`. **Contact profile navigation (July 2026):** Breadcrumbs handle return paths (e.g. **Dashboard > Contacts > People**); the redundant profile back button was removed. `returnTo` query param and session-tracked paths still apply for **Open full profile** links and post-delete redirects. Key files: `components/contacts/contact-profile-client.tsx`, `lib/navigation/return-to.ts`.

**Contact Financial → Pledges + reminders (June 2026 / July 2026 / August 2026):** Contact profile **Pledges** section uses `DonorPledgesTab`. Row click (and **Add Pledge**) opens the canonical **Pledge Details** window so actions stay in sync with campaign and global pledge pages.
...
**Campaign progress gauge (June 2026):** Speedometer-style fundraising gauge on `/donations/campaigns` (card grid for campaigns with goals) and campaign detail Overview (right of **Overview metrics**, no heading). Red/orange/green arc, needle, and total raised; supports exceeding 100% of goal. Component: `components/donations/campaign-progress-gauge.tsx`.

**Campaign source breakdown (June 2026):** Campaign detail (`/donations/campaigns/[id]`) shows fundraising metrics in a **colorful table** (Cash, Checks, Square, One-Time, Recurring, Ticket Sales, Donors, Largest Gift, Pledges last with highlight) plus the goal gauge on the right, top-aligned with **Overview metrics**. **Per-campaign metric customization:** **Customize** on the overview table toggles visible rows and order; **Automatic** mode (default) hides empty source rows such as Ticket Sales or Square until they have activity. Stored in `campaigns.overview_metric_keys` (migration `134`). Below metrics: **Outstanding Pledges** table for the campaign (donor with **Primary contact** subline for organizations/groups or colored group badges for individuals, pledged/paid/balance in red, status with orange **Open** badge, date). Row click opens **Pledge Details**. Donor names open **Contact profile in a modal** (`ContactProfileDialog`) from outstanding pledges, campaign donors list, and largest-gift row. Logic: `computeCampaignSourceBreakdown`, `fetchCampaignOutstandingPledges` in `lib/donations/campaign-analytics.ts`; metric config: `lib/donations/campaign-overview-metrics.ts`; UI: `campaign-source-breakdown-cards.tsx`, `campaign-overview-metrics-editor.tsx` (`CampaignOverviewMetricsTable`), `campaign-outstanding-pledges-table.tsx`.

**Campaign workspace + phases (August 2026 — Phase A / September 2026):** Opening a campaign is a tabbed workspace (`?tab=`): **Overview**, **Event**, **Prospects**, **Pledges**, **Donations**, **Sponsorship**, **Groups**, **Wishlist**. Workspace tabs stay pinned below the staff header while the rest of the page scrolls. On **Overview**, the Campaign Goal KPI cards stay pinned with those tabs and wrap to extra rows instead of scrolling sideways. **Prospects** is a single donation prospect table (no Strategy sub-nav). Pinned KPIs: prospects, total ask, overdue follow-ups, pledged. Overview uses clear money language: **Committed** (valid pledges), **Collected** (all campaign payments), **Outstanding** (committed minus collected; never below zero). Open pledge balances stay on the Outstanding Pledges table and can be higher when some gifts were not applied to a pledge. Each campaign has **one goal** (`campaigns.goal_amount`) — Goal Breakdown / campaign phases are no longer used. Existing phase rows can be cleared with `scripts/270_disable_campaign_goal_phases.sql` (table/columns kept unused). Pledges and Donations tabs filter the existing ledger by campaign (no duplicate financial records). On **Donations**, staff can **Receive Donation** and **New Recurring Plan** in place (campaign locked; same `payments` / `recurring_donation_plans` ledger as Fund Development → Donations). Recurring create requires a start date plus the **number of installments or an end date**. KPI cards stay pinned with the workspace tabs: one-time total, recurring plan count, monthly recurring amount (active/past-due plans, frequency converted to monthly), one-time gift count, and unique donors (`computeCampaignDonationKpis`, `campaign-donations-kpis.tsx`). Click a pledge row (or a Donations **Pledge Applied To** id) to open **Pledge Details** in place. Donation and pledge dates display as calendar dates only (no time). Donation **Status** is **Allocated** when the payment is applied to a pledge, **Unallocated** when the donor still has an open campaign pledge that this payment has not been applied to, and **—** for standalone gifts with no pledge to allocate against (refunded/voided statuses still win). Opening a campaign loads **that campaign’s** pledges, payments, and recurring plans once on the server (`getCampaignDetailAction` → `fetchCampaignWorkspaceLedger`); it does not download the whole-org ledger. Key files: `campaign-workspace-paths.ts`, `campaign-overview-tab.tsx`, `campaign-fundraising-plan-tab.tsx`, `campaign-fundraising-plan-table.tsx`, `lib/donations/campaign-analytics.ts`. Migration `260_campaign_phases.sql` remains historical.

**Campaign Donations receive + recurring (September 2026):** Campaign → **Donations** (`?tab=donations`) can record a one-time gift (**Receive Donation**) and create a recurring plan (**New Recurring Plan**) without leaving the campaign. Both write the same ledger as `/donations/payments/transactions` and `/donations/payments/recurring`. Campaign is locked on the form; category/fund, group, wishlist, and memo match the global receive flow. New recurring plans require installments or an end date (the other field fills in). Same schedule fields are required on Fund Development → Donations → Recurring → **New Plan**. Key files: `components/donations/campaign-donations-tab.tsx`, `lib/donations/recurring-donation-actions.ts`, `lib/donations/recurring-donation-schedule.ts`.

**Staff load performance (September 2026):** `/api/organizations/sidebar-modules` counts Directory roles with the service-role client (org-scoped, after membership check) and caches the summary for 60 seconds (`fetchCachedDirectoryNavSummary`). Donation staff access loads the five donations permissions in one `role_permissions` query.
...
**Campaign strategy ask levels (August 2026 — Phase B; retired September 2026):** The gift/ask chart (`campaign_ask_levels`, migration `261`) is no longer shown. Target # / Target Value / Gap planning is retired. Ask amount is typed per prospect on Prospects (`suggested_ask_amount`). Table/columns and `pledges.ask_level_id` remain unused by the UI.

**Campaign prospects (August 2026 — Phase C; simplified September 2026):** Campaign → **Prospects** (`?tab=prospects`) is a donation-only working list (`campaign_prospects.ask_type = donation`). **Add prospect** opens a popup to search the donor, type **Ask level**, type **Assigned to** (a name, not a Directory person/organization), optionally set dates/notes, then **Save**. Assigned to is for an employee; with Administration off for Fund Development, any typed name saves. Later, when Administration is on, this field will search employees only and can add a missing employee. The table is read-only; click a row to open the same window to edit, remove, or **Record pledge** / **View pledge**. No stage or ask-type columns. Sponsorships are added on Campaign → **Sponsorship**, not this table. Unique `(campaign_id, contact_id, ask_type)` remains. SQL **`299`** adds `assigned_to_name`. Key files: `campaign-fundraising-plan-tab.tsx`, `campaign-fundraising-plan-table.tsx`, `campaign-fundraising-plan-dialog.tsx`, `campaign-prospect-actions.ts`. Legacy `?tab=plan` / `?tab=strategy` / `section=prospects` redirect to `?tab=prospects`.

**Prospect → pledge conversion (August 2026 — Phase D):** **Record Pledge** in the Prospects window opens the canonical **Pledge Details** window and creates **one** `pledges` row with `campaign_id` and `campaign_prospect_id`. Typed ask amount (`suggested_ask_amount`) is preserved. `converted_pledge_id` is set. **View pledge** opens the same window. Action: `convertCampaignProspectToPledgeAction`. UI: `components/donations/pledge-details-dialog.tsx`.

**Prospect → sponsorship conversion (August 2026):** Staff create committed sponsors on Campaign → **Sponsorship** (**Create Sponsorship**). Creates one `campaign_sponsorships` row (contact, campaign, optional event/package, cash/in-kind/mixed, committed amount, status, payment status). Does **not** write a donation or pledge. Package benefits are copied onto `campaign_sponsorship_benefits` at create so later package edits do not change existing agreements. Permissions reuse `donations.prospects.manage`. Key files: `campaign-sponsorship-actions.ts`, `campaign-sponsors-tab.tsx`, `campaign-sponsorship-dialog.tsx`. SQL: `scripts/284_campaign_sponsorship_prospects.sql`, `scripts/285_campaign_sponsorship_packages.sql`.

**Campaign sponsorship packages (August 2026):** Packages live under Campaign → **Sponsorship → Packages** (not a top-level workspace tab). Each package belongs to one campaign (`campaign_id` required; `event_id` optional). Staff define name, amount, description, status, display order, and structured benefits. Active packages appear on sponsorship prospect and create-sponsorship selectors and prefill suggested ask (override allowed). Duplicate copies package + benefits, not sponsors. Used packages are deactivated instead of hard-deleted. Sponsor fulfillment (`pending` / `in progress` / `completed` / `not applicable`) is tracked per sponsorship, not on the package. Cash sponsorship commitments add to campaign `totalCommitted`; they are **not** added to `totalRaised` (payments stay the collected ledger; in-kind is reported separately). Key files: `campaign-sponsorship-package-actions.ts`, `campaign-sponsorship-package-dialog.tsx`. Run `scripts/285_campaign_sponsorship_packages.sql` after `284`.

**Campaign groups (August 2026 — Phase E):** Campaign → **Groups** (`campaign_groups`, migration `263`). Lead contact, optional link to existing org group contact, stable `public_token` donation URL `/donate/g/{token}`, Copy link / Copy QR code icons, Regenerate/Deactivate. Group goals are not used in the UI (`goal_amount` column kept unused). Group detail via `?tab=groups&group=`. Metrics: pledged/collected/donors from `pledges`/`payments.campaign_group_id` (no double count). Group detail KPI cards are colorful; **Donors** number opens a popup of names and gift amounts. Key files: `campaign-groups-tab.tsx`, `campaign-group-actions.ts`, `campaign-group-helpers.ts`. Run `scripts/263_campaign_groups.sql`.

**Campaign wishlist (August 2026):** Campaign → **Wishlist** (`campaign_wishlist_items`, migration `267`). Campaign-specific funding priorities (not funds, not a second ledger). Target amounts do **not** add to the campaign goal. Table lists items sorted High → Medium → Low (no search/filters; no manual Order column). Row actions: public **link** icon opens donation URL + QR; **pencil** edits; **Carry Forward** is on the item window, not the table. Funding status is derived from attributed pledges/payments (`wishlist_item_id`); project status is operational and independent (an item can be Completed but Partially Funded, or Fully Funded but still Planned). Carry-forward clones the need into a destination campaign with `previous_funding_amount` / `remaining_need_at_carry_forward` — old payments stay on the source campaign. Public items use `/donate/w/{public_token}` (Copy/QR reuse group QR helper). Org-wide wishlist lives on **Campaigns → Wishlist** (`/donations/campaigns?view=wishlist`). Permissions reuse `donations.view` / `donations.campaigns.manage`. Key files: `campaign-wishlist-tab.tsx`, `campaign-wishlist-actions.ts`, `campaign-wishlist-types.ts`, `campaign-wishlist-funding.ts`. Run `scripts/267_campaign_wishlist.sql`.

**Campaign tenant isolation (September 2026):** Wishlist rows for **Annual Fundraiser - September 2026** (Repair Sinking Walkways, Youth Development Camps & Retreats) belong to **MAS Dallas**, not Horizon. Horizon’s demo wishlist is on **Annual Community Impact 2026** / **Youth Scholarship Drive**. Staff queries already filter `organization_id` + `campaign_id`. Service-role writes bypass RLS, so migration **`292`** adds composite `(campaign_id, organization_id)` FKs on campaign children and same-org triggers on pledges/payments/plans/checkout. Wishlist loads refuse a campaign that is not in the selected org. Run `scripts/292_campaign_same_organization_fks.sql`.
...
**Campaign overview insights (August 2026 — Phase G):** Campaign → Overview adds **Action Required** (overdue/upcoming follow-ups, unassigned donation prospects), **Team Summary** (by assignee: open / overdue / pledged / ask $), and **Campaign Groups** rollup. Deep-links into Prospects filters (`followUp`, `assignee`). Contact Financial panel shows **Fund Development** history (prospects, assignments, group gifts) gated by `donations.view`. Key files: `campaign-overview-insights.tsx`, `contact-fund-development-history.tsx`.
...
**Org-wide Campaign Groups report (August 2026):** Campaign fundraising groups analytics live under **Campaigns → Campaign Groups** (`/donations/campaigns?view=groups`). Legacy `/donations/reports/campaigns?view=groups` and `/donations/reports/campaign-groups` redirect there. Separate from Donors → Group Giving (CRM groups). Reuses `computeCampaignGroupMetrics`. **Export CSV** on that view and on Campaign → Groups.
...
**Fund Development IA redesign (August 2026):** Sidebar became **Overview / Campaigns / Pledges / Donations / Reports / Settings**. Operational work (transactions, recurring plans, import/match, receipts & statements) lives under **Donations** (`/donations/payments/*`). Date range and CSV export on Transactions filter KPIs, charts, and the payment table together. Transaction columns are date, donor, amount, method, campaign, fund, receipt status, and status; method/campaign/fund filters use stored values. CSV export still includes type and group. Receipts has a Missing queue (`?status=missing`); year-end KPIs come from `donation_receipts` where `receipt_type = annual_statement`. No schema change — same `payments` / `pledges` / `recurring_donation_plans` / `donation_receipts` records. Key files: `donations-sidebar-children.ts`, `donation-ops-chrome.tsx`, `donation-payments-panel.tsx`. **Superseded September 2026** by one home per object (Reports sibling removed).

**Fund Development one home per object (September 2026):** Each object has one working page. Sidebar is **Overview / Campaigns / Donors / Pledges / Donations / Settings**. Giving charts live on **Donations → Transactions**; pledge overdue/collection % on **Pledges**; campaign comparison and groups/wishlist views on **Campaigns**; person/household/group rollup is sidebar **Donors**. Legacy `/donations/reports/*` URLs redirect except **Donors** (`/donations/reports/donors`). Contact Financial and the customer portal stay person-scoped.

**Donations sidebar (July 2026):** Under Fund Development: **Overview**, **Campaigns**, **Donors**, **Pledges**, **Donations**, **Settings**. See September 2026 one-home IA for current destinations. **New campaign → fund (July 2026):** Creating a campaign on `/donations/campaigns` also creates an open fund under category **General Donation** with the same name (`ensureCampaignDonationFund` via `createCampaignAction`); creates the category if missing; skips if a matching fund already exists. Editing a campaign does not rename the fund. **Campaign create/edit/delete (Aug 2026):** Overview table uses server actions (`createCampaignAction` / `updateCampaignAction` / `deleteCampaignAction`). Campaign names are blue links into the workspace; edit/delete icons are not on the list. Delete lives in the workspace **Edit Campaign** dialog and is blocked when the campaign has pledges, payments, or recurring plans. Apply `scripts/258_campaigns_rls_policies.sql` for permission-aware campaign RLS (staff view/manage + org-member SELECT of active campaigns for portal pickers).
...
**Pledges summary cards (June 2026 / September 2026):** Pledges page stat cards match Donations Overview styling (colored left border, rounded icon badges). Status filter defaults to **Open** (any outstanding balance). Statuses are **Open** and **Fulfilled** only — a partly paid pledge stays Open. SQL `scripts/296_pledge_open_fulfilled_status.sql`. KPI cards and table column headers stay pinned; only pledge rows scroll. File: `app/(dashboard)/donations/campaigns/pledges/page.tsx`.

**Canonical Pledge Details window (August 2026 / September 2026):** Staff add, edit, view, collect, remind, and delete pledges from one **Pledge Details** dialog (`components/donations/pledge-details-dialog.tsx`). **Add Pledge** and row click open the same window (title is always Pledge Details). Monthly, quarterly, and yearly pledges require a first payment date plus the **number of installments or an end date** (the other field fills in; amount per payment is calculated). Status is not editable — **Open** until the balance is paid, then **Fulfilled**. One footer **Save** writes the pledge and schedule together, plus any payment amount entered on an existing pledge; **Cancel** closes without saving. **Save** always closes the window after a successful add or payment (it does not stay open or reopen as details). If Contact search has no match, **Create “{name}”** opens Add Contact (person or organization; names with Foundation/Inc./LLC default to organization). Collection fields (Receive Payment, Remind, Mark Contacted) and **Delete** appear only when you open an existing pledge. Row **⋯** menus were removed from Pledges, campaign outstanding pledges, campaign pledges, and contact/donor pledge tables. Deep links (`?pledgeId=` / `?action=add`) still open this window on `/donations/campaigns/pledges`. Customer portal pledges are unchanged.

**Donation attribution fields (June 2026 / August 2026):** Add Pledge / Record Payment / Record Pledge pick **Category** (from `donation_categories`) and **Fund** on the same row. New gifts default to **General Donation** / **General Fund** when those records exist (no “No category” / “No fund” options). Choosing a fund still fills its category; choosing a category filters the fund list. Campaign is its own row, or hidden when already locked (prospect Record Pledge). Manage categories and funds under **Donations → Settings → Categories** (`donation_categories`, `donation_subcategories`). Staff access uses the same `donations.view` / `donations.manage` rules as payments (migration **`294`**); the page reads the selected organization, not `profiles.organization_id`. **Fund close (July 2026):** `donation_subcategories.is_active` (migration `161`); closed funds show **Closed** in settings, are omitted from customer/staff fund pickers for new gifts, and remain on historical pledges/payments. Toggle **Accept new gifts** in the Edit Fund dialog. Settings **Funds** table defaults to **open** funds with **View all** for closed. **Customer dashboard Donation Options (July 2026):** lists every donation category (`buildCustomerOpenDonationCategories` in `lib/customer/customer-open-donation-categories.ts`); the **Specific Fund** picker appears only when that category has open funds. Categories with no open funds (or only closed funds) accept category-level gifts without picking a fund. Customer donations validate open funds in `validateCustomerDonationAttribution` (portal UI, Stripe checkout, offline server action); migration `162` blocks portal payment inserts to closed funds at the database layer. Files: `components/donations/donation-attribution-fields.tsx`, `app/(dashboard)/donations/settings/page.tsx`, `lib/donations/donation-fund-status.ts`, `lib/customer/customer-open-donation-categories.ts`, `lib/customer/customer-donation-actions.ts`.
...
## Programs Home (August 2026)

**Programs** in the staff rail is one item (no children overlay). Clicking it opens **Programs Home**.

**Overview** (`/programs`) is the module home: page title **Overview**, breadcrumb **Dashboard > Programs > Overview**. Colored whole-card links open Programs, Offerings, Registrations, Finance, Financial Assistance, and Reports. There is no module tab bar. Nested pages keep **Programs** in the breadcrumb pointing at `/programs` so staff can return to Overview.

Reports pages keep the report tab bar (`ProgramsReportsNav`). Finance pages keep Transactions | Payroll (`ProgramsFinanceNav`). Financial Assistance keeps its own hub tabs.

The **Programs** list (`/programs/list`) lists every year/season together. Academic vs Seasonal is a badge on the row (`programs.program_kind`), not a sidebar or tab split. Each card stacks department, date range, offering count, and total enrolled (non-cancelled enrollments across non-archived offerings). Filters: Search programs, Department, Type (Academic / Seasonal), and Status (Active / Draft / Closed / Archived / All; default **Active**). Closed and archived years stay one Status change away. **New Program** (`programs.manage`) opens Quick Create at `/programs/create`. Clicking a row opens the **Programs module workspace** at `/programs/[id]` (Overview / Offerings / Registrations / Settings). Department **Programs** is a summary doorway only — not a second place to manage programs. Create still respects org `program_kinds` entitlement on Add Year / Add Season in the Programs module.

The **Offerings** page (`/programs/catalog`) is an org-wide administrative table over the same `program_offerings` records (not a second dataset). Default Status = Active. Offering and program names open the existing Offering / Program workspaces. Public catalog and customer browse still use offering cards. Add Offering asks staff to select a program, then continues into that program’s existing Add Offering workflow. View Public Catalog / Copy Public Catalog Link live in the page `•••` menu.

Key files: `app/(dashboard)/programs/page.tsx`, `lib/navigation/sidebar-nav.ts`, `lib/navigation/return-to.ts`, `components/navigation/navigation-breadcrumbs.tsx`, `components/programs/programs-staff-subnav.tsx`, `components/programs/programs-list-client.tsx`, `components/programs/programs-all-list.tsx`, `components/programs/offerings-management-page.tsx`, `lib/programs/programs-module-nav.ts`, `lib/programs/programs-list-filters.ts`, `lib/programs/offerings-management.ts`.
...
* **Catalog** (`/programs/catalog`) — staff Offerings table (search, filters, Table/Grouped views) over existing `program_offerings`. Not in the Programs flyout; open by URL for now. Offering name opens Offering Workspace; program name opens Program Workspace. Default status filter is Active. Public catalog link is in the page `•••` menu. Customer portal **`/customer/programs`** and **public browse** **`/o/[orgSlug]/programs`** still use offering cards for families. Run **`scripts/191_offering_catalog_branding.sql`**.
* **Program detail** (`/programs/[id]`) — Programs module workspace for department-linked years (Overview / Offerings / Registrations / Schedule / Settings with General, Notifications, Promo Codes). Orphan years (no `department_id`) keep standalone `ProgramDetailClient`. Key files: `program-workspace-client.tsx`, `program-workspace-settings-panel.tsx`, `program-detail-client.tsx`, `department-program-dashboard-panel.tsx`, `department-program-overview-panel.tsx`, `department-programs-panel.tsx`.
...
* **Service Needs** on **Event workspace → Settings** (`/event-management/[id]?tab=settings`). Volunteers / Youth / Vendors save on that event (`requires_volunteers` / `requires_childcare` / `requires_vendors` + `service_requirements`). Department **Settings** has one **Save** (name/color/description/terms) and **Delete department** at the bottom; delete is blocked when programs, offerings, or employees exist. **General**, **Notifications**, and **Promo Codes** live on Program Workspace **Settings** (`/programs/[id]?tab=settings`; leftover `?section=defaults|registration|year-defaults` open General; `?section=notifications|promo-codes` keep those tabs). Registration policy unique fields (capacity, duration, payment, form, cancellation) and Notifications still save to `department_program_settings` (stubs). Promo codes remain department-scoped (`discount_codes.department_id`); sibling discounts lock to the open program. Leftover department `?section=year-defaults|registration|notifications|promo-codes` opens the Programs doorway. Leftover `?section=service-needs` opens department **Events**. Programs flyout **Settings** is `/programs/settings` (org-wide placeholder). `/programs/settings/service-needs` redirects to `/event-management`. Key files: `components/events/internal-event-service-needs-settings.tsx`, `components/programs/program-workspace-settings-panel.tsx`, `components/programs/program-policy-settings-panel.tsx`, `components/departments/department-settings-panel.tsx`, `components/departments/department-promo-codes-settings-panel.tsx`. Apply SQL `scripts/190_department_settings_promo_codes.sql`.
...
* **Departments** is a top-level drawer item under **Administration** → `/workforce/departments` (standalone page — not a Workforce tab). **List UI (August 2026):** card grid (`DepartmentsManager`) with color + initial (no flyer, no color dot, no ⋯ menu). The whole card opens `/workforce/departments/[id]`. Cards show program count, **Director** (active `staff.is_department_head`), and **Employees** (staff assigned to the department). Add Department stays on the list; name, color, description, Terms, and delete are on department **Settings** (one Save; delete blocked unless the department has no programs, offerings, or employees). Run **`scripts/203_department_flyer_url.sql`** for `departments.flyer_url` (column retained; not shown on Overview). **Department-level tabs:** Overview (KPI snapshot) | Programs (program cards) | Events | Group giving | Financial (sticky combined KPIs, then Employees / Payroll / Expenses / Financial Summary) | Settings. **Year-level** (`?year=`): leftover URLs redirect to `/programs/[id]`. Program workspace tabs: Overview | Offerings | Applications (Application & Approval) | Registrations | Schedule | Finance | Reports | Settings. Run **`scripts/241_department_terms.sql`** for `terms_html` / `terms_pdf_url`. Employees is a top-level tab (`?tab=employees`; roster click → employee profile sheet; Name, Position, Pay, Status). Leftover `?tab=financial&section=employees` opens Employees. Hours / pay periods / Log hours / Create pay period live under Financial → Payroll. Legacy `/workforce/settings/departments`, `/hr/departments`, and `/workforce?tab=departments` redirect to `/workforce/departments`. Programs drawer group starts at Catalog (not Departments). **Payments** is a tab on Programs → Reports (opens `/programs/registrations`).
...
* Registrations under **Event Management → Reports → Childcare** (`/event-management/reports/childcare`). Leftover `/programs/reports/childcare` and `/workforce/childcare/registrations` redirect there.
...
**Customer apply (July 2026 / September 2026):**
* Volunteer → `/customer/apply/volunteer` (Profile → Applications when **Programs** or **Event Management** is on; staff **Copy apply link** on Volunteers). Approve creates/links a `volunteers` roster row (`lib/volunteers/ensure-volunteer-from-application.ts`).
* Childcare provider → `/customer/apply/childcare` (same module gate as Volunteer; staff **Copy apply link** on Childcare Providers). Approve creates/links childcare `staff`.
* Vendor → `/customer/apply/vendor` (Profile → Applications when **Vendor Hub** is on).
...
* Staff **Manaratee** wordmark lives in the header above breadcrumbs (`components/layout/header.tsx`, `public/Logo2.png`); sidebar top is a spacer only so nav still lines up with the trail. Current organization **name** is shown immediately left of the tenant logo, then the user menu. On **Horizon Community Foundation** (demo), the user menu shows **Admin** instead of the signed-in email username. Dashboard org branding (`/dashboard`) sizes the logo box to the image (`object-contain`) so wide marks are not cropped in a square. **Change Logo** appears only in **Edit** mode. **Edit** / **Cancel** / **Save Changes** sit at the bottom of the dashboard org profile; Cancel restores unsaved field edits. **Subscribed Modules** lists enabled product modules (same source as the staff sidebar): Event Management, Programs, Vendor Hub, Venue Rentals, Fund Development, Membership. Workforce, Finance, Facilities, and Community Calendar are not listed as subscribed SKUs.
...
| Recurring plan create | `components/donations/donation-recurring-panel.tsx` (`/donations/payments/recurring`) and Campaign → Donations | Category + fund + campaign on plan; installments or end date required |
...
* `/customer/dashboard` — modular customer home. Fund Development widgets (when `donations` is enabled): Lifetime Giving / Active Pledges / Outstanding Balance, Needs Your Attention, up to 2 featured campaigns, Recent Activity from existing pledges/payments. Key: `lib/customer/customer-dashboard-data.ts`, `components/customer/customer-dashboard-view.tsx`.
* `/customer/donation` — tabs: **Giving Opportunities** (default; Donation Options via `CustomerDashboardGivingSection` / `customer-donation-dialog.tsx`), **My Pledges**, **Giving History**. Featured campaign cards live on the customer Dashboard, not this tab. Donation Options use a compact grid and omit **Unrestricted Giving** when **General Donation** is already listed. Summary cards: Lifetime Giving, Active Pledges, Outstanding Pledge Balance; **Next Payment** only appears when a scheduled installment exists. Deep links: `?tab=giving|pledges|payments`, `?campaign={id}&action=pledge`, `?campaign={id}&give=one-time|recurring`, `?tab=pledges&pledge={id}&action=pay`
...
* `/customer/donation` — **New Pledge** (My Pledges tab): required **campaign** + **total pledge amount** only; pledge date is set automatically. After creating the pledge, donors use **Make a Payment** (pay in full or any amount toward balance) or **Set Up Payment Plan** (monthly/quarterly/annually, number of payments, amount per payment, first payment date). Pledge cards show pledged / paid / remaining, progress, schedule, and next payment when applicable. Key files: `lib/customer/customer-pledge-actions.ts`, `lib/donations/pledge-payment-plan.ts`, migrations `158_pledge_payment_plan.sql`, `159_customer_pledge_plan_update.sql`
* **Admin/customer pledge alignment (July 2026 / August 2026):** Staff set or edit installment **payment plans** and record payments inside the canonical **Pledge Details** window (`components/donations/pledge-details-dialog.tsx`, `updatePledgePaymentPlanAction`, `recordPledgePaymentAction`). Plan summary and suggested pay amount match the customer portal. Admin `Yearly` frequency stores as `annually` for consistency. Customer portal **My Pledges** is unchanged.
...
| `donation_categories` | same (migration **`294`**) | same | customers SELECT their org (portal pickers) |
| `donation_subcategories` (funds) | same (migration **`294`**) | same | customers SELECT their org (portal pickers) |
...
* Campaign children cannot point at another org’s campaign (migration **`292`**). Apply the same `(parent_id, organization_id)` pattern to other modules (events, programs, directory) as more tenants go live. Service-role code must still filter `organization_id` on every query.
...
* `/donations/payments/transactions` — summary metric cards, giving charts (daily line for 7/30/90 day ranges, monthly for last year / all time), server-paginated payments table + search/status filters
* `/donations/campaigns/pledges` — server-paginated table; filters: status (default **Open** = any outstanding balance), campaign, minimum pledged amount; statuses are **Open** and **Fulfilled** only (a partly paid pledge stays Open). Pledge summary cards reflect the same filters and include overdue count plus collection % of pledged; KPI cards and table headers stay pinned while rows scroll; donor name opens contact profile in a modal (`ContactProfileDialog`). **Export CSV** downloads the current filters (donor, status, campaign) including last reminder and last contacted. Legacy `/donations/pledges` and `/donations/reports/pledges` redirect here.
* `/donations/reports/donors` — sidebar **Donors**; `DonorsReportPanel` via `donation_donor_giving_report` RPC: period (lifetime / calendar year / custom), **column header filters** (Donor, Email, Phone, Total Given min, Last Gift), email and phone columns, CSV + PDF export. Pledge status and outstanding balance live on **Pledges**, not this list. Last Gift filter options: all, gift within 12 months, no gift in 12+/24+ months, never gave. Apply migrations `127`–`146`, **`150`**, **`151`**, **`152`**, **`153`**, **`163`** (report prefers `contacts.full_name` over stale `donors.full_name`; backfills donor name/email/phone from linked contacts). Contact profile name edits sync the linked `donors` row (`syncDonorExtensionFromContact`).
...
Status: Implemented (September 2026 — one home per object)

* Sidebar: **Overview**, **Campaigns**, **Donors**, **Pledges**, **Donations**, **Settings** (`lib/navigation/donations-sidebar-children.ts`). There is no Reports sibling — unique analytics live on the object they describe.
* **Overview** — dashboard snapshot and shortcuts into the object homes
* **Campaigns** — `/donations/campaigns` list + workspace; in-page views **Campaigns** / **Campaign Groups** / **Wishlist** (`?view=groups` / `?view=wishlist`). Goal, committed, collected, outstanding, and donor counts are on this list.
* **Donors** — `/donations/reports/donors` (Individual / Household / Group). Title plus view/export tabs stay left-aligned on every view (no subtitle under Donor Giving). KPI cards stay pinned while the table scrolls. Table title is on the left (**Add Group** beside Group Giving) and the period dropdown is on the right. Group name and Last Gift columns sort. Group table columns: Group, Primary Contact, Combined Total, Gifts, Last Gift. Household Giving lists only families with two or more adult donors. Group workspace is `/donations/groups/[id]`. Pledge and Outstanding Balance columns are omitted — pledge detail stays on **Pledges**.
* **Pledges** — `/donations/campaigns/pledges` working list (add, collect, remind). Summary cards include overdue count and collection % of pledged.
* **Donations** (operations) — in-page tabs:
  * **Transactions** — `/donations/payments/transactions` (payment register + Receive Payment + giving charts; date range, CSV export, campaign/fund/receipt columns)
...
* Legacy report URLs keep working: `/donations/reports` and `/donations/reports/giving` → transactions (optional `?range=` preserved); `/donations/reports/pledges` → pledges; `/donations/reports/recurring-giving` → recurring; `/donations/reports/campaigns` → campaigns (preserves `view=groups|wishlist`); `/donations/reports/campaign-groups` → campaigns groups view; `/donations/reports/one-time` → transactions; `/donations/reports/recurring` → recurring; `/donations/reports/import` and `/match` → import-match; `/donations/reports/receipts` → receipts
...
* Person-scoped screens stay on contact **Financial** and the customer portal
...
| Route | Purpose |
|-------|---------|
| `/donations/campaigns` | Campaigns home — campaign list with goal/committed/collected/outstanding/donors; **Campaign Groups** and **Wishlist** views (`?view=groups` / `?view=wishlist`) |
| `/donations/campaigns/[id]` | Campaign workspace — Overview, Prospects, Pledges, Donations (Receive Donation + New Recurring Plan), Sponsorship (Sponsors \| Packages), Groups, Wishlist; `?tab=` / `?section=` / `?group=` |
...
| `/donations/reports/campaigns` | Redirects to `/donations/campaigns` (preserves `view=groups` / `view=wishlist`) |
...
| `/donations/campaigns/pledges` | Pledge list (filters: campaign, status default Open, min amount), add/edit/pay, last reminder/contacted columns, **Export CSV**, inline reminder actions, detail dialog; KPI cards and table headers stay pinned |
...
| `/directory/[id]?tab=financial` | Contact Financial; inner module via `?module=` |
...
| Route | Features |
|-------|----------|
| `/donations/payments/recurring` | Plan table (donor, category/fund, frequency, plan start/end, total payments, amount, payments made, status); MRR/ARR metrics; create plan; record payment. Status column filter defaults to **Active**. |
| `/donations/reports/recurring-giving` | Redirects to `/donations/payments/recurring` |
