...
* **Subscription catalog matches staff modules (August 2026 / September 2026):** Tenants subscribe to **Event Management**, **Programs**, **Vendor Hub**, **Venue Rentals**, **Fund Development**, and **Membership**. **Workforce**, **Finance**, **Facilities**, and **Community Calendar** are not sellable SKUs. Administration (`workforce`) is included with Programs or Event Management; Finance is included with Programs; Facilities (`spaces`) is included with Programs, Event Management, Venue Rentals, and Vendor Hub; Community Calendar is included with Vendor Hub or Event Management. Membership and Fund Development do not include Administration, Facilities, or Community Calendar. SQL **`273_subscription_modules_match_staff_nav.sql`**, **`298_administration_programs_events_capability.sql`**. Keys: `lib/modules/module-catalog.ts`, `lib/modules/staff-module-labels.ts`.
...
**Bazaar create/edit organizer + space (August 2026 / September 2026):** Create/Edit Bazaar Event drawer uses real CRM contacts (`HrContactPicker` + quick-add) and Facilities venues instead of mock dropdowns. Persists `organizer_contact_id`, `organizer_name`, and `venue_id` on `vendor_hub_events`. On-site space is optional: picking a facility writes a date/place hold (`internal_events.source_module = vendor_hub`) so Facilities can block the room; off-site skips Facilities. There is no Event Management picker, ticketing, or program link. Run **`scripts/227_vendor_hub_event_organizer_venue.sql`**, **`scripts/300_event_management_source_of_truth.sql`**, and **`scripts/305_bazaar_owned_internal_events.sql`**.

**Vendor Hub Reports (August 2026):** `/vendor-hub/reports` no longer uses hardcoded sample vendors/revenue. Tabs are **Vendor Sales**, **Booth Performance**, and **Participation History** (optional `?eventId=` on sales/booth; `?tab=history` and optional `?contact=`). Former Reports Overview KPIs/tables live on Vendor Hub **Overview**. Empty states when there are no assignments/payments. Vendor names come from `contacts.full_name` (Directory names live on `people`, not `contacts.first_name`). **Booth Performance** uses organization default booth types from Settings when the event has that layout (totals from numbered inventory, not Eventbrite ticket-type counts). Key files: `lib/vendor-hub/vendor-hub-reports-queries.ts`, `components/vendor-hub/vendor-hub-reports-client.tsx`, `vendor-hub-reports-overview-panels.tsx`.
...
**Vendor Hub Overview (August 2026):** `/vendor-hub` is **organization-level** (not event-scoped). Sidebar label is **Overview** (exact match). One KPI row: Onboarding pending, Active vendors (Vendor Network last activity within 2 years), Outstanding balance (all events), Total revenue, Food category. Below: Revenue by Category, Top Performing Vendors, Upcoming events (today + future, sorted by date — click into event workspace), and org Quick Actions (Create Vendor Event, Network, Onboarding, Participation History → Reports, Calendar, Reports). Event overview KPIs: Booth occupancy, **Booth registrations** (distinct vendors registered for the event), Revenue collected, Evaluations pending. Key: `getVendorHubOrgDashboard`, `getVendorHubReportsData`, `getVendorHubDashboardMetrics`, `components/vendor-hub/vendor-hub-dashboard-client.tsx`, `bazaar-event-overview-client.tsx`.
