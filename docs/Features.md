...
* **Reports** — tab bar (`components/donations/donation-reports-nav.tsx`):
  * **One-Time Donations** — `/donations/reports/one-time` (summary metric cards + server-paginated payments table)
  * **Recurring Donations** — `/donations/reports/recurring`
  * **Pledges** — `/donations/reports/pledges` (pledge table, filters, collection queue, add/edit pledge dialogs)
  * **Donors** — `/donations/reports/donors` (giving report + CSV export)
