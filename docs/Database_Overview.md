...
**Staff pay basis (migration `169_staff_pay_basis.sql` + `293_staff_pay_basis_unpaid.sql`):** `staff.pay_basis` (`hourly` | `monthly` | `unpaid`) and optional `staff.monthly_salary`. Unpaid / volunteer keeps `hourly_rate` and `monthly_salary` so pay can be restored later. Payroll skips unpaid staff.
...
* program_schedule_items — S3 `offering_id` required (`178`); weekly class times edited on offering Schedule tab; optional `venue_id` for shared facility calendar/conflicts (`209`). Saving Delivery as **Online** clears `venue_id` and `location` so the class is released from the Facilities calendar.
...
* venues — Spaces catalog under Facilities → Settings. **`color` + `flyer_url` (migration `204`):** card branding on Spaces settings (3-column grid like Departments). **Required:** run `scripts/204_venue_color_flyer.sql` in Supabase SQL Editor (includes `NOTIFY pgrst, 'reload schema'`) or color/flyer will not persist. **Per-day hours/rates:** `rental_space_pricing` (from `046`; seed `205`) — Sunday–Saturday open hours + flat/hourly; Spaces edit form replaces peak/non-peak buckets. **Setup/cleanup overrides (`222`):** nullable `venues.setup_minutes` / `cleanup_minutes` (NULL inherits org defaults on `venue_rental_settings`). **Calendar:** Facilities sidebar **Calendar** is `/facilities/calendar` (merged former Space Availability + Schedule); `/facilities/availability` redirects there. Views: Day, Grid, and List (`?view=list` — agenda with time, space, holder, title, PoC; custom range via `?date=` and `?endDate=`). Module filtered views use `?sources=` against the same `resource_reservations` (+ program expand). **Overview:** `/facilities/overview` is the Facilities landing (read-only schedule metrics). **Inventory:** `facility_inventory_items` (migrations `207` + `208`) — Facilities → Inventory catalog with category, size/style/color, quantity, location, purchased_at, unit_cost, notes, active, sort_order. **Shared scheduling (migration `209`):** `program_schedule_items.venue_id`; `setup_minutes` / `cleanup_minutes` on `rental_reservations` and `internal_events` expand occupied windows in sync triggers to `resource_reservations`. Run **`scripts/209_shared_scheduling_foundation.sql`** after `208`. **Event location types (`210`):** `internal_events.location_type`, `location_address`. **Multi-venue events (`211`):** `internal_event_venues` junction; sync creates one `resource_reservations` row per venue; `internal_events.venue_id` remains primary. Run **`210`** then **`211`**. If submit fails with ON CONFLICT on resource_reservations, run **`212_fix_internal_event_sync_on_conflict.sql`**.
...
* internal_events — **`flyer_url` (migration `214`):** optional event flyer on workspace Overview; uploads reuse `program-flyers` storage. Run `scripts/214_internal_event_flyer_url.sql` in Supabase SQL Editor (`NOTIFY pgrst, 'reload schema'`). **`flyer_focal_x` / `flyer_focal_y` (migration `249`):** Community Calendar featured banner crop (object-position %). **`community_calendar_status` (migration `247`):** Community Calendar visibility — UI is **Private** / **Public** (`not_published` | `published`; legacy `community_visible` still accepted). Set on event Settings → General or bazaar create/edit. Community Calendar lists published `internal_events` including bazaar holds. **`campaign_id` (`300`):** Fund Development campaign FK (`internal_events_campaign_org_fkey`); JSON `ticketing_config.linkedCampaignId` is kept in sync for older builds. **`source_module` (`305`):** `event_management` (Event Management workspace) or `vendor_hub` (bazaar date/place hold for Facilities + Community Calendar; hidden from Event Management lists). **Event Workspace redesign (`252`):** `workspace_features` (JSONB toggles), `audience` / `event_tags` (text[]), `coordinator_contact_id`, `estimated_attendance`, `internal_notes`; attendance mode lives in `ticketing_config.attendanceMode`; optional **`ticketing_config.communications`** for confirmation/reservation email subject + message overrides. **`ticketing_category_id` (`287`):** optional Ticketing Events category (`ticketing_event_categories`); ON DELETE SET NULL. **Internal Events Google Form import (`INTERNAL_EVENTS_XLSX_V1`, September 2026):** `scripts/import-internal-events-xlsx.mjs` writes the tag into `internal_notes` for idempotent re-runs. **Public recurring gatherings (`INTERNAL_EVENTS_RECURRING_PUBLIC_V1`, September 2026):** `scripts/import-internal-events-recurring-public.mjs` — one row per meeting; published series use `community_calendar_status = published` and `ticketing_config.attendanceMode = open_public`.
* ticketing_event_categories — **`287`:** org-managed labels for grouping ticketed events (Kids Workshop, I Pray Party, Bazaar, …). Unique `(organization_id, slug)`. Staff add/rename/hide/delete on Ticketing → Settings. Separate from `event_types`.
...
* childcare_events — signup sheet for an Event Management event or program, not a second event list. **`300`:** `source_type` + `source_id` required. Staff create only from an event or program; there is no standalone Add Event.
...
**Campaign prospects (migration `262_campaign_prospects.sql`, extended `284_campaign_sponsorship_prospects.sql`, assignee name `299_campaign_prospect_assigned_to_name.sql`):** `campaign_prospects` (org + campaign scoped; FK to contacts, optional ask level / assignee contact / assignee name / event / package / converted pledge / converted sponsorship). `ask_type` (`donation` | `sponsorship`; existing rows default donation). Unique `(campaign_id, contact_id, ask_type)`. Prospects lists donation rows only. Add/edit uses a popup for ask amount, typed **Assigned to** name (not a Directory search), last contact, next follow-up, and notes; the table is read-only. `assigned_to_name` is the staff-facing assignee; `assigned_to_contact_id` stays for a later employee lookup. Nullable `pledges.campaign_prospect_id`. Outreach history: `campaign_prospect_activities` (table kept; not shown on the Prospects tab). RLS via donations view/manage helpers.
...
npx supabase db query --linked -f scripts/300_event_management_source_of_truth.sql
...
campaign_prospects.assigned_to_name → typed employee name (`299`)
...
* vendor_hub_booth_setup_templates / `_lines` — reusable layouts. Lines may set `booth_numbers` (T1, T15) and `selection_fee` (`301`). MAS Dallas seed: **Default bazaar layout** rebuilt from default types (`302`; slug may still be `mas-dallas-fall-bazaar-layout`).
