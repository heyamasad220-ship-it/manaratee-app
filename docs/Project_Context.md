...
**Household Giving multi-adult filter (September 2026)** — Reports → Donor Giving → Household Giving lists only families with two or more adult Directory contacts who each donated in the period (`scripts/295_household_giving_multi_adult.sql`). One-parent program households remain on Directory → Families.
...
**Fund Development Prospects nav (September 2026)** — **Prospects** (`?tab=prospects`) has no Strategy sub-nav. Pinned KPIs: prospects, total ask, overdue follow-ups, pledged. Legacy `?tab=plan`, `?tab=strategy`, and `section=prospects` redirect to `?tab=prospects`. Key files: `campaign-fundraising-plan-tab.tsx`, `campaign-fundraising-plan-table.tsx`, `campaign-fundraising-plan-dialog.tsx`.
