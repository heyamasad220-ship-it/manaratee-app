...
import { ChildcareProviderApplyClient } from "@/components/customer/childcare-provider-apply-client"
...
export default async function CustomerChildcareApplyPage() {
      ...
      <div className="mx-auto max-w-3xl space-y-4 p-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Sign in to your community account to apply as a childcare provider.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
      ...
      <div className="mx-auto max-w-3xl space-y-4 p-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Join or select an organization before submitting a childcare provider application.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
      ...
      <div className="mx-auto max-w-3xl space-y-4 p-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Your account needs a linked contact before you can submit a childcare provider
              application. Complete your profile first, then return here.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button asChild>
  ...
  const { data: contactDetails } = await supabase
    .from("contacts")
    .select("address, city, state, zip, email, phone, full_name")
    .eq("organization_id", organizationId)
    .eq("id", contact.id)
    .maybeSingle()

  const existingApplications = (await fetchContactApplications(contact.id)).filter(
    (app) => app.application_type === "childcare_provider"
  )
