...
import { saveEventBoothSetupAsTemplate } from "@/lib/vendor-hub/booth-template-actions"
...
export default function BazaarBoothsPanel({ eventId }: { eventId: string }) {
  ...
  async function saveAsTemplate() {
    const name = window.prompt("Template name", "Standard Bazaar Setup")
    if (!name?.trim()) return

    try {
      await saveEventBoothSetupAsTemplate({
        eventId,
        name: name.trim(),
      })
      alert("Booth setup saved as a reusable template.")
    } catch (error) {
      alert(error instanceof Error ? error.message : "Could not save template.")
    }
  }
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" className="gap-2" onClick={() => void saveAsTemplate()}>
                Save as Template
              </Button>
