...
import {
  ...
  contactProfilePath,
  type LegacyVendorHubVendorRef,
  type VendorHubContactRef,
} from "@/lib/vendor-hub/contact-centric-model"
...
type VendorHubVendor = LegacyVendorHubVendorRef
...
type BoothAssignment = {
  ...
  status: string | null
  contacts?: VendorHubContactRef | null
...
async function loadBoothAssignmentsForEvent(
  supabase: ReturnType<typeof createClient>,
  eventId: string
): Promise<{ assignments: BoothAssignment[]; error: unknown | null }> {
  ...
  const boothIds = [...new Set(rows.map((row) => row.booth_id).filter(Boolean))] as string[]
  ...
  const [boothsResult, vendorsResult] = await Promise.all([
    boothIds.length > 0
      ? supabase.from("vendor_hub_booths").select("id, number, location, booth_type_id").in("id", boothIds)
      : Promise.resolve({ data: [], error: null }),
    vendorIds.length > 0
      ? supabase
          .from("vendor_hub_vendors")
          .select("id, business_name, contact_name, email, phone, status, contact_id")
          .in("id", vendorIds)
      : Promise.resolve({ data: [], error: null }),
  ])

  const boothTypeIds = [
    ...new Set((boothsResult.data ?? []).map((booth) => booth.booth_type_id).filter(Boolean)),
  ] as string[]

  const vendorContactIds = [
    ...new Set((vendorsResult.data ?? []).map((vendor) => vendor.contact_id).filter(Boolean)),
  ] as string[]

  const allContactIds = [...new Set([...contactIds, ...vendorContactIds])]

  const [boothTypesResult, contactsResult] = await Promise.all([
    boothTypeIds.length > 0
      ? supabase
          .from("vendor_hub_booth_types")
          .select("id, name, size, price, color")
          .in("id", boothTypeIds)
      : Promise.resolve({ data: [], error: null }),
  ...
  const boothById = new Map((boothsResult.data ?? []).map((booth) => [booth.id, booth]))
  const boothTypeById = new Map((boothTypesResult.data ?? []).map((type) => [type.id, type]))
  ...
  const assignments = rows.map((row) => {
    const booth = row.booth_id ? boothById.get(row.booth_id) : null
    const boothType = booth?.booth_type_id ? boothTypeById.get(booth.booth_type_id) : null
    const vendor = row.vendor_id ? vendorById.get(row.vendor_id) : null
    const contactId = row.contact_id ?? vendor?.contact_id ?? null
    ...
    return {
      ...
      contacts: contact,
      vendor_hub_vendors: vendor ?? null,
      vendor_hub_booths: booth
        ? {
            ...booth,
            vendor_hub_booth_types: boothType ?? null,
          }
...
export default function BazaarPaymentsPanel({ eventId }: { eventId: string }) {
  ...
  async function loadPaymentData(eventId: string) {
    ...
    const { assignments: assignmentData, error: assignmentError } =
      await loadBoothAssignmentsForEvent(supabase, eventId)
    ...
    if (assignmentError) {
      logSupabaseError("Error loading booth assignments:", assignmentError)
      setAssignments([])
    } else {
    ...
    if (paymentError) {
      logSupabaseError("Error loading payments:", paymentError)
      setPayments([])
    } else {
    ...
    return assignments.map((assignment) => {
      ...
      return {
        ...
        contactId:
          ...
          assignment.contacts?.id ??
          assignment.vendor_hub_vendors?.contact_id ??
          null,
        vendorName: resolveParticipantDisplayName({
          contact: assignment.contacts,
          legacyVendor: assignment.vendor_hub_vendors,
        }),
  ...
  async function recordPayment() {
    ...
    const { error } = await supabase.from("vendor_hub_payments").insert({
      ...
      contact_id:
        selectedPaymentRow.contactId ??
        selectedPaymentRow.assignment.contact_id ??
        null,
  ...
  async function issueRefund(row: PaymentRow) {
    ...
    const { error } = await supabase.from("vendor_hub_payments").insert({
      ...
      contact_id: row.contactId ?? row.assignment.contact_id ?? null,
    ...
    <>
      <div className="flex flex-col gap-6">
        <p className="text-sm text-muted-foreground">
          Track vendor booth fees, balances, payments, and refunds for this bazaar event.
        </p>
                      ...
                      return (
                        <TableRow key={payment.assignment.id}>
                          <TableCell className="font-medium">
                            {payment.contactId ? (
                              <Link
                                href={contactProfilePath(payment.contactId)}
