...
import { searchGroupsForDonationPickerAction } from "@/lib/contacts/group-giving-actions"
...
export function DonationGroupPicker({
  ...
  const loadGroups = useCallback(async (term: string) => {
    setLoading(true)
    const result = await searchGroupsForDonationPickerAction(term, 30)
    setLoading(false)
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        Count this gift toward a group total. The donor is added as a group member automatically.
      </p>
    </div>
  )
}
