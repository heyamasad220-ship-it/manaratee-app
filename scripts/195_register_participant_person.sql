-- Youth registration by person (Contact + Participant model).
-- Minors are people under the parent Contact — they must not require a CRM contact.
--
-- Run in Supabase SQL editor after 179 / 194.
--
-- Changes:
--   1) is_registrant_related_person — family check by person_id
--   2) quote_registrant_has_sibling_enrollment — works when participant_contact_id is null
--   3) register_for_program — optional p_participant_person_id; minors keep participant_contact_id null

CREATE OR REPLACE FUNCTION public.is_registrant_related_person(
...
AS $$
  SELECT EXISTS (
    ...
    WHERE pr.organization_id = p_organization_id
      ...
      AND pr.related_person_id = p_related_person_id
  );
$$;
...
AS $$
  SELECT EXISTS (
    ...
    WHERE e.organization_id = p_organization_id
      ...
      AND (
        CASE
          WHEN p_participant_contact_id IS NOT NULL THEN
            e.participant_contact_id IS DISTINCT FROM p_participant_contact_id
          ELSE
            -- Person-only minors: set app.quote_participant_person_id before quoting
            -- (see register_for_program) so the same child is not treated as a sibling.
            e.child_person_id IS DISTINCT FROM NULLIF(
              current_setting('app.quote_participant_person_id', true),
              ''
            )::uuid
        END
      )
...
DROP FUNCTION IF EXISTS public.register_for_program(
...
GRANT EXECUTE ON FUNCTION public.register_for_program(
  uuid, uuid, uuid, uuid, uuid[], text, text, text, text, text, boolean, boolean, text, uuid, numeric, text, uuid
) TO authenticated;

COMMENT ON FUNCTION public.is_registrant_related_person(uuid, uuid, uuid) IS
  'True when related person is in the registrant household (person_relationships).';

COMMENT ON FUNCTION public.register_for_program(
  uuid, uuid, uuid, uuid, uuid[], text, text, text, text, text, boolean, boolean, text, uuid, numeric, text, uuid
) IS
  'Customer registration. Youth prefers p_participant_person_id; minors leave participant_contact_id null.';

-- ---------------------------------------------------------------------------
-- quote_program_registration: allow youth quotes by person_id (no minor contact)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  v_participant_person_id := NULLIF(p_addons->>'participant_person_id', '')::uuid;
  ...
  ELSE
    ...
    ELSIF v_participant_person_id IS NOT NULL THEN
      IF v_registrant.person_id IS NULL OR NOT public.is_registrant_related_person(
        p_organization_id,
        v_registrant.person_id,
        v_participant_person_id
      ) THEN
