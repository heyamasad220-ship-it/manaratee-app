-- Donor giving report: prefer contacts.full_name over stale donors.full_name.
-- Also backfill donors.full_name / email / phone from linked contacts.
-- Run after 153_donor_giving_report_last_gift_filter.sql

-- Backfill denormalized donor profile fields from the linked contact.
UPDATE public.donors d
SET
  full_name = COALESCE(NULLIF(btrim(c.full_name), ''), d.full_name),
  email = COALESCE(NULLIF(btrim(c.email), ''), d.email),
  phone = COALESCE(NULLIF(btrim(c.phone), ''), d.phone)
FROM public.contacts c
WHERE c.id = d.contact_id
  AND (
    COALESCE(d.full_name, '') IS DISTINCT FROM COALESCE(NULLIF(btrim(c.full_name), ''), d.full_name)
    OR COALESCE(d.email, '') IS DISTINCT FROM COALESCE(NULLIF(btrim(c.email), ''), d.email)
    OR COALESCE(d.phone, '') IS DISTINCT FROM COALESCE(NULLIF(btrim(c.phone), ''), d.phone)
  );
...
CREATE OR REPLACE FUNCTION public.donation_donor_giving_report(
  ...
  p_last_gift_filter text DEFAULT 'all',
...
AS $$
DECLARE
  v_last_gift_cutoff_12 date := (CURRENT_DATE - INTERVAL '12 months')::date;
  v_last_gift_cutoff_24 date := (CURRENT_DATE - INTERVAL '24 months')::date;
  v_last_gift_filter text := COALESCE(NULLIF(btrim(p_last_gift_filter), ''), 'all');
...
BEGIN
  ...
  donor_rows AS (
    SELECT
      d.id,
      d.contact_id,
      COALESCE(NULLIF(btrim(c.full_name), ''), d.full_name) AS full_name,
      NULLIF(btrim(COALESCE(c.email, d.email, '')), '') AS email,
      NULLIF(btrim(COALESCE(c.phone, d.phone, '')), '') AS phone,
      d.donor_type,
  ...
  filtered AS (
    ...
    WHERE
      ...
      AND (
        v_last_gift_filter = 'all'
        OR (v_last_gift_filter = 'active_12m' AND dr.lifetime_last_donation_date >= v_last_gift_cutoff_12)
        OR (
          v_last_gift_filter = 'lapsed_12m'
          AND (
            dr.lifetime_last_donation_date IS NULL
            OR dr.lifetime_last_donation_date < v_last_gift_cutoff_12
          )
        )
        OR (
          v_last_gift_filter = 'lapsed_24m'
          AND (
            dr.lifetime_last_donation_date IS NULL
            OR dr.lifetime_last_donation_date < v_last_gift_cutoff_24
          )
        )
        OR (v_last_gift_filter = 'never' AND dr.lifetime_last_donation_date IS NULL)
      )
      AND (
...
GRANT EXECUTE ON FUNCTION public.donation_donor_giving_report(
  uuid, date, date, text, text, text, text, text, text, text, text, numeric, text, boolean, integer, integer
) TO authenticated, service_role;

GRANT EXECUTE ON FUNCTION public.donation_donor_giving_report_summary(
  uuid, date, date, text, text, text, text, text, text, text, text, numeric
) TO authenticated, service_role;
