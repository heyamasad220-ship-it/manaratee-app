-- Donor affiliation requires at least one payment (pledge or donors row alone is insufficient).
-- Run after 112_patch_sync_contact_affiliations_vendor_guard.sql
-- Safe to re-run.
...
COMMENT ON FUNCTION public.sync_contact_affiliations IS
  'SECURITY DEFINER: reconcile derived contact_roles. Donor requires payment (114 patch).';

-- Remove auto-assigned donor tags where no payment exists (pledge-only / extension-only cases).
...
WHERE cr.role = 'donor'
  ...
  AND NOT EXISTS (
    ...
    WHERE p.organization_id = cr.organization_id
      AND (
        p.contact_id = cr.contact_id
        OR EXISTS (
          SELECT 1
          FROM public.donors d
          WHERE d.id = p.donor_id
            AND d.organization_id = cr.organization_id
            AND d.contact_id = cr.contact_id
        )
      )
  );
