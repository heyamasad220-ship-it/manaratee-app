-- Patch sync_contact_affiliations: remove public.vendors.contact_id derivation.
-- public.vendors is a legacy directory table without contact_id on staging.
-- Canonical vendor identity: approved vendor applications (applications.contact_id).
...
BEGIN
  ...
  -- Vendor: approved applications only (no public.vendors.contact_id linkage)
  IF EXISTS (
...
COMMENT ON FUNCTION public.sync_contact_affiliations IS
  'SECURITY DEFINER: reconcile derived contact_roles. Vendor via approved applications only (112 patch).';
