-- Fix donor giving report RPCs: cast payment_date to date, use net payment amounts.
-- Resolves: "structure of query does not match function result type" on Reports → Donors.
-- Run after 128_donor_giving_report_contact_id.sql (and 125 for payment_net_amount).
...
COMMENT ON FUNCTION public.donation_donor_giving_report IS
  'Paginated donor giving report (net amounts, date casts). Includes contact_id for /contacts/[id] links.';
