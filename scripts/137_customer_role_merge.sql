-- Merge program_participant, event_attendee, and venue_rental_customer into customer.
-- Run after 136_payment_attributed_group.sql

-- ---------------------------------------------------------------------------
-- Backfill contact_roles
-- ---------------------------------------------------------------------------
INSERT INTO public.contact_roles (organization_id, contact_id, role, is_manual)
SELECT cr.organization_id, cr.contact_id, 'customer', bool_or(COALESCE(cr.is_manual, false))
FROM public.contact_roles cr
WHERE cr.role IN ('program_participant', 'event_attendee', 'venue_rental_customer')
GROUP BY cr.organization_id, cr.contact_id
ON CONFLICT DO NOTHING;

DELETE FROM public.contact_roles
WHERE role IN ('program_participant', 'event_attendee', 'venue_rental_customer');

-- ---------------------------------------------------------------------------
-- Migrate org affiliation settings (preserve most restrictive explicit toggle)
-- ---------------------------------------------------------------------------
INSERT INTO public.organization_affiliation_settings (organization_id, role, auto_sync_enabled)
SELECT
  oas.organization_id,
  'customer',
  bool_and(oas.auto_sync_enabled)
FROM public.organization_affiliation_settings oas
WHERE oas.role IN ('program_participant', 'event_attendee')
GROUP BY oas.organization_id
ON CONFLICT (organization_id, role) DO UPDATE
SET auto_sync_enabled = EXCLUDED.auto_sync_enabled,
    updated_at = NOW();

DELETE FROM public.organization_affiliation_settings
WHERE role IN ('program_participant', 'event_attendee');

-- ---------------------------------------------------------------------------
-- contact_roles.role CHECK — drop legacy participation roles
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE public.contact_roles IS
  'Contact roles. Derived roles sync from module activity; staff may override manual labels (is_manual). Sticky roles: donor, volunteer, vendor, customer.';
...
-- ---------------------------------------------------------------------------
-- Default auto-sync for customer (programs, events/ticketing, venue rentals)
-- ---------------------------------------------------------------------------
...
BEGIN
  CASE p_role
    ...
    WHEN 'customer' THEN
      RETURN public.org_has_any_module_slug(
        p_organization_id,
        ARRAY['programs', 'event-management', 'ticketing', 'bookings']
      );
    ELSE
      RETURN false;
  END CASE;
...
-- ---------------------------------------------------------------------------
-- sync_contact_affiliations — unified customer derivation
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.sync_contact_affiliations IS
  'SECURITY DEFINER: reconcile derived contact_roles. Customer role covers programs, events, and venue rentals (137).';
