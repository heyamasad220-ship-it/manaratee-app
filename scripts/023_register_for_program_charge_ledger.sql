-- Phase 2B: Wire register_for_program to charge ledger
-- Run after 022_program_charge_line_admin.sql
--
-- On enrollment:
--   • Resolves initial status (pending vs pending_payment)
--   • Creates program_charges + lines + schedule from quote
--   • Sets charge_id, payment_required, capacity_hold_type on enrollment

DO $$
BEGIN
  IF to_regclass('public.program_charges') IS NULL THEN
    RAISE EXCEPTION '023: program_charges missing — run 020 first';
  END IF;

  IF to_regprocedure(
    'public.build_program_charge_from_quote(uuid,jsonb,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid,date)'
  ) IS NULL
  AND to_regprocedure(
    'public.build_program_charge_from_quote(uuid,jsonb,uuid,uuid,uuid,uuid,uuid,uuid,uuid,uuid)'
  ) IS NULL THEN
    RAISE EXCEPTION '023: build_program_charge_from_quote missing — run 020/021 first';
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 1) Capacity hold helper
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.resolve_enrollment_capacity_hold_type(
...
DECLARE
  v_settings record;
BEGIN
  IF lower(COALESCE(p_initial_status, '')) = 'pending_payment' THEN
    SELECT * INTO v_settings
    FROM public.program_payment_settings s
    WHERE s.organization_id = p_organization_id;

    IF FOUND AND COALESCE(v_settings.hold_capacity_on_pending_payment, true) THEN
      RETURN 'soft';
    END IF;
...
-- ---------------------------------------------------------------------------
-- 2) Backfill charges for existing enrollments (staff only)
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.staff_backfill_enrollment_charges(
  p_organization_id uuid,
...
BEGIN
  ...
  FOR v_row IN
    ...
    WHERE e.organization_id = p_organization_id
      AND e.charge_id IS NULL
      AND e.quote_snapshot IS NOT NULL
      AND COALESCE(e.quote_snapshot->>'ok', 'false') = 'true'
  ...
  LOOP
    v_charge_id := public.build_program_charge_from_quote(
      p_organization_id,
      v_row.quote_snapshot,
      v_row.id,
      v_row.program_id,
      v_row.offering_id,
      v_row.registration_option_id,
      v_row.payer_contact_id,
      v_row.registrant_contact_id,
      v_row.participant_contact_id,
      NULL,
      COALESCE(v_row.enrollment_date, CURRENT_DATE)
    );
...
GRANT EXECUTE ON FUNCTION public.staff_backfill_enrollment_charges(uuid, integer) TO authenticated;

-- ---------------------------------------------------------------------------
-- 3) register_for_program — auto-create charge ledger
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
SELECT
  to_regprocedure('public.resolve_enrollment_capacity_hold_type(uuid,boolean,text)') IS NOT NULL
    AS capacity_hold_helper_exists,
  to_regprocedure('public.staff_backfill_enrollment_charges(uuid,integer)') IS NOT NULL
