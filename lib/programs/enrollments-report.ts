...
import { getOpenPrograms, getProgramById } from "@/lib/programs/program-queries"
import {
...
export async function getEnrollmentsReportRows(filters?: {
  programId?: string
}): Promise<
  | { success: true; rows: EnrollmentsReportTableRow[] }
...
> {
  ...
  const [programs, departments] = await Promise.all([
    filters?.programId
      ? getProgramById(filters.programId).then((program) =>
          program ? [program] : []
        )
      : getOpenPrograms(),
    getDepartments(),
  ...
  if (enrollmentsResult.error) {
    return { success: false, error: enrollmentsResult.error.message }
  }
