...
export function selectExpiredHoldRentals(
  ...
  const holdStatuses = new Set<string>(VENUE_RENTAL_HOLD_PAYMENT_STATUSES)
...
/**
 * Expire unpaid holds within an optional single-organization scope.
 * Idempotent: only updates rentals still in hold-payment statuses with elapsed hold_expires_at.
 */
export async function expireVenueRentalHoldsForScope(input: {
  ...
  let query = input.supabase
    ...
    .in("status", VENUE_RENTAL_HOLD_PAYMENT_STATUSES)
